/* ../netlib/ztfttp.f -- translated by f2c (version 20100827). You must link the resulting object
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/ztfttp.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/ztfttp.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/ztfttp.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
{
#if FLA_ENABLE_ILP64
    aocl_lapack_ztfttp(transr, uplo, n, arf, ap, info);
#else
    aocl_int64_t n_64 = *n;
    aocl_int64_t info_64 = *info;

    aocl_lapack_ztfttp(transr, uplo, &n_64, arf, ap, &info_64);

    *info = (aocl_int_t)info_64;
#endif
}

void aocl_lapack_ztfttp(char *transr, char *uplo, aocl_int64_t *n, dcomplex *arf,
                        dcomplex *ap, aocl_int64_t *info)
{
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("ZTFTTP", &i__1, (ftnlen)6);
            ...
            if(lower)
            {
                ...
                for(i__ = 0; i__ <= i__1; ++i__)
                {
                    ...
                    for(ij = i__ * (lda + 1); i__3 < 0 ? ij >= i__2 : ij <= i__2; ij += i__3)
                    {
                        i__4 = ijp;
                        d_cnjg(&z__1, &arf[ij]);
                        ap[i__4].real = z__1.real;
                        ap[i__4].imag = z__1.imag; // , expr subst
                        ++ijp;
                    }
                }
                js = 1;
