...
/* ./cunbdb2.f -- translated by f2c (version 20190311). You must link the resulting object file with
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/cunbdb2
 * .f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/cunbdb2
 * .f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/cunbdb2
 * .f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
void cunbdb2_(aocl_int_t *m, aocl_int_t *p, aocl_int_t *q, scomplex *x11, aocl_int_t *ldx11,
...
{
#if FLA_ENABLE_ILP64
    aocl_lapack_cunbdb2(m, p, q, x11, ldx11, x21, ldx21, theta, phi, taup1, taup2, tauq1, work,
                        lwork, info);
#else
    aocl_int64_t m_64 = *m;
    aocl_int64_t p_64 = *p;
    ...
    aocl_lapack_cunbdb2(&m_64, &p_64, &q_64, x11, &ldx11_64, x21, &ldx21_64, theta, phi, taup1,
...
void aocl_lapack_cunbdb2(aocl_int64_t *m, aocl_int64_t *p, aocl_int64_t *q, scomplex *x11,
                         ...
                         aocl_int64_t *lwork, aocl_int64_t *info)
{
...
#if FLA_ENABLE_ILP64
    snprintf(buffer, 256,
             "cunbdb2 inputs: m %lld, p %lld, q %lld, ldx11 %lld, ldx21 %lld, lwork %lld", *m, *p,
             *q, *ldx11, *ldx21, *lwork);
#else
    snprintf(buffer, 256, "cunbdb2 inputs: m %d, p %d, q %d, ldx11 %d, ldx21 %d, lwork %d", *m, *p,
             *q, *ldx11, *ldx21, *lwork);
#endif
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("CUNBDB2", &i__1, (ftnlen)7);
    ...
    for(i__ = 1; i__ <= i__1; ++i__)
    {
        ...
        {
            i__2 = *q - i__ + 1;
            aocl_blas_csrot(&i__2, &x11[i__ + i__ * x11_dim1], ldx11,
        ...
        i__3 = *m - *p - i__ + 1;
        /* Computing 2nd power */
        r__2 = aocl_blas_scnrm2(&i__3, &x21[i__ + i__ * x21_dim1], &c__1);
        s = sqrt(r__1 * r__1 + r__2 * r__2);
        theta[i__] = atan2(s, c__);
        i__2 = *p - i__;
        i__3 = *m - *p - i__ + 1;
        i__4 = *q - i__;
        aocl_lapack_cunbdb5(&i__2, &i__3, &i__4, &x11[i__ + 1 + i__ * x11_dim1], &c__1,
                            &x21[i__ + i__ * x21_dim1], &c__1, &x11[i__ + 1 + (i__ + 1) * x11_dim1],
                            ldx11, &x21[i__ + (i__ + 1) * x21_dim1], ldx21, &work[iorbdb5],
                            &lorbdb5, &childinfo);
        i__2 = *p - i__;
        aocl_blas_cscal(&i__2, &c_b1, &x11[i__ + 1 + i__ * x11_dim1], &c__1);
        i__2 = *m - *p - i__ + 1;
