...
void stbtrs_(char *uplo, char *trans, char *diag, aocl_int_t *n, aocl_int_t *kd, aocl_int_t *nrhs,
             real *ab, aocl_int_t *ldab, real *b, aocl_int_t *ldb, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_stbtrs(uplo, trans, diag, n, kd, nrhs, ab, ldab, b, ldb, info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_stbtrs(uplo, trans, diag, &n_64, &kd_64, &nrhs_64, ab, &ldab_64, b, &ldb_64,
                       &info_64);
...
void aocl_lapack_stbtrs(char *uplo, char *trans, char *diag, aocl_int64_t *n, aocl_int64_t *kd,
                        aocl_int64_t *nrhs, real *ab, aocl_int64_t *ldab, real *b,
                        aocl_int64_t *ldb, aocl_int64_t *info)
{
    AOCL_DTL_TRACE_LOG_INIT
    AOCL_DTL_SNPRINTF("stbtrs inputs: uplo %c, trans %c, diag %c, n %" FLA_IS ", kd %" FLA_IS
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("STBTRS", &i__1, (ftnlen)6);
    ...
    *info = 0;
    ...
    for(j = 1; j <= i__1; ++j)
    {
        aocl_blas_stbsv(uplo, trans, diag, n, kd, &ab[ab_offset], ldab, &b[j * b_dim1 + 1], &c__1);
        /* L30: */
    }
    AOCL_DTL_TRACE_LOG_EXIT
    return;
