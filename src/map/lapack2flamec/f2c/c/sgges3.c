...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/sgges3.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/sgges3.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/sgges3.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
/* > */
/* > Note that in the ill-conditioned case, a selected scomplex */
...
void sgges3_(char *jobvsl, char *jobvsr, char *sort, L_fps3 selctg, aocl_int_t *n, real *a,
             aocl_int_t *lda, real *b, aocl_int_t *ldb, aocl_int_t *sdim, real *alphar,
             real *alphai, real *beta, real *vsl, aocl_int_t *ldvsl, real *vsr, aocl_int_t *ldvsr,
             real *work, aocl_int_t *lwork, logical *bwork, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_sgges3(jobvsl, jobvsr, sort, selctg, n, a, lda, b, ldb, sdim, alphar, alphai, beta,
    ...
    aocl_lapack_sgges3(jobvsl, jobvsr, sort, selctg, &n_64, a, &lda_64, b, &ldb_64, &sdim_64,
                       alphar, alphai, beta, vsl, &ldvsl_64, vsr, &ldvsr_64, work, &lwork_64, bwork,
                       &info_64);
...
void aocl_lapack_sgges3(char *jobvsl, char *jobvsr, char *sort, L_fps3 selctg, aocl_int64_t *n,
                        real *a, aocl_int64_t *lda, real *b, aocl_int64_t *ldb, aocl_int64_t *sdim,
                        real *alphar, real *alphai, real *beta, real *vsl, aocl_int64_t *ldvsl,
                        real *vsr, aocl_int64_t *ldvsr, real *work, aocl_int64_t *lwork,
                        logical *bwork, aocl_int64_t *info)
{
    ...
    *info = 0;
    ...
    if(*info == 0)
    {
        ...
        if(ilvsl)
        {
            aocl_lapack_sorgqr(n, n, n, &vsl[vsl_offset], ldvsl, &work[1], &work[1], &c_n1, &ierr);
            /* Computing MAX */
            i__1 = lwkopt;
            i__2 = *n * 3 + (integer)work[1]; // , expr subst
            lwkopt = fla_max(i__1, i__2);
        }
        aocl_lapack_sgghd3(jobvsl, jobvsr, n, &c__1, n, &a[a_offset], lda, &b[b_offset], ldb,
                           &vsl[vsl_offset], ldvsl, &vsr[vsr_offset], ldvsr, &work[1], &c_n1,
                           &ierr);
        /* Computing MAX */
        i__1 = lwkopt;
        i__2 = *n * 3 + (integer)work[1]; // , expr subst
        lwkopt = fla_max(i__1, i__2);
        aocl_lapack_shgeqz("S", jobvsl, jobvsr, n, &c__1, n, &a[a_offset], lda, &b[b_offset], ldb,
                           &alphar[1], &alphai[1], &beta[1], &vsl[vsl_offset], ldvsl,
                           &vsr[vsr_offset], ldvsr, &work[1], &c_n1, &ierr);
        /* Computing MAX */
        i__1 = lwkopt;
        ...
        if(wantst)
        {
            aocl_lapack_stgsen(&c__0, &ilvsl, &ilvsr, &bwork[1], n, &a[a_offset], lda, &b[b_offset],
                               ldb, &alphar[1], &alphai[1], &beta[1], &vsl[vsl_offset], ldvsl,
                               &vsr[vsr_offset], ldvsr, sdim, &pvsl, &pvsr, dif, &work[1], &c_n1,
                               idum, &c__1, &ierr);
            /* Computing MAX */
            i__1 = lwkopt;
            i__2 = (*n << 1) + (integer)work[1]; // , expr subst
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("SGGES3", &i__1, (ftnlen)6);
    ...
    iwrk = itau + irows;
    i__1 = *lwork + 1 - iwrk;
    aocl_lapack_sgeqrf(&irows, &icols, &b[ilo + ilo * b_dim1], ldb, &work[itau], &work[iwrk], &i__1,
                       &ierr);
    ...
    if(ilvsl)
    {
        aocl_lapack_slaset("Full", n, n, &c_b37, &c_b38, &vsl[vsl_offset], ldvsl);
        if(irows > 1)
        {
            i__1 = irows - 1;
            i__2 = irows - 1;
    ...
    if(ilvsr)
    {
        aocl_lapack_slaset("Full", n, n, &c_b37, &c_b38, &vsr[vsr_offset], ldvsr);
    }
    /* Reduce to generalized Hessenberg form */
    i__1 = *lwork + 1 - iwrk;
    aocl_lapack_sgghd3(jobvsl, jobvsr, n, &ilo, &ihi, &a[a_offset], lda, &b[b_offset], ldb,
                       &vsl[vsl_offset], ldvsl, &vsr[vsr_offset], ldvsr, &work[iwrk], &i__1, &ierr);
    ...
    *sdim = 0;
    if(wantst)
    {
        ...
        aocl_lapack_stgsen(&c__0, &ilvsl, &ilvsr, &bwork[1], n, &a[a_offset], lda, &b[b_offset],
                           ldb, &alphar[1], &alphai[1], &beta[1], &vsl[vsl_offset], ldvsl,
                           &vsr[vsr_offset], ldvsr, sdim, &pvsl, &pvsr, dif, &work[iwrk], &i__1,
                           idum, &c__1, &ierr);
        if(ierr == 1)
        {
            *info = *n + 3;
    ...
    if(ilascl)
    {
        aocl_lapack_slascl("H", &c__0, &c__0, &anrmto, &anrm, n, n, &a[a_offset], lda, &ierr);
        aocl_lapack_slascl("G", &c__0, &c__0, &anrmto, &anrm, n, &c__1, &alphar[1], n, &ierr);
        aocl_lapack_slascl("G", &c__0, &c__0, &anrmto, &anrm, n, &c__1, &alphai[1], n, &ierr);
    }
    if(ilbscl)
    {
        aocl_lapack_slascl("U", &c__0, &c__0, &bnrmto, &bnrm, n, n, &b[b_offset], ldb, &ierr);
        aocl_lapack_slascl("G", &c__0, &c__0, &bnrmto, &bnrm, n, &c__1, &beta[1], n, &ierr);
    }
    if(wantst)
    {
