...
/* ../netlib/clasyf_rook.f -- translated by f2c (version 20100827). You must link the resulting
...
/* > \verbatim */
/* > */
/* > CLASYF_ROOK computes a partial factorization of a scomplex symmetric */
                             ...
                             aocl_int64_t *ldw, aocl_int64_t *info)
{
    ...
    char buffer[256];
#if FLA_ENABLE_ILP64
    snprintf(buffer, 256, "clasyf_rook inputs: uplo %c, n %lld, nb %lld, lda %lld, ldw %lld", *uplo,
             *n, *nb, *lda, *ldw);
#else
    snprintf(buffer, 256, "clasyf_rook inputs: uplo %c, n %d, nb %d, lda %d, ldw %d", *uplo, *n,
             *nb, *lda, *ldw);
#endif
    ...
    *info = 0;
    ...
    L120:
        ...
        if(jp2 != jj && j >= 1)
        {
            aocl_blas_cswap(&j, &a[jp2 + a_dim1], lda, &a[jj + a_dim1], lda);
        }
        jj = j + 1;
        if(jp1 != jj && kstep == 2)
        {
            aocl_blas_cswap(&j, &a[jp1 + a_dim1], lda, &a[jj + a_dim1], lda);
        }
        if(j >= 1)
        {
            goto L120;
