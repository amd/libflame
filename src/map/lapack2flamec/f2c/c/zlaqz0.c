...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/zlaqz0.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/zlaqz0.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/zlaqz0.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
             ...
             doublecomplex *work, integer *lwork, doublereal *rwork, integer *rec, integer *info)
{
    ...
    *(unsigned char *)&jbcmpz[2] = *(unsigned char *)wantz;
    ...
    for(iiter = 1; iiter <= i__1; ++iiter)
    {
        ...
        d__1 = smlnum;
        d__2 = ulp
               * (z_abs(&a[istop + istop * a_dim1])
                  + z_abs(&a[istop - 1 + (istop - 1) * a_dim1])); // , expr subst
        ...
        d__1 = smlnum;
        d__2 = ulp
               * (z_abs(&a[istart + istart * a_dim1])
                  + z_abs(&a[istart + 1 + (istart + 1) * a_dim1])); // , expr subst
        ...
        for(k = istop; k >= i__2; --k)
        {
            ...
            d__2 = ulp
                   * (z_abs(&a[k + k * a_dim1])
                      + z_abs(&a[k - 1 + (k - 1) * a_dim1])); // , expr subst
        ...
        if(ld % 6 == 0)
        {
            ...
            if((doublereal)maxit * safmin * z_abs(&a[istop + (istop - 1) * a_dim1])
