...
/* ../netlib/dla_syrfsx_extended.f -- translated by f2c (version 20160102). You must link the
...
/* > \brief \b DLA_SYRFSX_EXTENDED improves the computed solution to a system of linear equations
...
/* ===================================================================== */
/* Subroutine */
void dla_syrfsx_extended_(integer *prec_type__, char *uplo, integer *n, integer *nrhs,
                          ...
                          logical *ignore_cwise__, integer *info)
{
    ...
    AOCL_DTL_SNPRINTF("dla_syrfsx_extended inputs: prec_type__ %" FLA_IS ", uplo %c, n %" FLA_IS
    ...
    extern /* Subroutine */
        int
        blas_dsymv_x_(integer *, integer *, doublereal *, doublereal *, integer *, doublereal *,
                      integer *, doublereal *, doublereal *, integer *, integer *);
    integer uplo2;
    extern logical lsame_(char *, char *, integer, integer);
    extern /* Subroutine */
        int
        blas_dsymv2_x_(integer *, integer *, doublereal *, doublereal *, integer *, doublereal *,
    ...
    *info = 0;
    ...
    for(j = 1; j <= i__1; ++j)
    {
        ...
        for(cnt = 1; cnt <= i__2; ++cnt)
        {
            ...
            if(y_prec_state__ == 0)
            {
                dsymv_(uplo, n, &c_b12, &a[a_offset], lda, &y[j * y_dim1 + 1], &c__1, &c_b14,
                       &res[1], &c__1);
            }
            else if(y_prec_state__ == 1)
            {
                blas_dsymv_x_(&uplo2, n, &c_b12, &a[a_offset], lda, &y[j * y_dim1 + 1], &c__1,
                              &c_b14, &res[1], &c__1, prec_type__);
            }
            else
            {
                blas_dsymv2_x_(&uplo2, n, &c_b12, &a[a_offset], lda, &y[j * y_dim1 + 1],
                               &y_tail__[1], &c__1, &c_b14, &res[1], &c__1, prec_type__);
            }
    ...
    L666: /* Set final_* when cnt hits ithresh. */
        ...
        /* op(A) = A, A**T, or A**H depending on TRANS (and type). */
        dcopy_(n, &b[j * b_dim1 + 1], &c__1, &res[1], &c__1);
        dsymv_(uplo, n, &c_b12, &a[a_offset], lda, &y[j * y_dim1 + 1], &c__1, &c_b14, &res[1],
               &c__1);
        i__2 = *n;
        for(i__ = 1; i__ <= i__2; ++i__)
        {
        ...
        dla_syamv_(&uplo2, n, &c_b14, &a[a_offset], lda, &y[j * y_dim1 + 1], &c__1, &c_b14, &ayb[1],
                   &c__1);
