/* ../netlib/v3.9.0/zhetrs_aa_2stage.f -- translated by f2c (version 20160102). You must link the
...
void zhetrs_aa_2stage_(char *uplo, aocl_int_t *n, aocl_int_t *nrhs, dcomplex *a,
                       aocl_int_t *lda, dcomplex *tb, aocl_int_t *ltb, aocl_int_t *ipiv,
                       aocl_int_t *ipiv2, dcomplex *b, aocl_int_t *ldb, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_zhetrs_aa_2stage(uplo, n, nrhs, a, lda, tb, ltb, ipiv, ipiv2, b, ldb, info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_zhetrs_aa_2stage(uplo, &n_64, &nrhs_64, a, &lda_64, tb, &ltb_64, ipiv, ipiv2, b,
                                 &ldb_64, &info_64);
...
void aocl_lapack_zhetrs_aa_2stage(char *uplo, aocl_int64_t *n, aocl_int64_t *nrhs, dcomplex *a,
                                  ...
                                  aocl_int64_t *ldb, aocl_int64_t *info)
{
    ...
    AOCL_DTL_SNPRINTF("zhetrs_aa_2stage inputs: uplo %c, n %" FLA_IS ", nrhs %" FLA_IS
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("ZHETRS_AA_2STAGE", &i__1, (ftnlen)16);
    ...
    if(upper)
    {
        ...
        if(*n > nb)
        {
            /* Pivot, P**T * B -> B */
            i__1 = nb + 1;
            aocl_lapack_zlaswp(nrhs, &b[b_offset], ldb, &i__1, n, &ipiv[1], &c__1);
            /* Compute (U**H \ B) -> B [ (U**H \P**T * B) ] */
            i__1 = *n - nb;
            aocl_blas_ztrsm("L", "U", "C", "U", &i__1, nrhs, &c_b1, &a[(nb + 1) * a_dim1 + 1], lda,
                            &b[nb + 1 + b_dim1], ldb);
        }
        /* Compute T \ B -> B [ T \ (U**H \P**T * B) ] */
        aocl_lapack_zgbtrs("N", n, &nb, &nb, nrhs, &tb[1], &ldtb, &ipiv2[1], &b[b_offset], ldb,
                           info);
        if(*n > nb)
        {
            /* Compute (U \ B) -> B [ U \ (T \ (U**H \P**T * B) ) ] */
            i__1 = *n - nb;
            aocl_blas_ztrsm("L", "U", "N", "U", &i__1, nrhs, &c_b1, &a[(nb + 1) * a_dim1 + 1], lda,
                            &b[nb + 1 + b_dim1], ldb);
            /* Pivot, P * B -> B [ P * (U \ (T \ (U**H \P**T * B) )) ] */
            i__1 = nb + 1;
            aocl_lapack_zlaswp(nrhs, &b[b_offset], ldb, &i__1, n, &ipiv[1], &c_n1);
        }
    }
    else
    {
        ...
        if(*n > nb)
        {
            ...
            i__1 = *n - nb;
            aocl_blas_ztrsm("L", "L", "N", "U", &i__1, nrhs, &c_b1, &a[nb + 1 + a_dim1], lda,
                            &b[nb + 1 + b_dim1], ldb);
        }
        ...
        if(*n > nb)
        {
            ...
            i__1 = *n - nb;
            aocl_blas_ztrsm("L", "L", "C", "U", &i__1, nrhs, &c_b1, &a[nb + 1 + a_dim1], lda,
                            &b[nb + 1 + b_dim1], ldb);
