...
/* ../netlib/v3.9.0/clahef_rk.f -- translated by f2c (version 20160102). You must link the resulting
                           ...
                           aocl_int64_t *ldw, aocl_int64_t *info)
{
    ...
    char buffer[256];
#if FLA_ENABLE_ILP64
    snprintf(buffer, 256, "clahef_rk inputs: uplo %c, n %lld, nb %lld, kb %lld, lda %lld, ldw %lld",
             *uplo, *n, *nb, *kb, *lda, *ldw);
#else
    snprintf(buffer, 256, "clahef_rk inputs: uplo %c, n %d, nb %d, kb %d, lda %d, ldw %d", *uplo,
             *n, *nb, *kb, *lda, *ldw);
#endif
