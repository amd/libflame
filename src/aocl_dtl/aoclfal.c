...
int32 AOCL_FAL_Close(AOCL_FAL_FILE *fpFilePointer) __attribute__((no_instrument_function));

int32 AOCL_FAL_Error(AOCL_FAL_FILE *fpFilePointer) __attribute__((no_instrument_function));
...
int32 AOCL_FAL_Close(AOCL_FAL_FILE *fpFilePointer)
{
    ...
    if(i32RetVal)
    {
        AOCL_DTL_LOG(AOCL_DTL_LEVEL_MAJOR, "Can't close file, Invalid file pointer passed");
        return i32RetVal;
    }
...
int32 AOCL_FAL_Read(void *pvBuffer, int32 i32Size, int32 i32Count, AOCL_FAL_FILE *fpFilePointer)
{
    ...
    if(NULL == pvBuffer)
    {
        AOCL_DTL_LOG(AOCL_DTL_LEVEL_MAJOR, "Can not read the file, Buffer pointer is NULL");
        return i32RetVal;
    }
    ...
    if(i32RetVal != i32Count)
    {
        ...
        {
            AOCL_DTL_LOG(AOCL_DTL_LEVEL_MAJOR, "There is an error condition while file read");
            i32RetVal = AOCL_FAL_READ_ERROR;
        }
