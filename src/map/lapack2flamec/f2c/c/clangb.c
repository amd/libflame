...
/* > \verbatim */
/* > */
/* > CLANGB = ( fla_max(f2c_abs(A(i,j))), NORM = 'M' or 'm' */
/* > ( */
...
real clangb_(char *norm, aocl_int_t *n, aocl_int_t *kl, aocl_int_t *ku, scomplex *ab,
             aocl_int_t *ldab, real *work)
{
#if FLA_ENABLE_ILP64
    return aocl_lapack_clangb(norm, n, kl, ku, ab, ldab, work);
#else
    aocl_int64_t n_64 = *n;
    ...
    return aocl_lapack_clangb(norm, &n_64, &kl_64, &ku_64, ab, &ldab_64, work);
#endif
}

real aocl_lapack_clangb(char *norm, aocl_int64_t *n, aocl_int64_t *kl, aocl_int64_t *ku,
                        scomplex *ab, aocl_int64_t *ldab, real *work)
{
    AOCL_DTL_TRACE_LOG_INIT
    AOCL_DTL_SNPRINTF("clangb inputs: norm %c, n %" FLA_IS ", kl %" FLA_IS ", ku %" FLA_IS
                      ", ldab %" FLA_IS "",
                      *norm, *n, *kl, *ku, *ldab);
    ...
    else if(lsame_(norm, "F", 1, 1) || lsame_(norm, "E", 1, 1))
    {
        ...
        for(j = 1; j <= i__1; ++j)
        {
            ...
            aocl_lapack_classq(&i__4, &ab[k + j * ab_dim1], &c__1, &scale, &sum);
            /* L90: */
        }
