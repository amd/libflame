/* ../netlib/cgesvx.f -- translated by f2c (version 20100827). You must link the resulting object
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/cgesvx.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/cgesvx.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/cgesvx.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
    ...
    *info = 0;
    ...
    if(equil)
    {
        ...
        if(infequ == 0)
        {
            ...
            aocl_lapack_claqge(n, n, &a[a_offset], lda, &r__[1], &c__[1], &rowcnd, &colcnd, &amax,
                               equed);
            rowequ = lsame_(equed, "R", 1, 1) || lsame_(equed, "B", 1, 1);
            colequ = lsame_(equed, "C", 1, 1) || lsame_(equed, "B", 1, 1);
        }
    ...
    if(nofact || equil)
    {
        ...
        aocl_lapack_clacpy("Full", n, n, &a[a_offset], lda, &af[af_offset], ldaf);
        aocl_lapack_cgetrf(n, n, &af[af_offset], ldaf, &ipiv[1], info);
        ...
        if(*info > 0)
        {
            ...
            {
                rpvgrw = aocl_lapack_clange("M", n, info, &a[a_offset], lda, &rwork[1]) / rpvgrw;
            }
            rwork[1] = rpvgrw;
            *rcond = 0.f;
            AOCL_DTL_TRACE_EXIT(AOCL_DTL_LEVEL_TRACE_5);
    ...
    {
        rpvgrw = aocl_lapack_clange("M", n, n, &a[a_offset], lda, &rwork[1]) / rpvgrw;
    }
    /* Compute the reciprocal of the condition number of A. */
    aocl_lapack_cgecon(norm, n, &af[af_offset], ldaf, &anorm, rcond, &work[1], &rwork[1], info);
