...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/dgelsy.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/dgelsy.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/dgelsy.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
void dgelsy_(aocl_int_t *m, aocl_int_t *n, aocl_int_t *nrhs, doublereal *a, aocl_int_t *lda,
                        ...
                        aocl_int64_t *lwork, aocl_int64_t *info)
{
    ...
    *info = 0;
    ...
    if(*info == 0)
    {
        ...
        {
            nb1 = aocl_lapack_ilaenv(&c__1, "DGEQRF", " ", m, n, &c_n1, &c_n1);
            nb2 = aocl_lapack_ilaenv(&c__1, "DGERQF", " ", m, n, &c_n1, &c_n1);
    ...
    else if(anrm == 0.)
    {
        ...
        i__1 = fla_max(*m, *n);
        aocl_lapack_dlaset("F", &i__1, nrhs, &c_b31, &c_b31, &b[b_offset], ldb);
        *rank = 0;
        goto L70;
    }
    ...
    /* A * P = Q * R */
    i__1 = *lwork - mn;
    aocl_lapack_dgeqp3(m, n, &a[a_offset], lda, &jpvt[1], &work[1], &work[mn + 1], &i__1, info);
    wsize = mn + work[mn + 1];
...
L10:
    ...
    {
        i__1 = *lwork - (mn << 1);
        aocl_lapack_dtzrzf(rank, n, &a[a_offset], lda, &work[mn + 1], &work[(mn << 1) + 1], &i__1,
                           info);
    }
    /* workspace: 2*MN. */
    ...
    for(j = 1; j <= i__1; ++j)
    {
        ...
        aocl_blas_dcopy(n, &work[1], &c__1, &b[j * b_dim1 + 1], &c__1);
        /* L60: */
    }
    /* workspace: N. */
    /* Undo scaling */
