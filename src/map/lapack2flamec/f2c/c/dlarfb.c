...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/dlarfb.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/dlarfb.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/dlarfb.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
void dlarfb_(char *side, char *trans, char *direct, char *storev, aocl_int_t *m, aocl_int_t *n,
             aocl_int_t *k, doublereal *v, aocl_int_t *ldv, doublereal *t, aocl_int_t *ldt,
             doublereal *c__, aocl_int_t *ldc, doublereal *work, aocl_int_t *ldwork)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_dlarfb(side, trans, direct, storev, m, n, k, v, ldv, t, ldt, c__, ldc, work,
                       ldwork);
#else
    aocl_int64_t m_64 = *m;
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_dlarfb(side, trans, direct, storev, &m_64, &n_64, &k_64, v, &ldv_64, t, &ldt_64,
                       c__, &ldc_64, work, &ldwork_64);
#endif
}

void aocl_lapack_dlarfb(char *side, char *trans, char *direct, char *storev, aocl_int64_t *m,
                        aocl_int64_t *n, aocl_int64_t *k, doublereal *v, aocl_int64_t *ldv,
                        doublereal *t, aocl_int64_t *ldt, doublereal *c__, aocl_int64_t *ldc,
                        doublereal *work, aocl_int64_t *ldwork)
{
    ...
    if(lsame_(storev, "C", 1, 1))
    {
        if(lsame_(direct, "F", 1, 1))
        {
            ...
            else if(lsame_(side, "R", 1, 1))
            {
                ...
                for(j = 1; j <= i__1; ++j)
                {
                    aocl_blas_dcopy(m, &c__[j * c_dim1 + 1], &c__1, &work[j * work_dim1 + 1],
                                    &c__1);
                    /* L40: */
                }
                /* W := W * V1 */
        ...
        else
        {
            ...
            if(lsame_(side, "L", 1, 1))
            {
                ...
                for(j = 1; j <= i__1; ++j)
                {
                    aocl_blas_dcopy(n, &c__[*m - *k + j + c_dim1], ldc, &work[j * work_dim1 + 1],
                                    &c__1);
                    /* L70: */
                }
                /* W := W * V2 */
