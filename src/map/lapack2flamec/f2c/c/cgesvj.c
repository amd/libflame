...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/cgesvj.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/cgesvj.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/cgesvj.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
                        ...
                        aocl_int64_t *lrwork, aocl_int64_t *info)
{
...
#if FLA_ENABLE_ILP64
    snprintf(buffer, 256,
             "cgesvj inputs: joba %c, jobu %c, jobv %c, m %lld, n %lld, lda %lld, mv %lld, ldv "
             "%lld, lwork %lld, lrwork %lld",
             *joba, *jobu, *jobv, *m, *n, *lda, *mv, *ldv, *lwork, *lrwork);
#else
    snprintf(buffer, 256,
             "cgesvj inputs: joba %c, jobu %c, jobv %c, m %d, n %d, lda %d, mv %d, ldv %d, lwork "
             "%d, lrwork %d",
             *joba, *jobu, *jobv, *m, *n, *lda, *mv, *ldv, *lwork, *lrwork);
#endif
    ...
    for(q = 1; q <= i__1; ++q)
    {
        i__2 = q;
        cwork[i__2].real = 1.f;
        cwork[i__2].imag = 0.f; // , expr subst
        /* L1868: */
    }
    swband = 3;
                            ...
                            if(aaqq > 0.f)
                            {
                                ...
                                if(f2c_abs(aapq1) > tol)
                                {
                                    ...
                                    if(rotok)
                                    {
                                        ...
                                        i__5 = p;
                                        i__6 = q;
                                        q__2.real = -cwork[i__6].real;
                                        q__2.imag = -cwork[i__6].imag; // , expr subst
                                        q__1.real = q__2.real * ompq.real - q__2.imag * ompq.imag;
                                        q__1.imag = q__2.real * ompq.imag + q__2.imag * ompq.real; // , expr subst
                                        cwork[i__5].real = q__1.real;
                                        cwork[i__5].imag = q__1.imag; // , expr subst
                                    }
                                    else
                                    {
