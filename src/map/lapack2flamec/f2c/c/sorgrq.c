...
void sorgrq_(aocl_int_t *m, aocl_int_t *n, aocl_int_t *k, real *a, aocl_int_t *lda, real *tau,
             real *work, aocl_int_t *lwork, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_sorgrq(m, n, k, a, lda, tau, work, lwork, info);
#else
    aocl_int64_t m_64 = *m;
    aocl_int64_t n_64 = *n;
    aocl_int64_t k_64 = *k;
    ...
    aocl_lapack_sorgrq(&m_64, &n_64, &k_64, a, &lda_64, tau, work, &lwork_64, &info_64);

    *info = (aocl_int_t)info_64;
#endif
}

void aocl_lapack_sorgrq(aocl_int64_t *m, aocl_int64_t *n, aocl_int64_t *k, real *a,
                        ...
                        aocl_int64_t *info)
{
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("SORGRQ", &i__1, (ftnlen)6);
    ...
    i__2 = *n - kk;
    i__3 = *k - kk;
    aocl_lapack_sorgr2(&i__1, &i__2, &i__3, &a[a_offset], lda, &tau[1], &work[1], &iinfo);
    if(kk > 0)
    {
        /* Use blocked code */
        i__1 = *k;
        ...
        for(i__ = *k - kk + 1; i__2 < 0 ? i__ >= i__1 : i__ <= i__1; i__ += i__2)
        {
            ...
            if(ii > 1)
            {
                ...
                i__3 = *n - *k + i__ + ib - 1;
                aocl_lapack_slarft("Backward", "Rowwise", &i__3, &ib, &a[ii + a_dim1], lda,
                                   &tau[i__], &work[1], &ldwork);
                /* Apply H**T to A(1:m-k+i-1,1:n-k+i+ib-1) from the right */
                i__3 = ii - 1;
                i__4 = *n - *k + i__ + ib - 1;
                aocl_lapack_slarfb("Right", "Transpose", "Backward", "Rowwise", &i__3, &i__4, &ib,
                                   &a[ii + a_dim1], lda, &work[1], &ldwork, &a[a_offset], lda,
                                   &work[ib + 1], &ldwork);
            }
            /* Apply H**T to columns 1:n-k+i+ib-1 of current block */
            i__3 = *n - *k + i__ + ib - 1;
            aocl_lapack_sorgr2(&ib, &i__3, &ib, &a[ii + a_dim1], lda, &tau[i__], &work[1], &iinfo);
