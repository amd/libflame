...
/* ../netlib/sla_gbrfsx_extended.f -- translated by f2c (version 20100827). You must link the
...
/* > \brief \b SLA_GBRFSX_EXTENDED improves the computed solution to a system of linear equations
    ...
    *(unsigned char *)trans = *(unsigned char *)&ch__1[0];
    ...
    for(j = 1; j <= i__1; ++j)
    {
        ...
        for(cnt = 1; cnt <= i__2; ++cnt)
        {
            ...
            if(y_prec_state__ == 0)
            {
                sgbmv_(trans, &m, n, kl, ku, &c_b6, &ab[ab_offset], ldab, &y[j * y_dim1 + 1], &c__1,
                       &c_b8, &res[1], &c__1);
            }
            else if(y_prec_state__ == 1)
            {
                blas_sgbmv_x_(trans_type__, n, n, kl, ku, &c_b6, &ab[ab_offset], ldab,
                              &y[j * y_dim1 + 1], &c__1, &c_b8, &res[1], &c__1, prec_type__);
            }
            else
            {
                blas_sgbmv2_x_(trans_type__, n, n, kl, ku, &c_b6, &ab[ab_offset], ldab,
    ...
    L666: /* Set final_* when cnt hits ithresh. */
        ...
        /* op(A) = A, A**T, or A**H depending on TRANS (and type). */
        scopy_(n, &b[j * b_dim1 + 1], &c__1, &res[1], &c__1);
        sgbmv_(trans, n, n, kl, ku, &c_b6, &ab[ab_offset], ldab, &y[j * y_dim1 + 1], &c__1, &c_b8,
               &res[1], &c__1);
        i__2 = *n;
        for(i__ = 1; i__ <= i__2; ++i__)
        {
        ...
        sla_gbamv_(trans_type__, n, n, kl, ku, &c_b8, &ab[ab_offset], ldab, &y[j * y_dim1 + 1],
