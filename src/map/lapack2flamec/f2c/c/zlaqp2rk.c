...
/* > \brief \b ZLAQP2RK computes truncated QR factorization with column pivoting of a scomplex matrix
...
void zlaqp2rk_(aocl_int_t *m, aocl_int_t *n, aocl_int_t *nrhs, aocl_int_t *ioffset,
               aocl_int_t *kmax, doublereal *abstol, doublereal *reltol, aocl_int_t *kp1,
               doublereal *maxc2nrm, dcomplex *a, aocl_int_t *lda, aocl_int_t *k,
               doublereal *maxc2nrmk, doublereal *relmaxc2nrmk, aocl_int_t *jpiv,
               dcomplex *tau, doublereal *vn1, doublereal *vn2, dcomplex *work,
               aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_zlaqp2rk(m, n, nrhs, ioffset, kmax, abstol, reltol, kp1, maxc2nrm, a, lda, k,
    ...
    aocl_lapack_zlaqp2rk(&m_64, &n_64, &nrhs_64, &ioffset_64, &kmax_64, abstol, reltol, &kp1_64,
...
void aocl_lapack_zlaqp2rk(aocl_int64_t *m, aocl_int64_t *n, aocl_int64_t *nrhs,
                          aocl_int64_t *ioffset, aocl_int64_t *kmax, doublereal *abstol,
                          doublereal *reltol, aocl_int64_t *kp1, doublereal *maxc2nrm,
                          dcomplex *a, aocl_int64_t *lda, aocl_int64_t *k,
                          doublereal *maxc2nrmk, doublereal *relmaxc2nrmk, aocl_int_t *jpiv,
                          dcomplex *tau, doublereal *vn1, doublereal *vn2, dcomplex *work,
                          aocl_int64_t *info)
{
    ...
    *kmax = fla_min(*kmax, minmnfact);
    ...
    for(kk = 1; kk <= i__1; ++kk)
    {
        ...
        if(kp != kk)
        {
            aocl_blas_zswap(m, &a[kp * a_dim1 + 1], &c__1, &a[kk * a_dim1 + 1], &c__1);
            vn1[kp] = vn1[kk];
            vn2[kp] = vn2[kk];
            itemp = jpiv[kp];
            jpiv[kp] = jpiv[kk];
        ...
        {
            i__2 = *m - i__ + 1;
            aocl_lapack_zlarfg(&i__2, &a[i__ + kk * a_dim1], &a[i__ + 1 + kk * a_dim1], &c__1,
                               &tau[kk]);
        }
        else
        {
        ...
        if(kk < minmnupdt)
        {
            ...
            i__3 = *n + *nrhs - kk;
            d_cnjg(&z__1, &tau[kk]);
            aocl_lapack_zlarf1f("Left", &i__2, &i__3, &a[i__ + kk * a_dim1], &c__1, &z__1,
            ...
            for(j = kk + 1; j <= i__2; ++j)
            {
                if(vn1[j] != 0.)
                {
                    ...
                    if(temp2 <= tol3z)
                    {
                        ...
                        i__3 = *m - i__;
                        vn1[j] = aocl_blas_dznrm2(&i__3, &a[i__ + 1 + j * a_dim1], &c__1);
                        vn2[j] = vn1[j];
                    }
                    else
                    {
