/* ../netlib/csyrfs.f -- translated by f2c (version 20100827). You must link the resulting object
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/csyrfs.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/csyrfs.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/csyrfs.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
void csyrfs_(char *uplo, aocl_int_t *n, aocl_int_t *nrhs, scomplex *a, aocl_int_t *lda, scomplex *af,
...
{
#if FLA_ENABLE_ILP64
    aocl_lapack_csyrfs(uplo, n, nrhs, a, lda, af, ldaf, ipiv, b, ldb, x, ldx, ferr, berr, work,
                       rwork, info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_csyrfs(uplo, &n_64, &nrhs_64, a, &lda_64, af, &ldaf_64, ipiv, b, &ldb_64, x,
...
void aocl_lapack_csyrfs(char *uplo, aocl_int64_t *n, aocl_int64_t *nrhs, scomplex *a,
                        ...
                        real *berr, scomplex *work, real *rwork, aocl_int64_t *info)
{
...
#if FLA_ENABLE_ILP64
    snprintf(buffer, 256,
             "csyrfs inputs: uplo %c, n %lld, nrhs %lld, lda %lld, ldaf %lld, ldb %lld, ldx %lld",
             *uplo, *n, *nrhs, *lda, *ldaf, *ldb, *ldx);
#else
    snprintf(buffer, 256, "csyrfs inputs: uplo %c, n %d, nrhs %d, lda %d, ldaf %d, ldb %d, ldx %d",
             *uplo, *n, *nrhs, *lda, *ldaf, *ldb, *ldx);
#endif
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("CSYRFS", &i__1, (ftnlen)6);
    ...
    L20: /* Loop until stopping criterion is satisfied. */
        ...
        aocl_lapack_csymv(uplo, n, &q__1, &a[a_offset], lda, &x[j * x_dim1 + 1], &c__1, &c_b1,
                          &work[1], &c__1);
        ...
        else
        {
            ...
            for(k = 1; k <= i__2; ++k)
            {
                ...
                xk = (r__1 = x[i__3].real, f2c_abs(r__1))
                     + (r__2 = r_imag(&x[k + j * x_dim1]), f2c_abs(r__2));
                i__3 = k + k * a_dim1;
                rwork[k] += ((r__1 = a[i__3].real, f2c_abs(r__1))
                             + (r__2 = r_imag(&a[k + k * a_dim1]), f2c_abs(r__2)))
                            * xk;
                i__3 = *n;
        ...
        if(berr[j] > eps && berr[j] * 2.f <= lstres && count <= 5)
        {
            ...
            aocl_lapack_csytrs(uplo, n, &c__1, &af[af_offset], ldaf, &ipiv[1], &work[1], n, info);
    ...
    L100:
        ...
        if(kase != 0)
        {
            if(kase == 1)
            {
                ...
                aocl_lapack_csytrs(uplo, n, &c__1, &af[af_offset], ldaf, &ipiv[1], &work[1], n,
                                   info);
                i__2 = *n;
                for(i__ = 1; i__ <= i__2; ++i__)
                {
