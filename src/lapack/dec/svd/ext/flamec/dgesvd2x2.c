...
/* Subroutine */ int dgesvd2x2(char *jobu, char *jobvt, aocl_int64_t *m, aocl_int64_t *n,
                               doublereal *a, aocl_int64_t *lda, doublereal *s, doublereal *u,
                               ...
                               doublereal *work, aocl_int64_t *lwork, aocl_int64_t *info)
{
    ...
    *info = 0;
...
#endif
    extern void aocl_lapack_xerbla(const char *srname, const aocl_int64_t *info, ftnlen srname_len);
    ...
    {
        if(*lwork == -1)
        { // lwork -1 means workspace assignment though not required
            if(*jobvt == 'O' || *jobu == 'O')
            {
                *lwork = 10;
    ...
    // S calculation
    s[0] = (sqrt(pow(a[0] - a[*lda + 1], 2) + pow(a[1] + a[*lda], 2))
            + sqrt(pow(a[0] + a[*lda + 1], 2) + pow(a[1] - a[*lda], 2)))
           / 2;
    s[1] = fabs(s[0] - sqrt(pow(a[0] - a[*lda + 1], 2) + pow(a[1] + a[*lda], 2)));

    // V calculation
    tempvt[2] = (s[0] > s[1]) ? sin((atan2(2 * (a[0] * a[1] + a[*lda] * a[*lda + 1]),
                                           a[0] * a[0] - a[1] * a[1] + a[*lda] * a[*lda]
                                               - a[*lda + 1] * a[*lda + 1]))
                                    / 2)
                              : 0; // atan2 =0 will never occur as in that case s[0]=s[1]
    ...
    // Output A matrix contains sigma in its diagonal and rest values as 0 ( exception when jobu or
    // jobv is O then A is overwritten)
    ...
    {
        a[0] = tempvt[0], a[1] = tempvt[1], a[*lda] = tempvt[*ldvt],
        a[*lda + 1] = tempvt[*ldvt + 1];
    }
    return 0;
}
