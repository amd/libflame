    ...
    *(unsigned char *)trans = *(unsigned char *)&ch__1[0];
    ...
    for(j = 1; j <= i__1; ++j)
    {
        ...
        for(cnt = 1; cnt <= i__2; ++cnt)
        {
            ...
            if(y_prec_state__ == 0)
            {
                cgemv_(trans, n, n, &c_b6, &a[a_offset], lda, &y[j * y_dim1 + 1], &c__1, &c_b8,
                       &res[1], &c__1);
            }
            else if(y_prec_state__ == 1)
            {
                blas_cgemv_x_(trans_type__, n, n, &c_b6, &a[a_offset], lda, &y[j * y_dim1 + 1],
                              &c__1, &c_b8, &res[1], &c__1, prec_type__);
            }
            else
            {
                blas_cgemv2_x_(trans_type__, n, n, &c_b6, &a[a_offset], lda, &y[j * y_dim1 + 1],
    ...
    L666: /* Set final_* when cnt hits ithresh */
        ...
        /* op(A) = A, A**T, or A**H depending on TRANS (and type). */
        ccopy_(n, &b[j * b_dim1 + 1], &c__1, &res[1], &c__1);
        cgemv_(trans, n, n, &c_b6, &a[a_offset], lda, &y[j * y_dim1 + 1], &c__1, &c_b8, &res[1],
               &c__1);
        i__2 = *n;
        for(i__ = 1; i__ <= i__2; ++i__)
        {
