...
void dsytrs_aa_2stage_(char *uplo, aocl_int_t *n, aocl_int_t *nrhs, doublereal *a, aocl_int_t *lda,
                       doublereal *tb, aocl_int_t *ltb, aocl_int_t *ipiv, aocl_int_t *ipiv2,
                       doublereal *b, aocl_int_t *ldb, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_dsytrs_aa_2stage(uplo, n, nrhs, a, lda, tb, ltb, ipiv, ipiv2, b, ldb, info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_dsytrs_aa_2stage(uplo, &n_64, &nrhs_64, a, &lda_64, tb, &ltb_64, ipiv, ipiv2, b,
                                 &ldb_64, &info_64);
...
void aocl_lapack_dsytrs_aa_2stage(char *uplo, aocl_int64_t *n, aocl_int64_t *nrhs, doublereal *a,
                                  ...
                                  aocl_int64_t *ldb, aocl_int64_t *info)
{
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("DSYTRS_AA_2STAGE", &i__1, (ftnlen)16);
    ...
    if(upper)
    {
        ...
        if(*n > nb)
        {
            /* Pivot, P**T * B -> B */
            i__1 = nb + 1;
            aocl_lapack_dlaswp(nrhs, &b[b_offset], ldb, &i__1, n, &ipiv[1], &c__1);
            /* Compute (U**T \ B) -> B [ (U**T \P**T * B) ] */
            i__1 = *n - nb;
            aocl_blas_dtrsm("L", "U", "T", "U", &i__1, nrhs, &c_b10, &a[(nb + 1) * a_dim1 + 1], lda,
                            &b[nb + 1 + b_dim1], ldb);
        }
        /* Compute T \ B -> B [ T \ (U**T \P**T * B) ] */
        aocl_lapack_dgbtrs("N", n, &nb, &nb, nrhs, &tb[1], &ldtb, &ipiv2[1], &b[b_offset], ldb,
                           info);
        if(*n > nb)
        {
            /* Compute (U \ B) -> B [ U \ (T \ (U**T \P**T * B) ) ] */
            i__1 = *n - nb;
            aocl_blas_dtrsm("L", "U", "N", "U", &i__1, nrhs, &c_b10, &a[(nb + 1) * a_dim1 + 1], lda,
                            &b[nb + 1 + b_dim1], ldb);
            /* Pivot, P * B -> B [ P * (U \ (T \ (U**T \P**T * B) )) ] */
            i__1 = nb + 1;
            aocl_lapack_dlaswp(nrhs, &b[b_offset], ldb, &i__1, n, &ipiv[1], &c_n1);
        }
    }
    else
    {
        ...
        if(*n > nb)
        {
            ...
            i__1 = *n - nb;
            aocl_blas_dtrsm("L", "L", "N", "U", &i__1, nrhs, &c_b10, &a[nb + 1 + a_dim1], lda,
                            &b[nb + 1 + b_dim1], ldb);
        }
        ...
        if(*n > nb)
        {
            ...
            i__1 = *n - nb;
            aocl_blas_dtrsm("L", "L", "T", "U", &i__1, nrhs, &c_b10, &a[nb + 1 + a_dim1], lda,
                            &b[nb + 1 + b_dim1], ldb);
