...
/* ./dorgbr.f -- translated by f2c (version 20190311). You must link the resulting object file with
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/dorgbr.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/dorgbr.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/dorgbr.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
    ...
    *info = 0;
    ...
    if(*info == 0)
    {
        ...
        if(wantq)
        {
            ...
            else
            {
                if(*m > 1)
                {
                    ...
                    i__2 = *m - 1;
                    i__3 = *m - 1;
                    lapack_dorgqr(&i__1, &i__2, &i__3, &a[a_offset], lda, &tau[1], &work[1], &c_n1,
                                  &iinfo);
                }
            }
        }
        else
        {
            if(*k < *n)
            {
                dorglq_fla(m, n, k, &a[a_offset], lda, &tau[1], &work[1], &c_n1, &iinfo);
            }
            else
            {
                if(*n > 1)
                {
                    ...
                    i__2 = *n - 1;
                    i__3 = *n - 1;
                    dorglq_fla(&i__1, &i__2, &i__3, &a[a_offset], lda, &tau[1], &work[1], &c_n1,
                                  &iinfo);
                }
            }
        }
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("DORGBR", &i__1, (ftnlen)6);
    ...
    if(wantq)
    {
        ...
        else
        {
            ...
            if(*m > 1)
            {
                ...
                i__2 = *m - 1;
                i__3 = *m - 1;
                lapack_dorgqr(&i__1, &i__2, &i__3, &a[(a_dim1 << 1) + 2], lda, &tau[1], &work[1],
                              lwork, &iinfo);
            }
        }
    }
    else
    {
        ...
        {
            /* If k < n, assume k <= m <= n */
            dorglq_fla(m, n, k, &a[a_offset], lda, &tau[1], &work[1], lwork, &iinfo);
        }
        else
        {
            /* If k >= n, assume m = n */
            ...
            if(*n > 1)
            {
                ...
                i__2 = *n - 1;
                i__3 = *n - 1;
                dorglq_fla(&i__1, &i__2, &i__3, &a[(a_dim1 << 1) + 2], lda, &tau[1], &work[1],
                              lwork, &iinfo);
            }
        }
    }
