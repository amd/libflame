/* ../netlib/zsytrs_rook.f -- translated by f2c (version 20100827). You must link the resulting
...
/* > a scomplex symmetric matrix A using the factorization A = U*D*U**T or */
...
void zsytrs_rook_(char *uplo, aocl_int_t *n, aocl_int_t *nrhs, dcomplex *a, aocl_int_t *lda,
                  aocl_int_t *ipiv, dcomplex *b, aocl_int_t *ldb, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_zsytrs_rook(uplo, n, nrhs, a, lda, ipiv, b, ldb, info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_zsytrs_rook(uplo, &n_64, &nrhs_64, a, &lda_64, ipiv, b, &ldb_64, &info_64);

    *info = (aocl_int_t)info_64;
#endif
}

void aocl_lapack_zsytrs_rook(char *uplo, aocl_int64_t *n, aocl_int64_t *nrhs, dcomplex *a,
                             aocl_int64_t *lda, aocl_int_t *ipiv, dcomplex *b,
                             aocl_int64_t *ldb, aocl_int64_t *info)
{
    AOCL_DTL_TRACE_LOG_INIT
    AOCL_DTL_SNPRINTF("zsytrs_rook inputs: uplo %c, n %" FLA_IS ", nrhs %" FLA_IS ", lda %" FLA_IS
                      ", ldb %" FLA_IS "",
                      *uplo, *n, *nrhs, *lda, *ldb);
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("ZSYTRS_ROOK", &i__1, (ftnlen)11);
    ...
    L10: /* If K < 1, exit from loop. */
        ...
        else
        {
            ...
            if(k > 2)
            {
                ...
                z__1.real = -1.;
                z__1.imag = -0.; // , expr subst
                aocl_blas_zgeru(&i__1, nrhs, &z__1, &a[(k - 1) * a_dim1 + 1], &c__1,
