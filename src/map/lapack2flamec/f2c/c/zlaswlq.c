...
void zlaswlq_(aocl_int_t *m, aocl_int_t *n, aocl_int_t *mb, aocl_int_t *nb, dcomplex *a,
              aocl_int_t *lda, dcomplex *t, aocl_int_t *ldt, dcomplex *work,
              aocl_int_t *lwork, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_zlaswlq(m, n, mb, nb, a, lda, t, ldt, work, lwork, info);
#else
    aocl_int64_t m_64 = *m;
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_zlaswlq(&m_64, &n_64, &mb_64, &nb_64, a, &lda_64, t, &ldt_64, work, &lwork_64,
                        &info_64);
...
void aocl_lapack_zlaswlq(aocl_int64_t *m, aocl_int64_t *n, aocl_int64_t *mb, aocl_int64_t *nb,
                         ...
                         dcomplex *work, aocl_int64_t *lwork, aocl_int64_t *info)
{
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("ZLASWLQ", &i__1, (ftnlen)7);
    ...
    if(*m >= *n || *nb <= *m || *nb >= *n)
    {
        aocl_lapack_zgelqt(m, n, mb, &a[a_offset], lda, &t[t_offset], ldt, &work[1], info);
        AOCL_DTL_TRACE_LOG_EXIT
        return;
    }
    kk = (*n - *m) % (*nb - *m);
    ...
    aocl_lapack_zgelqt(m, nb, mb, &a[a_dim1 + 1], lda, &t[t_offset], ldt, &work[1], info);
    ctr = 1;
    i__1 = ii - *nb + *m;
    i__2 = *nb - *m;
    for(i__ = *nb + 1; i__2 < 0 ? i__ >= i__1 : i__ <= i__1; i__ += i__2)
    {
        ...
        i__3 = *nb - *m;
        aocl_lapack_ztplqt(m, &i__3, &c__0, mb, &a[a_dim1 + 1], lda, &a[i__ * a_dim1 + 1], lda,
    ...
    if(ii <= *n)
    {
        aocl_lapack_ztplqt(m, &kk, &c__0, mb, &a[a_dim1 + 1], lda, &a[ii * a_dim1 + 1], lda,
                           &t[(ctr * *m + 1) * t_dim1 + 1], ldt, &work[1], info);
    }
    i__2 = *m * *mb;
