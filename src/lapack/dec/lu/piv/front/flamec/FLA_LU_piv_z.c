 ...
 */
int FLA_LU_piv_small_z_var0( fla_dim_t *m, fla_dim_t *n,
                                   dcomplex *a, fla_dim_t *lda,
                                   aocl_int_t *ipiv,
                                   fla_dim_t *info)
{
    fla_dim_t mi, ni;
    fla_dim_t i, j, i_1, i_2, i_3;
...
{
    fla_dim_t a_dim1, i__1, i__2, i__, n1, n2, block_size;
    fla_dim_t iinfo;
    ...
    *info = 0;
    ...
    else
    {
        ...
        /*                       [ A22 ] */
        aocl_lapack_zlaswp(&n2, &a[(n1 ) * a_dim1 ], lda, &c__1, &n1, ipiv, & c__1);

        /* Solve A12 */
        aocl_blas_ztrsm("L", "L", "N", "U", &n1, &n2, &c_b1, a, lda, &a[(n1 ) * a_dim1], lda);

        /* Update A22 */
        i__1 = *m - n1;
        aocl_blas_zgemm("N", "N", &i__1, &n2, &n1, &z__1, &a[n1], lda, &a[ (n1) * a_dim1 ], lda, &c_b1, &a[n1 +  (n1) * a_dim1], lda);
        ...
        i__1 = n1 + 1;
        i__2 = fla_min(*m,*n);
        aocl_lapack_zlaswp(&n1, a, lda, &i__1, &i__2, ipiv, &c__1);
    }
...
int FLA_LU_piv_z_var1_parallel( fla_dim_t *m, fla_dim_t *n, dcomplex *a, fla_dim_t *lda, aocl_int_t *ipiv, fla_dim_t *info)
{
    ...
    fla_dim_t a_dim1, a_offset, i__1, i__2, i__3, i__4, i__5, i__6, i__7, i__8;
    fla_dim_t i__, j, s, iinfo;
    fla_dim_t jb, jb_prev, jb_offset, nb;
    fla_dim_t c__1 = 1;
    ...
    *info = 0;
    ...
    #pragma omp parallel num_threads(n_threads) private(i__3, i__4, i__5, i__6, i__7, i__8, j, s, jb, jb_prev, jb_offset, threads_id)
    {
        ...
        for (j = 1; i__2 < 0 ? j >= i__1 : j <= i__1; j += i__2)
        {
            ...
            if(threads_id == 0)
            {
                ...
                aocl_lapack_zlaswp(&i__3, (dcomplex*) &a_ref(1, j + jb_prev), lda, &j, &i__4, &ipiv[1], &c__1);

                // compute U10
                i__3 = *n - j - jb_prev + 1;
                i__3 = fla_min(i__3, jb_prev);
                aocl_blas_ztrsm("Left", "Lower", "No transpose", "Unit", &jb_prev, &i__3, &c_b1, (dcomplex*) &a_ref(j, j), lda, (dcomplex*) &a_ref(j, j + jb_prev), lda);
                ...
                if (j + jb_prev <= *m)
                {
                    ...
                    aocl_blas_zgemm("No transpose", "No transpose", &i__3, &i__4, &jb_prev, &z__1,(dcomplex*)  &a_ref(j + jb_prev, j), lda,(dcomplex*) &a_ref(j, j + jb_prev), lda, &c_b1,(dcomplex*) &a_ref(j + jb_prev, j + jb_prev), lda);
                }
                ...
                if(s <= i__1)
                {
                    ...
                    {
                        ipiv[i__] = (aocl_int_t)(s - 1 + ipiv[i__]);
                    }
                }
            }
            else
            {
                ...
                if (j + jb_prev <= *n)
                {
                    ...
                    aocl_lapack_zlaswp(&i__5,(dcomplex*) &a_ref(1, j + jb_offset + i__6), lda, &j, &i__4, &ipiv[1], &c__1);

                    // compute U10
                    i__3 = *n - j - jb_prev + 1 - jb_prev;
                    FLA_Thread_get_subrange(threads_id - 1, n_threads - 1, i__3, &i__5, &i__6);
                    aocl_blas_ztrsm("Left", "Lower", "No transpose", "Unit", &jb_prev, &i__5, &c_b1,(dcomplex*) &a_ref(j, j), lda,(dcomplex*) &a_ref(j, j + jb_offset + i__6), lda);
                    ...
                    if (j + jb_prev <= *m)
                    {
                        ...
                        aocl_blas_zgemm("No transpose", "No transpose", &i__3, &i__7, &jb_prev, &z__1,(dcomplex*) &a_ref(j + jb_prev, j), lda,(dcomplex*) &a_ref(j, j + jb_offset + i__8), lda, &c_b1,(dcomplex*) &a_ref(j + jb_prev, j + jb_offset + i__8), lda);
                    }
                }
            }
    ...
    #pragma omp parallel num_threads(n_threads) private(j, i__3, i__4, i__5, i__6, jb, threads_id)
    {
        ...
        for (j = 1; j <= i__1; j += i__2)
        {
            ...
            aocl_lapack_zlaswp(&i__5,(dcomplex*) &a[a_offset + (i__6 * a_dim1)], lda, &j, &i__4, &ipiv[1], &c__1);
            #pragma omp barrier
        }
    }
...
void parallel_gemm_kernel(obj_t* alpha, obj_t* a, obj_t* b, obj_t* beta, obj_t* c, cntx_t* cntx, rntm_t* rntm, thrcomm_t* gl_comm, fla_dim_t bli_n_threads, array_t* array)
{
...
int FLA_LU_piv_z_var2_parallel( fla_dim_t *m, fla_dim_t *n, dcomplex *a, fla_dim_t *lda, aocl_int_t *ipiv, fla_dim_t *info)
{
    /* System generated locals */
    ...
    #pragma omp parallel num_threads(n_threads) private(i__3, i__4, i__5, i__6, j, threads_id)
    {
        ...
        for (j = 1; i__2 < 0 ? j >= i__1 : j <= i__1; j += i__2)
        {
            ...
            if (j + jb <= *n)
            {
                ...
                i__4 = j + jb - 1;
                FLA_Thread_get_subrange(threads_id, n_threads, i__3, &i__5, &i__6);
                aocl_lapack_zlaswp(&i__5, &a_ref(1, j + jb + i__6), lda, &j, &i__4, &ipiv[1], &c__1);

                // compute U10
                i__3 = *n - j - jb + 1;
                FLA_Thread_get_subrange(threads_id, n_threads, i__3, &i__5, &i__6);
                aocl_blas_ztrsm("Left", "Lower", "No transpose", "Unit", &jb, &i__5, &c_b1, &a_ref(j, j), lda, &a_ref(j, j + jb + i__6), lda);
