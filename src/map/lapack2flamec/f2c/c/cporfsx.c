...
/* ../netlib/cporfsx.f -- translated by f2c (version 20160102). You must link the resulting object
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/cporfsx
 * .f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/cporfsx
 * .f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/cporfsx
 * .f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
              ...
              integer *info)
{
...
#if FLA_ENABLE_ILP64
    snprintf(buffer, 256,
             "cporfsx inputs: uplo %c, equed %c, n %lld, nrhs %lld, lda %lld, ldaf %lld, ldb %lld, "
             "ldx %lld, n_err_bnds__ %lld, nparams %lld",
             *uplo, *equed, *n, *nrhs, *lda, *ldaf, *ldb, *ldx, *n_err_bnds__, *nparams);
#else
    snprintf(buffer, 256,
             "cporfsx inputs: uplo %c, equed %c, n %d, nrhs %d, lda %d, ldaf %d, ldb %d, ldx %d, "
    ...
    *(unsigned char *)norm = 'I';
    ...
    if(ref_type__ != 0)
    {
        ...
        cla_porfsx_extended_(&prec_type__, uplo, n, nrhs, &a[a_offset], lda, &af[af_offset], ldaf,
    ...
    if(*n_err_bnds__ >= 1 && n_norms__ >= 1)
    {
        ...
        if(rcequ)
        {
            rcond_tmp__ = cla_porcond_c_(uplo, n, &a[a_offset], lda, &af[af_offset], ldaf, &s[1],
                                         &c_true, info, &work[1], &rwork[1]);
        }
        else
        {
            rcond_tmp__ = cla_porcond_c_(uplo, n, &a[a_offset], lda, &af[af_offset], ldaf, &s[1],
                                         &c_false, info, &work[1], &rwork[1]);
        }
        i__1 = *nrhs;
        for(j = 1; j <= i__1; ++j)
        {
    ...
    if(*n_err_bnds__ >= 1 && n_norms__ >= 2)
    {
        ...
        for(j = 1; j <= i__1; ++j)
        {
            ...
            {
                rcond_tmp__ = cla_porcond_x_(uplo, n, &a[a_offset], lda, &af[af_offset], ldaf,
