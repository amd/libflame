...
/* > \brief <b> ZGGEVX computes the eigenvalues and, optionally, the left and/or right eigenvectors
 * for GE mat rices</b> */
/* =========== DOCUMENTATION =========== */
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/zggevx.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/zggevx.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/zggevx.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
/* > \verbatim */
/* > */
/* > ZGGEVX computes for a pair of N-by-N scomplex nonsymmetric matrices */
...
/* > or both, then A contains the first part of the scomplex Schur */
...
/* > or both, then B contains the second part of the scomplex */
...
void zggevx_(char *balanc, char *jobvl, char *jobvr, char *sense, aocl_int_t *n, dcomplex *a,
             aocl_int_t *lda, dcomplex *b, aocl_int_t *ldb, dcomplex *alpha,
             dcomplex *beta, dcomplex *vl, aocl_int_t *ldvl, dcomplex *vr,
             aocl_int_t *ldvr, aocl_int_t *ilo, aocl_int_t *ihi, doublereal *lscale,
             doublereal *rscale, doublereal *abnrm, doublereal *bbnrm, doublereal *rconde,
             doublereal *rcondv, dcomplex *work, aocl_int_t *lwork, doublereal *rwork,
             aocl_int_t *iwork, logical *bwork, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_zggevx(balanc, jobvl, jobvr, sense, n, a, lda, b, ldb, alpha, beta, vl, ldvl, vr,
    ...
    aocl_lapack_zggevx(balanc, jobvl, jobvr, sense, &n_64, a, &lda_64, b, &ldb_64, alpha, beta, vl,
...
void aocl_lapack_zggevx(char *balanc, char *jobvl, char *jobvr, char *sense, aocl_int64_t *n,
                        dcomplex *a, aocl_int64_t *lda, dcomplex *b, aocl_int64_t *ldb,
                        dcomplex *alpha, dcomplex *beta, dcomplex *vl,
                        aocl_int64_t *ldvl, dcomplex *vr, aocl_int64_t *ldvr,
                        aocl_int64_t *ilo, aocl_int64_t *ihi, doublereal *lscale,
                        doublereal *rscale, doublereal *abnrm, doublereal *bbnrm,
                        doublereal *rconde, doublereal *rcondv, dcomplex *work,
                        aocl_int64_t *lwork, doublereal *rwork, aocl_int_t *iwork, logical *bwork,
                        aocl_int64_t *info)
{
    AOCL_DTL_TRACE_LOG_INIT
    AOCL_DTL_SNPRINTF("zggevx inputs: balanc %c, jobvl %c, jobvr %c, sense %c, n %" FLA_IS
    ...
    *info = 0;
    ...
    if(*info == 0)
    {
        ...
        else
        {
            ...
            i__2 = *n
                   + *n
                         * aocl_lapack_ilaenv(&c__1, "ZGEQRF", " ", n, &c__1, n,
                                              &c__0); // , expr subst
            ...
            i__2 = *n
                   + *n
                         * aocl_lapack_ilaenv(&c__1, "ZUNMQR", " ", n, &c__1, n,
                                              &c__0); // , expr subst
            maxwrk = fla_max(i__1, i__2);
            if(ilvl)
            {
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("ZGGEVX", &i__1, (ftnlen)6);
    ...
    if(ilascl)
    {
        aocl_lapack_zlascl("G", &c__0, &c__0, &anrm, &anrmto, n, n, &a[a_offset], lda, &ierr);
    }
    ...
    if(ilbscl)
    {
        aocl_lapack_zlascl("G", &c__0, &c__0, &bnrm, &bnrmto, n, n, &b[b_offset], ldb, &ierr);
    }
    /* Permute and/or balance the matrix pair (A,B) */
    /* (Real Workspace: need 6*N if BALANC = 'S' or 'B', 1 otherwise) */
    aocl_lapack_zggbal(balanc, n, &a[a_offset], lda, &b[b_offset], ldb, ilo, ihi, &lscale[1],
                       &rscale[1], &rwork[1], &ierr);
    /* Compute ABNRM and BBNRM */
    *abnrm = aocl_lapack_zlange("1", n, n, &a[a_offset], lda, &rwork[1]);
    if(ilascl)
    {
        rwork[1] = *abnrm;
        aocl_lapack_dlascl("G", &c__0, &c__0, &anrmto, &anrm, &c__1, &c__1, &rwork[1], &c__1,
                           &ierr);
        *abnrm = rwork[1];
    }
    *bbnrm = aocl_lapack_zlange("1", n, n, &b[b_offset], ldb, &rwork[1]);
    if(ilbscl)
    {
        rwork[1] = *bbnrm;
        aocl_lapack_dlascl("G", &c__0, &c__0, &bnrmto, &bnrm, &c__1, &c__1, &rwork[1], &c__1,
                           &ierr);
        *bbnrm = rwork[1];
    }
    ...
    iwrk = itau + irows;
    i__1 = *lwork + 1 - iwrk;
    aocl_lapack_zgeqrf(&irows, &icols, &b[*ilo + *ilo * b_dim1], ldb, &work[itau], &work[iwrk],
                       &i__1, &ierr);
    ...
    if(ilv || !wantsn)
    {
        ...
        aocl_lapack_zgghrd(jobvl, jobvr, n, ilo, ihi, &a[a_offset], lda, &b[b_offset], ldb,
                           &vl[vl_offset], ldvl, &vr[vr_offset], ldvr, &ierr);
    }
    else
    {
        aocl_lapack_zgghrd("N", "N", &irows, &c__1, &irows, &a[*ilo + *ilo * a_dim1], lda,
                           &b[*ilo + *ilo * b_dim1], ldb, &vl[vl_offset], ldvl, &vr[vr_offset],
                           ldvr, &ierr);
    }
    ...
    if(ilv || !wantsn)
    {
        if(ilv)
        {
            ...
            aocl_lapack_ztgevc(chtemp, "B", ldumma, n, &a[a_offset], lda, &b[b_offset], ldb,
                               &vl[vl_offset], ldvl, &vr[vr_offset], ldvr, n, &in, &work[iwrk],
                               &rwork[1], &ierr);
            if(ierr != 0)
            {
                *info = *n + 2;
        ...
        if(!wantsn)
        {
            ...
            for(i__ = 1; i__ <= i__1; ++i__)
            {
                ...
                if(wantse || wantsb)
                {
                    aocl_lapack_ztgevc("B", "S", &bwork[1], n, &a[a_offset], lda, &b[b_offset], ldb,
                                       &work[1], n, &work[iwrk], n, &c__1, &m, &work[iwrk1],
                                       &rwork[1], &ierr);
                    if(ierr != 0)
                    {
                        *info = *n + 2;
                ...
                i__2 = *lwork - iwrk1 + 1;
                aocl_lapack_ztgsna(sense, "S", &bwork[1], n, &a[a_offset], lda, &b[b_offset], ldb,
                                   &work[1], n, &work[iwrk], n, &rconde[i__], &rcondv[i__], &c__1,
                                   &m, &work[iwrk1], &i__2, &iwork[1], &ierr);
                /* L20: */
            }
        }
    }
    ...
    if(ilvl)
    {
        aocl_lapack_zggbak(balanc, "L", n, ilo, ihi, &lscale[1], &rscale[1], n, &vl[vl_offset],
                           ldvl, &ierr);
        i__1 = *n;
        for(jc = 1; jc <= i__1; ++jc)
        {
            ...
            for(jr = 1; jr <= i__2; ++jr)
            {
                ...
                i__3 = jr + jc * vl_dim1;
                d__3 = temp;
                d__4 = (d__1 = vl[i__3].real, f2c_abs(d__1))
                       + (d__2 = d_imag(&vl[jr + jc * vl_dim1]), f2c_abs(d__2)); // , expr subst
    ...
    if(ilvr)
    {
        aocl_lapack_zggbak(balanc, "R", n, ilo, ihi, &lscale[1], &rscale[1], n, &vr[vr_offset],
                           ldvr, &ierr);
        i__1 = *n;
        for(jc = 1; jc <= i__1; ++jc)
        {
            ...
            for(jr = 1; jr <= i__2; ++jr)
            {
                ...
                i__3 = jr + jc * vr_dim1;
                d__3 = temp;
                d__4 = (d__1 = vr[i__3].real, f2c_abs(d__1))
                       + (d__2 = d_imag(&vr[jr + jc * vr_dim1]), f2c_abs(d__2)); // , expr subst
                temp = fla_max(d__3, d__4);
                /* L60: */
            }
            if(temp < smlnum)
...
L90:
    if(ilascl)
    {
        aocl_lapack_zlascl("G", &c__0, &c__0, &anrmto, &anrm, n, &c__1, &alpha[1], n, &ierr);
    }
    if(ilbscl)
    {
