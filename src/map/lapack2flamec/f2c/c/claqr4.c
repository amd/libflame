...
/* > \brief \b CLAQR4 computes the eigenvalues of a Hessenberg matrix, and optionally the matrices
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/claqr4.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/claqr4.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/claqr4.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
{
#if FLA_ENABLE_ILP64
    aocl_lapack_claqr4(wantt, wantz, n, ilo, ihi, h__, ldh, w, iloz, ihiz, z__, ldz, work, lwork,
                       info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_claqr4(wantt, wantz, &n_64, &ilo_64, &ihi_64, h__, &ldh_64, w, &iloz_64, &ihiz_64,
...
void aocl_lapack_claqr4(logical *wantt, logical *wantz, aocl_int64_t *n, aocl_int64_t *ilo,
                        ...
                        scomplex *work, aocl_int64_t *lwork, aocl_int64_t *info)
{
...
#if FLA_ENABLE_ILP64
    snprintf(buffer, 256,
             "claqr4 inputs: n %lld, ilo %lld, ihi %lld, ldh %lld, iloz %lld, ihiz %lld, ldz %lld, "
             "lwork %lld",
             *n, *ilo, *ihi, *ldh, *iloz, *ihiz, *ldz, *lwork);
#else
    snprintf(buffer, 256,
             "claqr4 inputs: n %d, ilo %d, ihi %d, ldh %d, iloz %d, ihiz %d, ldz %d, lwork %d", *n,
    ...
    *info = 0;
    ...
    if(*n <= 15)
    {
        ...
        if(*lwork != -1)
        {
            aocl_lapack_clahqr(wantt, wantz, n, ilo, ihi, &h__[h_offset], ldh, &w[1], iloz, ihiz,
    ...
    else
    {
        ...
        *info = 0;
        ...
        i__1 = nwr + 1;
        aocl_lapack_claqr2(wantt, wantz, n, ilo, ihi, &i__1, &h__[h_offset], ldh, iloz, ihiz,
        ...
        kacc22 = aocl_lapack_ilaenv(&c__16, "CLAQR4", jbcmpz, n, ilo, ihi, lwork);
        ...
        L20:
            ...
            nve = *n - nw - kwv + 1;
            /* ==== Aggressive early deflation ==== */
            aocl_lapack_claqr2(wantt, wantz, n, &ktop, &kbot, &nw, &h__[h_offset], ldh, iloz, ihiz,
            ...
            if(ld == 0 || ld * 100 <= nw * nibble && kbot - ktop + 1 > fla_min(nmin, nwmax))
            {
                ...
                else
                {
                    ...
                    if(kbot - ks + 1 <= ns / 2)
                    {
                        ks = kbot - ns + 1;
                        kt = *n - ns + 1;
                        aocl_lapack_clacpy("A", &ns, &ns, &h__[ks + ks * h_dim1], ldh,
                                           &h__[kt + h_dim1], ldh);
                        aocl_lapack_clahqr(&c_false, &c_false, &ns, &c__1, &ns, &h__[kt + h_dim1],
                ...
                nve = *n - kdu - kwv + 1;
                /* ==== Small-bulge multi-shift QR sweep ==== */
                aocl_lapack_claqr5(wantt, wantz, &kacc22, n, &ktop, &kbot, &ns, &w[ks],
