/* ./slaqp3rk.f -- translated by f2c (version 20190311). You must link the resulting object file
...
/* > \brief \b SLAQP3RK computes a step of truncated QR factorization with column pivoting of a real
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/slaqp3r
 * k.f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/slaqp3r
 * k.f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/slaqp3r
 * k.f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
/* > \param[out] TAU */
/* > \verbatim */
/* > TAU is REAL array, dimension (fla_min(M-IOFFSET,N)) */
...
void slaqp3rk_(aocl_int_t *m, aocl_int_t *n, aocl_int_t *nrhs, aocl_int_t *ioffset, aocl_int_t *nb,
               real *abstol, real *reltol, aocl_int_t *kp1, real *maxc2nrm, real *a,
               aocl_int_t *lda, logical *done, aocl_int_t *kb, real *maxc2nrmk, real *relmaxc2nrmk,
               aocl_int_t *jpiv, real *tau, real *vn1, real *vn2, real *auxv, real *f,
               aocl_int_t *ldf, aocl_int_t *iwork, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_slaqp3rk(m, n, nrhs, ioffset, nb, abstol, reltol, kp1, maxc2nrm, a, lda, done, kb,
    ...
    aocl_lapack_slaqp3rk(&m_64, &n_64, &nrhs_64, &ioffset_64, &nb_64, abstol, reltol, &kp1_64,
...
void aocl_lapack_slaqp3rk(aocl_int64_t *m, aocl_int64_t *n, aocl_int64_t *nrhs,
                          aocl_int64_t *ioffset, aocl_int64_t *nb, real *abstol, real *reltol,
                          aocl_int64_t *kp1, real *maxc2nrm, real *a, aocl_int64_t *lda,
                          logical *done, aocl_int64_t *kb, real *maxc2nrmk, real *relmaxc2nrmk,
                          aocl_int_t *jpiv, real *tau, real *vn1, real *vn2, real *auxv, real *f,
                          aocl_int64_t *ldf, aocl_int_t *iwork, aocl_int64_t *info)
{
    AOCL_DTL_TRACE_LOG_INIT
    AOCL_DTL_SNPRINTF("slaqp3rk inputs: m %" FLA_IS ",n %" FLA_IS ",nrhs %" FLA_IS
    ...
    *done = FALSE_;
    ...
    while(k < *nb && lsticc == 0)
    {
            ...
            *maxc2nrmk = vn1[kp];
            ...
            if(sisnan_(maxc2nrmk))
            {
                ...
                *relmaxc2nrmk = *maxc2nrmk;
                ...
                {
                    i__1 = *m - if__;
                    aocl_blas_sgemm("No transpose", "Transpose", &i__1, nrhs, kb, &c_b7,
            ...
            if(*maxc2nrmk <= *abstol || *relmaxc2nrmk <= *reltol)
            {
                ...
                *kb = k - 1;
                ...
                if(*kb < minmnupdt)
                {
                    i__1 = *m - if__;
                    i__2 = *n + *nrhs - *kb;
                    aocl_blas_sgemm("No transpose", "Transpose", &i__1, &i__2, kb, &c_b7,
        ...
        if(kp != k)
        {
            aocl_blas_sswap(m, &a[kp * a_dim1 + 1], &c__1, &a[k * a_dim1 + 1], &c__1);
            i__1 = k - 1;
            aocl_blas_sswap(&i__1, &f[kp + f_dim1], ldf, &f[k + f_dim1], ldf);
            vn1[kp] = vn1[k];
            vn2[kp] = vn2[k];
            itemp = jpiv[kp];
            jpiv[kp] = jpiv[k];
        ...
        if(k > 1)
        {
            i__1 = *m - i__ + 1;
            i__2 = k - 1;
            aocl_blas_sgemv("No transpose", &i__1, &i__2, &c_b7, &a[i__ + a_dim1], lda,
        ...
        {
            i__1 = *m - i__ + 1;
            aocl_lapack_slarfg(&i__1, &a[i__ + k * a_dim1], &a[i__ + 1 + k * a_dim1], &c__1,
                               &tau[k]);
        }
        else
        {
        ...
        if(k < *n + *nrhs)
        {
            i__1 = *m - i__ + 1;
            i__2 = *n + *nrhs - k;
            aocl_blas_sgemv("Transpose", &i__1, &i__2, &tau[k], &a[i__ + (k + 1) * a_dim1], lda,
        ...
        if(k > 1)
        {
            ...
            i__2 = k - 1;
            r__1 = -tau[k];
            aocl_blas_sgemv("Transpose", &i__1, &i__2, &r__1, &a[i__ + a_dim1], lda,
                            &a[i__ + k * a_dim1], &c__1, &c_b30, &auxv[1], &c__1);
            i__1 = *n + *nrhs;
            i__2 = k - 1;
            aocl_blas_sgemv("No transpose", &i__1, &i__2, &c_b8, &f[f_dim1 + 1], ldf, &auxv[1],
        ...
        {
            i__1 = *n + *nrhs - k;
            aocl_blas_sgemv("No transpose", &i__1, &k, &c_b7, &f[k + 1 + f_dim1], ldf,
        ...
        if(k < minmnfact)
        {
            ...
            for(j = k + 1; j <= i__1; ++j)
            {
                if(vn1[j] != 0.f)
                {
                    ...
                    /* Lapack Working Note 176. */
                    temp = (r__1 = a[i__ + j * a_dim1], f2c_abs(r__1)) / vn1[j];
                    /* Computing MAX */
                    r__1 = 0.f;
    ...
    *kb = k;
    ...
    if(*kb < minmnupdt)
    {
        i__1 = *m - if__;
        i__2 = *n + *nrhs - *kb;
        aocl_blas_sgemm("No transpose", "Transpose", &i__1, &i__2, kb, &c_b7, &a[if__ + 1 + a_dim1],
    ...
    while(lsticc > 0)
    {
        ...
        /* SQRT(SLAMCH('S')) */
        i__1 = *m - if__;
        vn1[lsticc] = aocl_blas_snrm2(&i__1, &a[if__ + 1 + lsticc * a_dim1], &c__1);
