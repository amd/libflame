...
/* > \brief <b> SSYEVR computes the eigenvalues and, optionally, the left and/or right eigenvectors
 * for SY mat rices</b> */
/* =========== DOCUMENTATION =========== */
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/ssyevr.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/ssyevr.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/ssyevr.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
void ssyevr_(char *jobz, char *range, char *uplo, aocl_int_t *n, real *a, aocl_int_t *lda, real *vl,
             real *vu, aocl_int_t *il, aocl_int_t *iu, real *abstol, aocl_int_t *m, real *w,
             real *z__, aocl_int_t *ldz, aocl_int_t *isuppz, real *work, aocl_int_t *lwork,
             aocl_int_t *iwork, aocl_int_t *liwork, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_ssyevr(jobz, range, uplo, n, a, lda, vl, vu, il, iu, abstol, m, w, z__, ldz, isuppz,
    ...
    aocl_lapack_ssyevr(jobz, range, uplo, &n_64, a, &lda_64, vl, vu, &il_64, &iu_64, abstol, &m_64,
...
void aocl_lapack_ssyevr(char *jobz, char *range, char *uplo, aocl_int64_t *n, real *a,
                        aocl_int64_t *lda, real *vl, real *vu, aocl_int64_t *il, aocl_int64_t *iu,
                        real *abstol, aocl_int64_t *m, real *w, real *z__, aocl_int64_t *ldz,
                        aocl_int_t *isuppz, real *work, aocl_int64_t *lwork, aocl_int_t *iwork,
                        aocl_int64_t *liwork, aocl_int64_t *info)
{
    AOCL_DTL_TRACE_LOG_INIT
    AOCL_DTL_SNPRINTF("ssyevr inputs: jobz %c, range %c, uplo %c, n %" FLA_IS ", lda %" FLA_IS
    ...
    --iwork;
    /* Function Body */
    ieeeok = aocl_lapack_ilaenv(&c__10, "SSYEVR", "N", &c__1, &c__2, &c__3, &c__4);
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("SSYEVR", &i__1, (ftnlen)6);
    ...
    *m = 0;
    ...
    if(iscale == 1)
    {
        if(lower)
        {
            ...
            {
                i__2 = *n - j + 1;
                aocl_blas_sscal(&i__2, &sigma, &a[j + j * a_dim1], &c__1);
                /* L10: */
            }
        }
        else
        {
            ...
            for(j = 1; j <= i__1; ++j)
            {
                aocl_blas_sscal(&j, &sigma, &a[j * a_dim1 + 1], &c__1);
                /* L20: */
            }
        }
        if(*abstol > 0.f)
    ...
    if((alleig || test) && ieeeok == 1)
    {
        ...
        else
        {
            ...
            aocl_lapack_sstemr(jobz, "A", n, &work[inddd], &work[indee], vl, vu, il, iu, m, &w[1],
