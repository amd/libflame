...
void zlaqr3_(logical *wantt, logical *wantz, aocl_int_t *n, aocl_int_t *ktop, aocl_int_t *kbot,
...
{
#if FLA_ENABLE_ILP64
    aocl_lapack_zlaqr3(wantt, wantz, n, ktop, kbot, nw, h__, ldh, iloz, ihiz, z__, ldz, ns, nd, sh,
    ...
    aocl_lapack_zlaqr3(wantt, wantz, &n_64, &ktop_64, &kbot_64, &nw_64, h__, &ldh_64, &iloz_64,
...
void aocl_lapack_zlaqr3(logical *wantt, logical *wantz, aocl_int64_t *n, aocl_int64_t *ktop,
                        ...
                        dcomplex *work, aocl_int64_t *lwork)
{
    ...
    else
    {
        ...
        aocl_lapack_zlaqr4(&c_true, &c_true, &jw, &c__1, &jw, &t[t_offset], ldt, &sh[1], &c__1, &jw,
    ...
    *nd = 0;
    ...
    nmin = aocl_lapack_ilaenv(&c__12, "ZLAQR3", "SV", &jw, &c__1, &jw, lwork);
    if(jw > nmin)
    {
        aocl_lapack_zlaqr4(&c_true, &c_true, &jw, &c__1, &jw, &t[t_offset], ldt, &sh[kwtop], &c__1,
                           &jw, &v[v_offset], ldv, &work[1], lwork, &infqr);
    }
    else
    {
        aocl_lapack_zlahqr(&c_true, &c_true, &jw, &c__1, &jw, &t[t_offset], ldt, &sh[kwtop], &c__1,
    ...
    *ns = jw;
    ...
    if(*ns < jw || s.real == 0. && s.imag == 0.)
    {
        ...
        if(kwtop > 1)
        {
            ...
            z__1.real = s.real * z__2.real - s.imag * z__2.imag;
            z__1.imag = s.real * z__2.imag + s.imag * z__2.real; // , expr subst
