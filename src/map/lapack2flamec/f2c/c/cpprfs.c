/* ../netlib/cpprfs.f -- translated by f2c (version 20100827). You must link the resulting object
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/cpprfs.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/cpprfs.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/cpprfs.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
                        ...
                        real *berr, scomplex *work, real *rwork, aocl_int64_t *info)
{
    ...
    char buffer[256];
#if FLA_ENABLE_ILP64
    snprintf(buffer, 256, "cpprfs inputs: uplo %c, n %lld, nrhs %lld, ldb %lld, ldx %lld", *uplo,
             *n, *nrhs, *ldb, *ldx);
#else
    snprintf(buffer, 256, "cpprfs inputs: uplo %c, n %d, nrhs %d, ldb %d, ldx %d", *uplo, *n, *nrhs,
             *ldb, *ldx);
#endif
    ...
    L20: /* Loop until stopping criterion is satisfied. */
        ...
        if(upper)
        {
            ...
            for(k = 1; k <= i__2; ++k)
            {
                ...
                {
                    i__4 = ik;
                    rwork[i__] += ((r__1 = ap[i__4].real, f2c_abs(r__1))
                                   + (r__2 = r_imag(&ap[ik]), f2c_abs(r__2)))
                                  * xk;
                    i__4 = ik;
                    i__5 = i__ + j * x_dim1;
                    s += ((r__1 = ap[i__4].real, f2c_abs(r__1))
                          + (r__2 = r_imag(&ap[ik]), f2c_abs(r__2)))
                ...
                i__3 = kk + k - 1;
                rwork[k] = rwork[k] + (r__1 = ap[i__3].real, f2c_abs(r__1)) * xk + s;
                kk += k;
                /* L50: */
            }
        }
        else
        {
            ...
            for(k = 1; k <= i__2; ++k)
            {
                ...
                xk = (r__1 = x[i__3].real, f2c_abs(r__1))
                     + (r__2 = r_imag(&x[k + j * x_dim1]), f2c_abs(r__2));
                i__3 = kk;
                rwork[k] += (r__1 = ap[i__3].real, f2c_abs(r__1)) * xk;
                ik = kk + 1;
                i__3 = *n;
