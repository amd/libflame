...
/* > \brief <b> CHESV_AA_2STAGE computes the solution to system of linear equations A * X = B for HE
 * matrices </b> */
/* =========== DOCUMENTATION =========== */
...
/* > \verbatim */
/* > */
/* > CHESV_AA_2STAGE computes the solution to a scomplex system of */
/* > linear equations */
/* > A * X = B, */
...
void chesv_aa_2stage_(char *uplo, aocl_int_t *n, aocl_int_t *nrhs, scomplex *a, aocl_int_t *lda,
...
{
#if FLA_ENABLE_ILP64
    aocl_lapack_chesv_aa_2stage(uplo, n, nrhs, a, lda, tb, ltb, ipiv, ipiv2, b, ldb, work, lwork,
                                info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_chesv_aa_2stage(uplo, &n_64, &nrhs_64, a, &lda_64, tb, &ltb_64, ipiv, ipiv2, b,
...
void aocl_lapack_chesv_aa_2stage(char *uplo, aocl_int64_t *n, aocl_int64_t *nrhs, scomplex *a,
                                 ...
                                 scomplex *work, aocl_int64_t *lwork, aocl_int64_t *info)
{
...
#if FLA_ENABLE_ILP64
    snprintf(buffer, 256,
             "chesv_aa_2stage inputs: uplo %c, n %lld, nrhs %lld, lda %lld, ltb %lld, ldb %lld, "
             "lwork %lld",
             *uplo, *n, *nrhs, *lda, *ltb, *ldb, *lwork);
#else
    snprintf(buffer, 256,
             "chesv_aa_2stage inputs: uplo %c, n %d, nrhs %d, lda %d, ltb %d, ldb %d, lwork %d",
             *uplo, *n, *nrhs, *lda, *ltb, *ldb, *lwork);
#endif
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("CHESV_AA_2STAGE", &i__1, (ftnlen)15);
    ...
    aocl_lapack_chetrf_aa_2stage(uplo, n, &a[a_offset], lda, &tb[1], ltb, &ipiv[1], &ipiv2[1],
                                 &work[1], lwork, info);
    if(*info == 0)
    {
        /* Solve the system A*X = B, overwriting B with X. */
        aocl_lapack_chetrs_aa_2stage(uplo, n, nrhs, &a[a_offset], lda, &tb[1], ltb, &ipiv[1],
