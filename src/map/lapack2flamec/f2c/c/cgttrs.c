/* ../netlib/cgttrs.f -- translated by f2c (version 20100827). You must link the resulting object
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/cgttrs.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/cgttrs.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/cgttrs.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
{
#if FLA_ENABLE_ILP64
    aocl_lapack_cgttrs(trans, n, nrhs, dl, d__, du, du2, ipiv, b, ldb, info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_cgttrs(trans, &n_64, &nrhs_64, dl, d__, du, du2, ipiv, b, &ldb_64, &info_64);

    *info = (aocl_int_t)info_64;
#endif
}

void aocl_lapack_cgttrs(char *trans, aocl_int64_t *n, aocl_int64_t *nrhs, scomplex *dl, scomplex *d__,
                        ...
                        aocl_int64_t *info)
{
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("CGTTRS", &i__1, (ftnlen)6);
    ...
    else
    {
        /* Computing MAX */
        i__1 = 1;
        i__2 = aocl_lapack_ilaenv(&c__1, "CGTTRS", trans, n, nrhs, &c_n1, &c_n1); // , expr subst
        nb = fla_max(i__1, i__2);
    }
    if(nb >= *nrhs)
    {
        aocl_lapack_cgtts2(&itrans, n, nrhs, &dl[1], &d__[1], &du[1], &du2[1], &ipiv[1],
                           &b[b_offset], ldb);
    }
    else
    {
        ...
        for(j = 1; i__2 < 0 ? j >= i__1 : j <= i__1; j += i__2)
        {
            ...
            i__3 = *nrhs - j + 1;
            jb = fla_min(i__3, nb);
            aocl_lapack_cgtts2(&itrans, n, &jb, &dl[1], &d__[1], &du[1], &du2[1], &ipiv[1],
                               &b[j * b_dim1 + 1], ldb);
            /* L10: */
        }
    }
