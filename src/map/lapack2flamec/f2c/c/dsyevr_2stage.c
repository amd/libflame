...
/* > \brief <b> DSYEVR_2STAGE computes the eigenvalues and, optionally, the left and/or right
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/dsyevr_
 * 2stage.f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/dsyevr_
 * 2stage.f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/dsyevr_
 * 2stage.f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
void dsyevr_2stage_(char *jobz, char *range, char *uplo, aocl_int_t *n, doublereal *a,
                    ...
                    aocl_int_t *ldz, aocl_int_t *isuppz, doublereal *work, aocl_int_t *lwork,
                    aocl_int_t *iwork, aocl_int_t *liwork, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_dsyevr_2stage(jobz, range, uplo, n, a, lda, vl, vu, il, iu, abstol, m, w, z__, ldz,
    ...
    aocl_lapack_dsyevr_2stage(jobz, range, uplo, &n_64, a, &lda_64, vl, vu, &il_64, &iu_64, abstol,
                              &m_64, w, z__, &ldz_64, isuppz, work, &lwork_64, iwork, &liwork_64,
                              &info_64);
...
void aocl_lapack_dsyevr_2stage(char *jobz, char *range, char *uplo, aocl_int64_t *n, doublereal *a,
                               ...
                               doublereal *z__, aocl_int64_t *ldz, aocl_int_t *isuppz,
                               doublereal *work, aocl_int64_t *lwork, aocl_int_t *iwork,
                               aocl_int64_t *liwork, aocl_int64_t *info)
{
    AOCL_DTL_TRACE_LOG_INIT
    AOCL_DTL_SNPRINTF("dsyevr_2stage inputs: jobz %c, range %c, uplo %c, n %" FLA_IS
                      ", lda %" FLA_IS ", il %" FLA_IS ", iu %" FLA_IS ", ldz %" FLA_IS
                      ", lwork %" FLA_IS ", liwork %" FLA_IS "",
                      *jobz, *range, *uplo, *n, *lda, *il, *iu, *ldz, *lwork, *liwork);
    ...
    --iwork;
    /* Function Body */
    ieeeok = aocl_lapack_ilaenv(&c__10, "DSYEVR", "N", &c__1, &c__2, &c__3, &c__4);
    ...
    kd = aocl_lapack_ilaenv2stage(&c__1, "DSYTRD_2STAGE", jobz, n, &c_n1, &c_n1, &c_n1);
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("DSYEVR_2STAGE", &i__1, (ftnlen)13);
    ...
    *m = 0;
    ...
    if(iscale == 1)
    {
        if(lower)
        {
            ...
            {
                i__2 = *n - j + 1;
                aocl_blas_dscal(&i__2, &sigma, &a[j + j * a_dim1], &c__1);
                /* L10: */
            }
        }
        else
        {
            ...
            for(j = 1; j <= i__1; ++j)
            {
                aocl_blas_dscal(&j, &sigma, &a[j * a_dim1 + 1], &c__1);
                /* L20: */
            }
        }
        if(*abstol > 0.)
    ...
    aocl_lapack_dsytrd_2stage(jobz, uplo, n, &a[a_offset], lda, &work[indd], &work[inde],
    ...
    if((alleig || indeig && *il == 1 && *iu == *n) && ieeeok == 1)
    {
        ...
        else
        {
            i__1 = *n - 1;
            aocl_blas_dcopy(&i__1, &work[inde], &c__1, &work[indee], &c__1);
            aocl_blas_dcopy(n, &work[indd], &c__1, &work[inddd], &c__1);
            if(*abstol <= *n * 2. * eps)
            {
                tryrac = TRUE_;
            }
            ...
            aocl_lapack_dstemr(jobz, "A", n, &work[inddd], &work[indee], vl, vu, il, iu, m, &w[1],
                               &z__[z_offset], ldz, n, &isuppz[1], &tryrac, &work[indwk], lwork,
                               &iwork[1], liwork, info);
    ...
    aocl_lapack_dstebz(range, order, n, &vll, &vuu, il, iu, &abstll, &work[indd], &work[inde], m,
                       &nsplit, &w[1], &iwork[indibl], &iwork[indisp], &work[indwk], &iwork[indiwo],
                       info);
    if(wantz)
    {
        ...
        indwkn = inde;
        llwrkn = *lwork - indwkn + 1;
        aocl_lapack_dormtr("L", uplo, "N", n, m, &a[a_offset], lda, &work[indtau], &z__[z_offset],
