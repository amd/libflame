...
/* ../netlib/zsyrfsx.f -- translated by f2c (version 20160102). You must link the resulting object
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/zsyrfsx
 * .f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/zsyrfsx
 * .f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/zsyrfsx
 * .f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
/* ===================================================================== */
/* Subroutine */
void zsyrfsx_(char *uplo, char *equed, integer *n, integer *nrhs, dcomplex *a, integer *lda,
              dcomplex *af, integer *ldaf, integer *ipiv, doublereal *s, dcomplex *b,
              integer *ldb, dcomplex *x, integer *ldx, doublereal *rcond, doublereal *berr,
              ...
              integer *info)
{
    ...
    AOCL_DTL_SNPRINTF("zsyrfsx inputs: uplo %c, equed %c, n %" FLA_IS ", nrhs %" FLA_IS
    ...
    extern /* Subroutine */
        void
        zla_syrfsx_extended_(integer *, char *, integer *, integer *, dcomplex *, integer *,
    ...
    doublereal anorm;
    logical rcequ;
    extern doublereal zla_syrcond_c_(char *, integer *, dcomplex *, integer *, dcomplex *,
                                     integer *, integer *, doublereal *, logical *, integer *,
                                     dcomplex *, doublereal *),
        zla_syrcond_x_(char *, integer *, dcomplex *, integer *, dcomplex *, integer *,
                       integer *, dcomplex *, integer *, dcomplex *, doublereal *),
        dlamch_(char *);
    extern /* Subroutine */
        void
        xerbla_(const char *srname, const integer *info, ftnlen srname_len);
    extern doublereal zlansy_(char *, char *, integer *, dcomplex *, integer *, doublereal *);
    extern /* Subroutine */
        void
        zsycon_(char *, integer *, dcomplex *, integer *, integer *, doublereal *,
    ...
    *(unsigned char *)norm = 'I';
    ...
    if(ref_type__ != 0)
    {
        ...
        zla_syrfsx_extended_(&prec_type__, uplo, n, nrhs, &a[a_offset], lda, &af[af_offset], ldaf,
                             ...
                             &work[*n + 1], &rwork[1], rcond, &ithresh, &rthresh,
    ...
    if(*n_err_bnds__ >= 1 && n_norms__ >= 1)
    {
        ...
        if(rcequ)
        {
            rcond_tmp__ = zla_syrcond_c_(uplo, n, &a[a_offset], lda, &af[af_offset], ldaf, &ipiv[1],
                                         &s[1], &c_true, info, &work[1], &rwork[1]);
        }
        else
        {
            rcond_tmp__ = zla_syrcond_c_(uplo, n, &a[a_offset], lda, &af[af_offset], ldaf, &ipiv[1],
                                         &s[1], &c_false, info, &work[1], &rwork[1]);
        }
        i__1 = *nrhs;
        for(j = 1; j <= i__1; ++j)
        {
    ...
    if(*n_err_bnds__ >= 1 && n_norms__ >= 2)
    {
        ...
        for(j = 1; j <= i__1; ++j)
        {
            ...
            {
                rcond_tmp__
                    = zla_syrcond_x_(uplo, n, &a[a_offset], lda, &af[af_offset], ldaf, &ipiv[1],
