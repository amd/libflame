...
/* ===================================================================== */
/* Subroutine */
void stpqrt_(integer *m, integer *n, integer *l, integer *nb, real *a, integer *lda, real *b,
             integer *ldb, real *t, integer *ldt, real *work, integer *info)
{
    AOCL_DTL_TRACE_LOG_INIT
    AOCL_DTL_SNPRINTF(
             "stpqrt inputs: m %" FLA_IS ", n %" FLA_IS ", l %" FLA_IS ", nb %" FLA_IS
    ...
    extern /* Subroutine */
        ...
        stprfb_(char *, char *, char *, char *, integer *, integer *, integer *, integer *, real *,
                ...
                integer *),
        stpqrt2_(integer *, integer *, integer *, real *, integer *, real *, integer *, real *,
                 integer *, integer *);
    ...
    *info = 0;
    ...
    for(i__ = 1; i__2 < 0 ? i__ >= i__1 : i__ <= i__1; i__ += i__2)
    {
        ...
        stpqrt2_(&mb, &ib, &lb, &a[i__ + i__ * a_dim1], lda, &b[i__ * b_dim1 + 1], ldb,
                 &t[i__ * t_dim1 + 1], ldt, &iinfo);
        /* Update by applying H^H to B(:,I+IB:N) from the left */
        if(i__ + ib <= *n)
        {
            i__3 = *n - i__ - ib + 1;
            stprfb_("L", "T", "F", "C", &mb, &i__3, &ib, &lb, &b[i__ * b_dim1 + 1], ldb,
