/* ../netlib/dtrrfs.f -- translated by f2c (version 20100827). You must link the resulting object
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/dtrrfs.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/dtrrfs.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/dtrrfs.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
void dtrrfs_(char *uplo, char *trans, char *diag, aocl_int_t *n, aocl_int_t *nrhs, doublereal *a,
             aocl_int_t *lda, doublereal *b, aocl_int_t *ldb, doublereal *x, aocl_int_t *ldx,
             doublereal *ferr, doublereal *berr, doublereal *work, aocl_int_t *iwork,
             aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_dtrrfs(uplo, trans, diag, n, nrhs, a, lda, b, ldb, x, ldx, ferr, berr, work, iwork,
                       info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_dtrrfs(uplo, trans, diag, &n_64, &nrhs_64, a, &lda_64, b, &ldb_64, x, &ldx_64, ferr,
...
void aocl_lapack_dtrrfs(char *uplo, char *trans, char *diag, aocl_int64_t *n, aocl_int64_t *nrhs,
                        ...
                        doublereal *work, aocl_int_t *iwork, aocl_int64_t *info)
{
    ...
    AOCL_DTL_SNPRINTF("dtrrfs inputs: uplo %c, trans %c, diag %c, n %" FLA_IS ", nrhs %" FLA_IS
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("DTRRFS", &i__1, (ftnlen)6);
    ...
    for(j = 1; j <= i__1; ++j)
    {
        ...
        aocl_blas_dtrmv(uplo, trans, diag, n, &a[a_offset], lda, &work[*n + 1], &c__1);
    ...
    L210:
        ...
        if(kase != 0)
        {
            if(kase == 1)
            {
                ...
                aocl_blas_dtrsv(uplo, transt, diag, n, &a[a_offset], lda, &work[*n + 1], &c__1);
                i__2 = *n;
                for(i__ = 1; i__ <= i__2; ++i__)
                {
            ...
            else
            {
                ...
                aocl_blas_dtrsv(uplo, trans, diag, n, &a[a_offset], lda, &work[*n + 1], &c__1);
            }
            goto L210;
        }
        /* Normalize error. */
