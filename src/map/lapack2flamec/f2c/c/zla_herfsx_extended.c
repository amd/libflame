...
/* ../netlib/zla_herfsx_extended.f -- translated by f2c (version 20100827). You must link the
                          ...
                          logical *ignore_cwise__, integer *info)
{
    ...
    AOCL_DTL_SNPRINTF("zla_herfsx_extended inputs: prec_type__ %" FLA_IS ", uplo %c, n %" FLA_IS
    ...
    *info = 0;
    ...
    for(j = 1; j <= i__1; ++j)
    {
        ...
        for(cnt = 1; cnt <= i__2; ++cnt)
        {
            ...
            if(y_prec_state__ == 0)
            {
                zhemv_(uplo, n, &c_b14, &a[a_offset], lda, &y[j * y_dim1 + 1], &c__1, &c_b15,
                       &res[1], &c__1);
            }
            else if(y_prec_state__ == 1)
            {
                blas_zhemv_x_(&uplo2, n, &c_b14, &a[a_offset], lda, &y[j * y_dim1 + 1], &c__1,
                              &c_b15, &res[1], &c__1, prec_type__);
            }
            else
            {
                blas_zhemv2_x_(&uplo2, n, &c_b14, &a[a_offset], lda, &y[j * y_dim1 + 1],
    ...
    L666: /* Set final_* when cnt hits ithresh. */
        ...
        /* op(A) = A, A**T, or A**H depending on TRANS (and type). */
        zcopy_(n, &b[j * b_dim1 + 1], &c__1, &res[1], &c__1);
        zhemv_(uplo, n, &c_b14, &a[a_offset], lda, &y[j * y_dim1 + 1], &c__1, &c_b15, &res[1],
               &c__1);
        i__2 = *n;
        for(i__ = 1; i__ <= i__2; ++i__)
        {
        ...
        zla_heamv_(&uplo2, n, &c_b37, &a[a_offset], lda, &y[j * y_dim1 + 1], &c__1, &c_b37, &ayb[1],
                   &c__1);
