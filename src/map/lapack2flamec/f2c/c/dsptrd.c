...
{
#if FLA_ENABLE_ILP64
    aocl_lapack_dsptrd(uplo, n, ap, d__, e, tau, info);
#else
    aocl_int64_t n_64 = *n;
    aocl_int64_t info_64 = *info;

    aocl_lapack_dsptrd(uplo, &n_64, ap, d__, e, tau, &info_64);

    *info = (aocl_int_t)info_64;
#endif
}

void aocl_lapack_dsptrd(char *uplo, aocl_int64_t *n, doublereal *ap, doublereal *d__, doublereal *e,
                        doublereal *tau, aocl_int64_t *info)
{
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("DSPTRD", &i__1, (ftnlen)6);
    ...
    if(upper)
    {
        ...
        for(i__ = *n - 1; i__ >= 1; --i__)
        {
            ...
            aocl_lapack_dlarfg(&i__, &ap[i1 + i__ - 1], &ap[i1], &c__1, &taui);
            e[i__] = ap[i1 + i__ - 1];
            if(taui != 0.)
            {
    ...
    else
    {
        ...
        for(i__ = 1; i__ <= i__1; ++i__)
        {
            ...
            i__2 = *n - i__;
            aocl_lapack_dlarfg(&i__2, &ap[ii + 1], &ap[ii + 2], &c__1, &taui);
            e[i__] = ap[ii + 1];
            if(taui != 0.)
            {
                ...
                i__2 = *n - i__;
                aocl_blas_daxpy(&i__2, &alpha, &ap[ii + 1], &c__1, &tau[i__], &c__1);
                /* Apply the transformation as a rank-2 update: */
                /* A := A - v * w**T - w * v**T */
                i__2 = *n - i__;
