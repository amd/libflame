...
/* Local prototypes.*/
void fla_test_sygvd_experiment(test_params_t *params, integer datatype, integer p_cur,
    ...
    if(argc >= 12 && argc <= 13)
    {
        ...
        if(n_repeats > 0)
        {
            ...
            for(i = 0; i < num_types; i++)
            {
                ...
                /* Call the test code */
                fla_test_sygvd_experiment(params, datatype, N, N, 0, n_repeats, einfo, &perf,
                                          &time_min, &residual);
                /* Print the results */
