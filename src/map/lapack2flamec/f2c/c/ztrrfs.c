...
{
#if FLA_ENABLE_ILP64
    aocl_lapack_ztrrfs(uplo, trans, diag, n, nrhs, a, lda, b, ldb, x, ldx, ferr, berr, work, rwork,
                       info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_ztrrfs(uplo, trans, diag, &n_64, &nrhs_64, a, &lda_64, b, &ldb_64, x, &ldx_64, ferr,
...
void aocl_lapack_ztrrfs(char *uplo, char *trans, char *diag, aocl_int64_t *n, aocl_int64_t *nrhs,
                        ...
                        dcomplex *work, doublereal *rwork, aocl_int64_t *info)
{
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("ZTRRFS", &i__1, (ftnlen)6);
    ...
    for(j = 1; j <= i__1; ++j)
    {
        ...
        aocl_blas_ztrmv(uplo, trans, diag, n, &a[a_offset], lda, &work[1], &c__1);
        z__1.real = -1.;
    ...
    L210:
        ...
        if(kase != 0)
        {
            if(kase == 1)
            {
                ...
                aocl_blas_ztrsv(uplo, transt, diag, n, &a[a_offset], lda, &work[1], &c__1);
                i__2 = *n;
                for(i__ = 1; i__ <= i__2; ++i__)
                {
                    i__3 = i__;
            ...
            else
            {
                ...
                aocl_blas_ztrsv(uplo, transn, diag, n, &a[a_offset], lda, &work[1], &c__1);
            }
            goto L210;
        }
        /* Normalize error. */
