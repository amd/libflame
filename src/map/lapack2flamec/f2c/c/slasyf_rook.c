...
/* > \brief \b SLASYF_ROOK computes a partial factorization of a real symmetric matrix using the
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/slasyf_
 * rook.f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/slasyf_
 * rook.f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/slasyf_
 * rook.f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
                  ...
                  aocl_int_t *lda, aocl_int_t *ipiv, real *w, aocl_int_t *ldw, aocl_int_t *info)
{
    ...
    aocl_lapack_slasyf_rook(uplo, &n_64, &nb_64, &kb_64, a, &lda_64, ipiv, w, &ldw_64, &info_64);
...
void aocl_lapack_slasyf_rook(char *uplo, aocl_int64_t *n, aocl_int64_t *nb, aocl_int64_t *kb,
                             real *a, aocl_int64_t *lda, aocl_int_t *ipiv, real *w,
                             aocl_int64_t *ldw, aocl_int64_t *info)
{
    ...
    *info = 0;
    ...
    L60:
        ...
        {
            i__1 = *n - j + 1;
            aocl_blas_sswap(&i__1, &a[jp2 + j * a_dim1], lda, &a[jj + j * a_dim1], lda);
        }
        jj = j - 1;
        if(jp1 != jj && kstep == 2)
        {
            i__1 = *n - j + 1;
            aocl_blas_sswap(&i__1, &a[jp1 + j * a_dim1], lda, &a[jj + j * a_dim1], lda);
        }
        if(j <= *n)
        {
            goto L60;
    ...
    L120:
        ...
        if(jp2 != jj && j >= 1)
        {
            aocl_blas_sswap(&j, &a[jp2 + a_dim1], lda, &a[jj + a_dim1], lda);
        }
        jj = j + 1;
        if(jp1 != jj && kstep == 2)
        {
            aocl_blas_sswap(&j, &a[jp1 + a_dim1], lda, &a[jj + a_dim1], lda);
        }
        if(j >= 1)
        {
            goto L120;
