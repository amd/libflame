/* ../netlib/dsbgst.f -- translated by f2c (version 20100827). You must link the resulting object
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/dsbgst.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/dsbgst.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/dsbgst.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
void dsbgst_(char *vect, char *uplo, aocl_int_t *n, aocl_int_t *ka, aocl_int_t *kb, doublereal *ab,
             aocl_int_t *ldab, doublereal *bb, aocl_int_t *ldbb, doublereal *x, aocl_int_t *ldx,
             doublereal *work, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_dsbgst(vect, uplo, n, ka, kb, ab, ldab, bb, ldbb, x, ldx, work, info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_dsbgst(vect, uplo, &n_64, &ka_64, &kb_64, ab, &ldab_64, bb, &ldbb_64, x, &ldx_64,
                       work, &info_64);
...
void aocl_lapack_dsbgst(char *vect, char *uplo, aocl_int64_t *n, aocl_int64_t *ka, aocl_int64_t *kb,
                        doublereal *ab, aocl_int64_t *ldab, doublereal *bb, aocl_int64_t *ldbb,
                        doublereal *x, aocl_int64_t *ldx, doublereal *work, aocl_int64_t *info)
{
    AOCL_DTL_TRACE_LOG_INIT
    AOCL_DTL_SNPRINTF("dsbgst inputs: vect %c, uplo %c, n %" FLA_IS ", ka %" FLA_IS ", kb %" FLA_IS
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("DSBGST", &i__1, (ftnlen)6);
    ...
    if(wantx)
    {
        aocl_lapack_dlaset("Full", n, n, &c_b8, &c_b9, &x[x_offset], ldx);
    }
    ...
    if(upper)
    {
        ...
        if(update)
        {
            ...
            for(k = i__ - kbt; k <= i__3; ++k)
            {
                ...
                for(j = i__ - kbt; j <= i__1; ++j)
                {
                    ab[j - k + ka1 + k * ab_dim1]
                        = ab[j - k + ka1 + k * ab_dim1]
                          - bb[j - i__ + kb1 + i__ * bb_dim1] * ab[k - i__ + ka1 + i__ * ab_dim1]
                          - bb[k - i__ + kb1 + i__ * bb_dim1] * ab[j - i__ + ka1 + i__ * ab_dim1]
                          + ab[ka1 + i__ * ab_dim1] * bb[j - i__ + kb1 + i__ * bb_dim1]
                                * bb[k - i__ + kb1 + i__ * bb_dim1];
                    /* L40: */
                }
                /* Computing MAX */
                i__1 = 1;
            ...
            if(wantx)
            {
                ...
                i__3 = *n - m;
                d__1 = 1. / bii;
                aocl_blas_dscal(&i__3, &d__1, &x[m + 1 + i__ * x_dim1], &c__1);
                if(kbt > 0)
                {
                    i__3 = *n - m;
                    aocl_blas_dger(&i__3, &kbt, &c_b20, &x[m + 1 + i__ * x_dim1], &c__1,
        ...
        for(k = 1; k <= i__3; ++k)
        {
            if(update)
            {
                ...
                if(i__ - k + *ka < *n && i__ - k > 1)
                {
                    ...
                    dlartg_(&ab[k + 1 + (i__ - k + *ka) * ab_dim1], &ra1,
                            &work[*n + i__ - k + *ka - m], &work[i__ - k + *ka - m], &ra);
                    /* create nonzero element a(i-k,i-k+ka+1) outside the */
                    /* band and store it in WORK(i-k) */
                    t = -bb[kb1 - k + i__ * bb_dim1] * ra1;
                    work[i__ - k] = work[*n + i__ - k + *ka - m] * t
                                    - work[i__ - k + *ka - m] * ab[(i__ - k + *ka) * ab_dim1 + 1];
                    ab[(i__ - k + *ka) * ab_dim1 + 1]
                        = work[i__ - k + *ka - m] * t
                          + work[*n + i__ - k + *ka - m] * ab[(i__ - k + *ka) * ab_dim1 + 1];
                    ra1 = ra;
                }
            }
            /* Computing MAX */
            ...
            if(nrt > 0)
            {
                aocl_lapack_dlargv(&nrt, &ab[j2t * ab_dim1 + 1], &inca, &work[j2t - m], &ka1,
                                   &work[*n + j2t - m], &ka1);
            }
            if(nr > 0)
            {
                ...
                for(l = 1; l <= i__4; ++l)
                {
                    aocl_lapack_dlartv(&nr, &ab[ka1 - l + j2 * ab_dim1], &inca,
                ...
                /* blocks */
                aocl_lapack_dlar2v(&nr, &ab[ka1 + j2 * ab_dim1], &ab[ka1 + (j2 + 1) * ab_dim1],
            ...
            for(l = *ka - 1; l >= i__4; --l)
            {
                ...
                if(nrt > 0)
                {
                    aocl_lapack_dlartv(&nrt, &ab[l + (j2 + ka1 - l) * ab_dim1], &inca,
            ...
            if(wantx)
            {
                ...
                {
                    i__1 = *n - m;
                    aocl_blas_drot(&i__1, &x[m + 1 + j * x_dim1], &c__1,
        ...
        for(k = *kb; k >= 1; --k)
        {
            ...
            for(l = *kb - k; l >= 1; --l)
            {
                ...
                if(nrt > 0)
                {
                    aocl_lapack_dlartv(&nrt, &ab[l + (j2 - l + 1) * ab_dim1], &inca,
        ...
        for(k = *kb; k >= 1; --k)
        {
            ...
            if(nr > 0)
            {
                ...
                aocl_lapack_dlargv(&nr, &ab[j2 * ab_dim1 + 1], &inca, &work[j2], &ka1,
                                   &work[*n + j2], &ka1);
            ...
            if(wantx)
            {
                ...
                {
                    i__4 = *n - m;
                    aocl_blas_drot(&i__4, &x[m + 1 + j * x_dim1], &c__1,
    ...
    else
    {
        ...
        if(update)
        {
            ...
            for(k = i__ - kbt; k <= i__4; ++k)
            {
                ...
                for(j = i__ - kbt; j <= i__2; ++j)
                {
                    ab[k - j + 1 + j * ab_dim1]
                        = ab[k - j + 1 + j * ab_dim1]
                          - bb[i__ - j + 1 + j * bb_dim1] * ab[i__ - k + 1 + k * ab_dim1]
                          - bb[i__ - k + 1 + k * bb_dim1] * ab[i__ - j + 1 + j * ab_dim1]
                          + ab[i__ * ab_dim1 + 1] * bb[i__ - j + 1 + j * bb_dim1]
                                * bb[i__ - k + 1 + k * bb_dim1];
                    /* L270: */
                }
                /* Computing MAX */
                i__2 = 1;
            ...
            if(wantx)
            {
                ...
                i__4 = *n - m;
                d__1 = 1. / bii;
                aocl_blas_dscal(&i__4, &d__1, &x[m + 1 + i__ * x_dim1], &c__1);
                if(kbt > 0)
                {
                    i__4 = *n - m;
                    i__3 = *ldbb - 1;
                    aocl_blas_dger(&i__4, &kbt, &c_b20, &x[m + 1 + i__ * x_dim1], &c__1,
        ...
        for(k = 1; k <= i__4; ++k)
        {
            if(update)
            {
                ...
                if(i__ - k + *ka < *n && i__ - k > 1)
                {
                    ...
                    dlartg_(&ab[ka1 - k + i__ * ab_dim1], &ra1, &work[*n + i__ - k + *ka - m],
                            &work[i__ - k + *ka - m], &ra);
                    /* create nonzero element a(i-k+ka+1,i-k) outside the */
                    /* band and store it in WORK(i-k) */
                    t = -bb[k + 1 + (i__ - k) * bb_dim1] * ra1;
                    work[i__ - k] = work[*n + i__ - k + *ka - m] * t
                                    - work[i__ - k + *ka - m] * ab[ka1 + (i__ - k) * ab_dim1];
                    ab[ka1 + (i__ - k) * ab_dim1]
                        = work[i__ - k + *ka - m] * t
                          + work[*n + i__ - k + *ka - m] * ab[ka1 + (i__ - k) * ab_dim1];
                    ra1 = ra;
                }
            }
            /* Computing MAX */
            ...
            for(j = j2t; i__1 < 0 ? j >= i__3 : j <= i__3; j += i__1)
            {
                ...
                ab[ka1 + (j - *ka + 1) * ab_dim1]
                    = work[*n + j - m] * ab[ka1 + (j - *ka + 1) * ab_dim1];
                /* L320: */
            }
            /* generate rotations in 1st set to annihilate elements which */
            /* have been created outside the band */
            if(nrt > 0)
            {
                aocl_lapack_dlargv(&nrt, &ab[ka1 + (j2t - *ka) * ab_dim1], &inca, &work[j2t - m],
            ...
            if(nr > 0)
            {
                ...
                for(l = 1; l <= i__1; ++l)
                {
                    aocl_lapack_dlartv(&nr, &ab[l + 1 + (j2 - l) * ab_dim1], &inca,
                ...
                /* apply rotations in 1st set from both sides to diagonal */
                /* blocks */
                aocl_lapack_dlar2v(&nr, &ab[j2 * ab_dim1 + 1], &ab[(j2 + 1) * ab_dim1 + 1],
            ...
            for(l = *ka - 1; l >= i__1; --l)
            {
                ...
                if(nrt > 0)
                {
                    aocl_lapack_dlartv(&nrt, &ab[ka1 - l + 1 + j2 * ab_dim1], &inca,
            ...
            if(wantx)
            {
                ...
                {
                    i__2 = *n - m;
                    aocl_blas_drot(&i__2, &x[m + 1 + j * x_dim1], &c__1,
                                   &x[m + 1 + (j + 1) * x_dim1], &c__1, &work[*n + j - m],
                                   &work[j - m]);
                    /* L350: */
                }
        ...
        for(k = *kb; k >= 1; --k)
        {
            ...
            for(l = *kb - k; l >= 1; --l)
            {
                ...
                if(nrt > 0)
                {
                    aocl_lapack_dlartv(&nrt, &ab[ka1 - l + 1 + (j2 - *ka) * ab_dim1], &inca,
            ...
            for(j = j2; i__4 < 0 ? j >= i__3 : j <= i__3; j += i__4)
            {
                ...
                ab[ka1 + (j - *ka + 1) * ab_dim1]
                    = work[*n + j] * ab[ka1 + (j - *ka + 1) * ab_dim1];
                /* L390: */
            }
            if(update)
            {
        ...
        for(k = *kb; k >= 1; --k)
        {
            ...
            if(nr > 0)
            {
                /* generate rotations in 2nd set to annihilate elements */
                /* which have been created outside the band */
                aocl_lapack_dlargv(&nr, &ab[ka1 + (j2 - *ka) * ab_dim1], &inca, &work[j2], &ka1,
                                   &work[*n + j2], &ka1);
                ...
                {
                    aocl_lapack_dlartv(&nr, &ab[l + 1 + (j2 - l) * ab_dim1], &inca,
                                       &ab[l + 2 + (j2 - l) * ab_dim1], &inca, &work[*n + j2],
                                       &work[j2], &ka1);
                    /* L410: */
                }
                /* apply rotations in 2nd set from both sides to diagonal */
...
L490:
    ...
    if(upper)
    {
        ...
        if(update)
        {
            ...
            for(k = i__ + 1; k <= i__3; ++k)
            {
                ...
                for(j = k; j <= i__4; ++j)
                {
                    ab[k - j + ka1 + j * ab_dim1]
                        = ab[k - j + ka1 + j * ab_dim1]
                          - bb[i__ - j + kb1 + j * bb_dim1] * ab[i__ - k + ka1 + k * ab_dim1]
                          - bb[i__ - k + kb1 + k * bb_dim1] * ab[i__ - j + ka1 + j * ab_dim1]
                          + ab[ka1 + i__ * ab_dim1] * bb[i__ - j + kb1 + j * bb_dim1]
                                * bb[i__ - k + kb1 + k * bb_dim1];
                    /* L520: */
                }
                /* Computing MIN */
                i__1 = *n;
            ...
            if(wantx)
            {
                /* post-multiply X by inv(S(i)) */
                d__1 = 1. / bii;
                aocl_blas_dscal(&nx, &d__1, &x[i__ * x_dim1 + 1], &c__1);
                if(kbt > 0)
                {
                    i__3 = *ldbb - 1;
                    aocl_blas_dger(&nx, &kbt, &c_b20, &x[i__ * x_dim1 + 1], &c__1,
        ...
        for(k = 1; k <= i__3; ++k)
        {
            if(update)
            {
                ...
                {
                    /* generate rotation to annihilate a(i+k-ka-1,i) */
                    dlartg_(&ab[k + 1 + i__ * ab_dim1], &ra1, &work[*n + i__ + k - *ka],
                            &work[i__ + k - *ka], &ra);
                    /* create nonzero element a(i+k-ka-1,i+k) outside the */
                    ...
                    ab[(i__ + k) * ab_dim1 + 1]
                        = work[i__ + k - *ka] * t
                          + work[*n + i__ + k - *ka] * ab[(i__ + k) * ab_dim1 + 1];
                    ra1 = ra;
                }
            }
            /* Computing MAX */
            ...
            if(nrt > 0)
            {
                aocl_lapack_dlargv(&nrt, &ab[(j1 + *ka) * ab_dim1 + 1], &inca, &work[j1], &ka1,
                                   &work[*n + j1], &ka1);
            }
            if(nr > 0)
            {
                ...
                for(l = 1; l <= i__1; ++l)
                {
                    aocl_lapack_dlartv(&nr, &ab[ka1 - l + (j1 + l) * ab_dim1], &inca,
                ...
                /* apply rotations in 1st set from both sides to diagonal */
                /* blocks */
                aocl_lapack_dlar2v(&nr, &ab[ka1 + j1 * ab_dim1], &ab[ka1 + (j1 - 1) * ab_dim1],
            ...
            for(l = *ka - 1; l >= i__1; --l)
            {
                ...
                if(nrt > 0)
                {
                    aocl_lapack_dlartv(&nrt, &ab[l + j1t * ab_dim1], &inca,
            ...
            if(wantx)
            {
                ...
                {
                    aocl_blas_drot(&nx, &x[j * x_dim1 + 1], &c__1, &x[(j - 1) * x_dim1 + 1], &c__1,
                                   &work[*n + j], &work[j]);
                    /* L600: */
                }
            }
        ...
        for(k = *kb; k >= 1; --k)
        {
            ...
            for(l = *kb - k; l >= 1; --l)
            {
                ...
                if(nrt > 0)
                {
                    aocl_lapack_dlartv(&nrt, &ab[l + (j1t + *ka) * ab_dim1], &inca,
            ...
            for(j = j1; i__3 < 0 ? j >= i__4 : j <= i__4; j += i__3)
            {
                ...
                ab[(j + *ka - 1) * ab_dim1 + 1]
                    = work[*n + m - *kb + j] * ab[(j + *ka - 1) * ab_dim1 + 1];
                /* L640: */
            }
            if(update)
            {
        ...
        for(k = *kb; k >= 1; --k)
        {
            ...
            if(nr > 0)
            {
                /* generate rotations in 2nd set to annihilate elements */
                /* which have been created outside the band */
                aocl_lapack_dlargv(&nr, &ab[(j1 + *ka) * ab_dim1 + 1], &inca, &work[m - *kb + j1],
    ...
    else
    {
        ...
        if(update)
        {
            ...
            for(k = i__ + 1; k <= i__4; ++k)
            {
                ...
                for(j = k; j <= i__3; ++j)
                {
                    ab[j - k + 1 + k * ab_dim1]
                        = ab[j - k + 1 + k * ab_dim1]
                          - bb[j - i__ + 1 + i__ * bb_dim1] * ab[k - i__ + 1 + i__ * ab_dim1]
                          - bb[k - i__ + 1 + i__ * bb_dim1] * ab[j - i__ + 1 + i__ * ab_dim1]
                          + ab[i__ * ab_dim1 + 1] * bb[j - i__ + 1 + i__ * bb_dim1]
                                * bb[k - i__ + 1 + i__ * bb_dim1];
                    /* L750: */
                }
                /* Computing MIN */
                i__1 = *n;
        ...
        for(k = 1; k <= i__4; ++k)
        {
            if(update)
            {
                ...
                if(i__ + k - ka1 > 0 && i__ + k < m)
                {
                    ...
                    dlartg_(&ab[ka1 - k + (i__ + k - *ka) * ab_dim1], &ra1,
                            &work[*n + i__ + k - *ka], &work[i__ + k - *ka], &ra);
                    /* create nonzero element a(i+k,i+k-ka-1) outside the */
                    /* band and store it in WORK(m-kb+i+k) */
                    t = -bb[k + 1 + i__ * bb_dim1] * ra1;
                    work[m - *kb + i__ + k]
                        = work[*n + i__ + k - *ka] * t
                          - work[i__ + k - *ka] * ab[ka1 + (i__ + k - *ka) * ab_dim1];
                    ab[ka1 + (i__ + k - *ka) * ab_dim1]
                        = work[i__ + k - *ka] * t
                          + work[*n + i__ + k - *ka] * ab[ka1 + (i__ + k - *ka) * ab_dim1];
                    ra1 = ra;
                }
            }
            /* Computing MAX */
            ...
            if(nrt > 0)
            {
                aocl_lapack_dlargv(&nrt, &ab[ka1 + j1 * ab_dim1], &inca, &work[j1], &ka1,
                                   &work[*n + j1], &ka1);
            }
            if(nr > 0)
            {
                ...
                for(l = 1; l <= i__1; ++l)
                {
                    aocl_lapack_dlartv(&nr, &ab[l + 1 + j1 * ab_dim1], &inca,
                ...
                /* apply rotations in 1st set from both sides to diagonal */
                /* blocks */
                aocl_lapack_dlar2v(&nr, &ab[j1 * ab_dim1 + 1], &ab[(j1 - 1) * ab_dim1 + 1],
                                   &ab[(j1 - 1) * ab_dim1 + 2], &inca, &work[*n + j1], &work[j1],
                                   &ka1);
            }
            /* start applying rotations in 1st set from the left */
            i__1 = *kb - k + 1;
            for(l = *ka - 1; l >= i__1; --l)
            {
                ...
                if(nrt > 0)
                {
                    aocl_lapack_dlartv(&nrt, &ab[ka1 - l + 1 + (j1t - ka1 + l) * ab_dim1], &inca,
        ...
        for(k = *kb; k >= 1; --k)
        {
            ...
            for(l = *kb - k; l >= 1; --l)
            {
                ...
                if(nrt > 0)
                {
                    aocl_lapack_dlartv(&nrt, &ab[ka1 - l + 1 + (j1t + l - 1) * ab_dim1], &inca,
        ...
        for(k = *kb; k >= 1; --k)
        {
            ...
            if(nr > 0)
            {
                /* generate rotations in 2nd set to annihilate elements */
                /* which have been created outside the band */
                aocl_lapack_dlargv(&nr, &ab[ka1 + j1 * ab_dim1], &inca, &work[m - *kb + j1], &ka1,
                                   &work[*n + m - *kb + j1], &ka1);
                /* apply rotations in 2nd set from the right */
                i__4 = *ka - 1;
                for(l = 1; l <= i__4; ++l)
