...
/* > \brief <b> SSBEVX_2STAGE computes the eigenvalues and, optionally, the left and/or right
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/ssbevx_
 * 2stage.f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/ssbevx_
 * 2stage.f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/ssbevx_
 * 2stage.f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
void ssbevx_2stage_(char *jobz, char *range, char *uplo, aocl_int_t *n, aocl_int_t *kd, real *ab,
                    aocl_int_t *ldab, real *q, aocl_int_t *ldq, real *vl, real *vu, aocl_int_t *il,
                    aocl_int_t *iu, real *abstol, aocl_int_t *m, real *w, real *z__,
                    aocl_int_t *ldz, real *work, aocl_int_t *lwork, aocl_int_t *iwork,
                    aocl_int_t *ifail, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_ssbevx_2stage(jobz, range, uplo, n, kd, ab, ldab, q, ldq, vl, vu, il, iu, abstol, m,
    ...
    aocl_lapack_ssbevx_2stage(jobz, range, uplo, &n_64, &kd_64, ab, &ldab_64, q, &ldq_64, vl, vu,
...
void aocl_lapack_ssbevx_2stage(char *jobz, char *range, char *uplo, aocl_int64_t *n,
                               aocl_int64_t *kd, real *ab, aocl_int64_t *ldab, real *q,
                               aocl_int64_t *ldq, real *vl, real *vu, aocl_int64_t *il,
                               aocl_int64_t *iu, real *abstol, aocl_int64_t *m, real *w, real *z__,
                               aocl_int64_t *ldz, real *work, aocl_int64_t *lwork,
                               aocl_int_t *iwork, aocl_int_t *ifail, aocl_int64_t *info)
{
    AOCL_DTL_TRACE_LOG_INIT
    AOCL_DTL_SNPRINTF("ssbevx_2stage inputs: jobz %c, range %c, uplo %c, n %" FLA_IS ", kd %" FLA_IS
    ...
    *info = 0;
    ...
    if(*info == 0)
    {
        ...
        {
            ib = aocl_lapack_ilaenv2stage(&c__2, "SSYTRD_SB2ST", jobz, n, kd, &c_n1, &c_n1);
            lhtrd = aocl_lapack_ilaenv2stage(&c__3, "SSYTRD_SB2ST", jobz, n, kd, &ib, &c_n1);
            lwtrd = aocl_lapack_ilaenv2stage(&c__4, "SSYTRD_SB2ST", jobz, n, kd, &ib, &c_n1);
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("SSBEVX_2STAGE ", &i__1, (ftnlen)14);
    ...
    *m = 0;
    ...
    if(iscale == 1)
    {
        if(lower)
        {
            aocl_lapack_slascl("B", kd, kd, &c_b24, &sigma, n, n, &ab[ab_offset], ldab, info);
        }
        else
        {
            aocl_lapack_slascl("Q", kd, kd, &c_b24, &sigma, n, n, &ab[ab_offset], ldab, info);
        }
        if(*abstol > 0.f)
        {
            abstll = *abstol * sigma;
    ...
    aocl_lapack_ssytrd_sb2st("N", jobz, uplo, n, kd, &ab[ab_offset], ldab, &work[indd], &work[inde],
    ...
    if(wantz)
    {
        ...
        for(j = 1; j <= i__1; ++j)
        {
            ...
            aocl_blas_sgemv("N", n, n, &c_b24, &q[q_offset], ldq, &work[1], &c__1, &c_b45,
                            &z__[j * z_dim1 + 1], &c__1);
            /* L20: */
        }
    }
