...
void dpttrf_(aocl_int_t *n, doublereal *d__, doublereal *e, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_dpttrf(n, d__, e, info);
#else
    aocl_int64_t n_64 = *n;
    aocl_int64_t info_64 = *info;

    aocl_lapack_dpttrf(&n_64, d__, e, &info_64);

    *info = (aocl_int_t)info_64;
#endif
}

void aocl_lapack_dpttrf(aocl_int64_t *n, doublereal *d__, doublereal *e, aocl_int64_t *info)
{
    ...
    *info = 0;
    if(*n < 0)
    {
        *info = -1;
        i__1 = -(*info);
        aocl_blas_xerbla("DPTTRF", &i__1, (ftnlen)6);
