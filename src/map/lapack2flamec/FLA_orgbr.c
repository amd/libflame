...
#define LAPACK_orgbr_body(prefix)                                               \
    ...
    {                                                                           \
        ...
        PREFIX2FLAME_INVERT_TAU(prefix, t);                                     \
                                                                                ...
                                                                                \
    FLA_Part_2x2(A, &ATL, &ATR, &ABL, &ABR, m_t, m_t, FLA_TL);                  \
                                                                                \
    /* Accumulate house-holder vectors */                                       \
    if(*vect == 'Q')                                                            \
    { /* U */                                                                   \
        FLA_Part_2x1(ATL, &A1, &A2, (uplo == FLA_LOWER_TRIANGULAR), FLA_TOP);   \
        FLA_Merge_2x1(A2, ABL, &Ah);                                            \
        FLA_Merge_2x1(ATL, ABL, &A2);                                           \
        FLA_Part_2x1(t, &t, &T, FLA_Obj_min_dim(Ah), FLA_TOP);                  \
                                                                                \
...
LAPACK_orgbr(s, org)
{
    ...
    AOCL_DTL_SNPRINTF("sorgbr inputs: vect %c, m %" FLA_IS ", n %" FLA_IS ", k %" FLA_IS
                      ", lda %" FLA_IS "",
                      *vect, *m, *n, *k, *ldim_A);
    {
        LAPACK_RETURN_CHECK_VAR1(
            sorgbr_check(vect, m, n, k, buff_A, ldim_A, buff_t, buff_w, lwork, info), fla_error)
    }
...
LAPACK_orgbr(c, ung)
{
    ...
    {
        LAPACK_RETURN_CHECK_VAR1(
            cungbr_check(vect, m, n, k, buff_A, ldim_A, buff_t, buff_w, lwork, info), fla_error)
    }
    if(fla_error == LAPACK_SUCCESS)
    {
        LAPACK_orgbr_body(c)
...
LAPACK_orgbr(z, ung)
{
    ...
    {
        LAPACK_RETURN_CHECK_VAR1(
            zungbr_check(vect, m, n, k, buff_A, ldim_A, buff_t, buff_w, lwork, info), fla_error)
    }
    if(fla_error == LAPACK_SUCCESS)
    {
        LAPACK_orgbr_body(z)
