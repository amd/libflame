...
void ssytrf_rook_(char *uplo, aocl_int_t *n, real *a, aocl_int_t *lda, aocl_int_t *ipiv, real *work,
                  aocl_int_t *lwork, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_ssytrf_rook(uplo, n, a, lda, ipiv, work, lwork, info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_ssytrf_rook(uplo, &n_64, a, &lda_64, ipiv, work, &lwork_64, &info_64);

    *info = (aocl_int_t)info_64;
#endif
}

void aocl_lapack_ssytrf_rook(char *uplo, aocl_int64_t *n, real *a, aocl_int64_t *lda,
                             aocl_int_t *ipiv, real *work, aocl_int64_t *lwork, aocl_int64_t *info)
{
    ...
    *info = 0;
    ...
    if(*info == 0)
    {
        ...
        nb = aocl_lapack_ilaenv(&c__1, "SSYTRF_ROOK", uplo, n, &c_n1, &c_n1, &c_n1);
        /* Computing MAX */
        i__1 = 1;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("SSYTRF_ROOK", &i__1, (ftnlen)11);
    ...
    if(nb > 1 && nb < *n)
    {
        ...
        if(*lwork < iws)
        {
            ...
            /* Computing MAX */
            i__1 = 2;
            i__2 = aocl_lapack_ilaenv(&c__2, "SSYTRF_ROOK", uplo, n, &c_n1, &c_n1,
                                      &c_n1); // , expr subst
    ...
    L10: /* If K < 1, exit from loop */
        ...
        if(k > nb)
        {
            ...
            aocl_lapack_slasyf_rook(uplo, &k, &nb, &kb, &a[a_offset], lda, &ipiv[1], &work[1],
                                    &ldwork, &iinfo);
        }
        else
        {
            /* Use unblocked code to factorize columns 1:k of A */
            aocl_lapack_ssytf2_rook(uplo, &k, &a[a_offset], lda, &ipiv[1], &iinfo);
            kb = k;
        }
    ...
    L20: /* If K > N, exit from loop */
        ...
        if(k <= *n - nb)
        {
            ...
            i__1 = *n - k + 1;
            aocl_lapack_slasyf_rook(uplo, &i__1, &nb, &kb, &a[k + k * a_dim1], lda, &ipiv[k],
                                    &work[1], &ldwork, &iinfo);
        }
        else
        {
            ...
            i__1 = *n - k + 1;
            aocl_lapack_ssytf2_rook(uplo, &i__1, &a[k + k * a_dim1], lda, &ipiv[k], &iinfo);
            kb = *n - k + 1;
        }
        /* Set INFO on the first occurrence of a zero pivot */
        if(*info == 0 && iinfo > 0)
