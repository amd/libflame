...
/* > \verbatim */
/* > */
/* > CSYTRS_AA solves a system of linear equations A*X = B with a scomplex */
...
void csytrs_aa_(char *uplo, aocl_int_t *n, aocl_int_t *nrhs, scomplex *a, aocl_int_t *lda,
                aocl_int_t *ipiv, scomplex *b, aocl_int_t *ldb, scomplex *work, aocl_int_t *lwork,
                aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_csytrs_aa(uplo, n, nrhs, a, lda, ipiv, b, ldb, work, lwork, info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_csytrs_aa(uplo, &n_64, &nrhs_64, a, &lda_64, ipiv, b, &ldb_64, work, &lwork_64,
                          &info_64);
...
void aocl_lapack_csytrs_aa(char *uplo, aocl_int64_t *n, aocl_int64_t *nrhs, scomplex *a,
                           ...
                           scomplex *work, aocl_int64_t *lwork, aocl_int64_t *info)
{
...
#if FLA_ENABLE_ILP64
    snprintf(buffer, 256,
             "csytrs_aa inputs: uplo %c, n %lld, nrhs %lld, lda %lld, ldb %lld, lwork %lld", *uplo,
             *n, *nrhs, *lda, *ldb, *lwork);
#else
    snprintf(buffer, 256, "csytrs_aa inputs: uplo %c, n %d, nrhs %d, lda %d, ldb %d, lwork %d",
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("CSYTRS_AA", &i__1, (ftnlen)9);
    ...
    if(upper)
    {
        ...
        if(*n > 1)
        {
            ...
            i__1 = *n - 1;
            aocl_blas_ctrsm("L", "U", "T", "U", &i__1, nrhs, &c_b9, &a[(a_dim1 << 1) + 1], lda,
                            &b[b_dim1 + 2], ldb);
        }
    ...
    else
    {
        ...
        if(*n > 1)
        {
            ...
            i__1 = *n - 1;
            aocl_blas_ctrsm("L", "L", "T", "U", &i__1, nrhs, &c_b9, &a[a_dim1 + 2], lda,
                            &b[b_dim1 + 2], ldb);
