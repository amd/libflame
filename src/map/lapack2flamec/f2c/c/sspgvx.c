...
{
#if FLA_ENABLE_ILP64
    aocl_lapack_sspgvx(itype, jobz, range, uplo, n, ap, bp, vl, vu, il, iu, abstol, m, w, z__, ldz,
    ...
    aocl_lapack_sspgvx(&itype_64, jobz, range, uplo, &n_64, ap, bp, vl, vu, &il_64, &iu_64, abstol,
...
void aocl_lapack_sspgvx(aocl_int64_t *itype, char *jobz, char *range, char *uplo, aocl_int64_t *n,
                        real *ap, real *bp, real *vl, real *vu, aocl_int64_t *il, aocl_int64_t *iu,
                        ...
                        real *work, aocl_int_t *iwork, aocl_int_t *ifail, aocl_int64_t *info)
{
    ...
    AOCL_DTL_SNPRINTF("sspgvx inputs: itype %" FLA_IS ", jobz %c, range %c, uplo %c, n %" FLA_IS
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("SSPGVX", &i__1, (ftnlen)6);
    ...
    *m = 0;
    ...
    aocl_lapack_sspevx(jobz, range, uplo, n, &ap[1], vl, vu, il, iu, abstol, m, &w[1],
    ...
    if(wantz)
    {
        ...
        if(*itype == 1 || *itype == 2)
        {
            ...
            for(j = 1; j <= i__1; ++j)
            {
                aocl_blas_stpsv(uplo, trans, "Non-unit", n, &bp[1], &z__[j * z_dim1 + 1], &c__1);
                /* L10: */
            }
        }
        else if(*itype == 3)
        {
            ...
            for(j = 1; j <= i__1; ++j)
            {
                aocl_blas_stpmv(uplo, trans, "Non-unit", n, &bp[1], &z__[j * z_dim1 + 1], &c__1);
                /* L20: */
            }
        }
    }
