...
/* Local prototypes */
void fla_test_larfg_experiment(test_params_t *params, integer datatype, integer p_cur, integer q_cur, integer pci,
                               integer n_repeats, integer einfo, double *perf, double *t, double *residual);
void prepare_larfg_run(integer datatype, integer n_A, integer incx, void *x, void *tau, integer n_repeats, double *time_min_);
void invoke_larfg(integer datatype, integer *n, void *x, integer *incx, integer *abs_incx, void *tau);
...
void fla_test_larfg(integer argc, char **argv, test_params_t *params)
{
    ...
    if (argc >= 6 && argc <= 7)
    {
        ...
        params->aux_paramslist[0].incx_larfg = strtoimax(argv[4], &endptr, CLI_DECIMAL_BASE);
        ...
        if (n_repeats > 0)
        {
            ...
            for (i = 0; i < num_types; i++)
            {
                ...
                /* Call the test code */
                fla_test_larfg_experiment(params, datatype,
                                          N, N,
                                          0,
                                          n_repeats, einfo,
    ...
    if (tests_not_run)
    {
        ...
        printf("./<EXE> LARFG <precisions - sdcz>  <N> <incx> <repeats>\n");
    }
    if (invalid_dtype)
    {
...
void fla_test_larfg_experiment(test_params_t *params,
                               integer datatype,
                               integer p_cur,
                               integer q_cur,
                               integer pci,
...
void prepare_larfg_run(integer datatype,
                       integer n_A, integer incx,
                       void *x, void *tau,
                       integer n_repeats,
                       double *time_min_)
{
