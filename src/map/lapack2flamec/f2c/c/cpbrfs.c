...
void cpbrfs_(char *uplo, aocl_int_t *n, aocl_int_t *kd, aocl_int_t *nrhs, scomplex *ab,
             aocl_int_t *ldab, scomplex *afb, aocl_int_t *ldafb, scomplex *b, aocl_int_t *ldb,
             scomplex *x, aocl_int_t *ldx, real *ferr, real *berr, scomplex *work, real *rwork,
             aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_cpbrfs(uplo, n, kd, nrhs, ab, ldab, afb, ldafb, b, ldb, x, ldx, ferr, berr, work,
                       rwork, info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_cpbrfs(uplo, &n_64, &kd_64, &nrhs_64, ab, &ldab_64, afb, &ldafb_64, b, &ldb_64, x,
...
void aocl_lapack_cpbrfs(char *uplo, aocl_int64_t *n, aocl_int64_t *kd, aocl_int64_t *nrhs,
                        ...
                        real *berr, scomplex *work, real *rwork, aocl_int64_t *info)
{
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("CPBRFS", &i__1, (ftnlen)6);
    ...
    L20: /* Loop until stopping criterion is satisfied. */
        ...
        aocl_blas_chbmv(uplo, n, kd, &q__1, &ab[ab_offset], ldab, &x[j * x_dim1 + 1], &c__1, &c_b1,
                        &work[1], &c__1);
        ...
        if(upper)
        {
            ...
            for(k = 1; k <= i__2; ++k)
            {
                ...
                i__5 = *kd + 1 + k * ab_dim1;
                rwork[k] = rwork[k] + (r__1 = ab[i__5].real, f2c_abs(r__1)) * xk + s;
                /* L50: */
            }
        }
        else
        {
            ...
            for(k = 1; k <= i__2; ++k)
            {
                ...
                xk = (r__1 = x[i__5].real, f2c_abs(r__1))
                     + (r__2 = r_imag(&x[k + j * x_dim1]), f2c_abs(r__2));
                i__5 = k * ab_dim1 + 1;
                rwork[k] += (r__1 = ab[i__5].real, f2c_abs(r__1)) * xk;
                l = 1 - k;
                /* Computing MIN */
                i__3 = *n;
                i__4 = k + *kd; // , expr subst
        ...
        if(berr[j] > eps && berr[j] * 2.f <= lstres && count <= 5)
        {
            ...
            aocl_lapack_cpbtrs(uplo, n, kd, &c__1, &afb[afb_offset], ldafb, &work[1], n, info);
