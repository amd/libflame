/* ../netlib/ctrrfs.f -- translated by f2c (version 20100827). You must link the resulting object
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/ctrrfs.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/ctrrfs.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/ctrrfs.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
void ctrrfs_(char *uplo, char *trans, char *diag, aocl_int_t *n, aocl_int_t *nrhs, scomplex *a,
             aocl_int_t *lda, scomplex *b, aocl_int_t *ldb, scomplex *x, aocl_int_t *ldx, real *ferr,
             real *berr, scomplex *work, real *rwork, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_ctrrfs(uplo, trans, diag, n, nrhs, a, lda, b, ldb, x, ldx, ferr, berr, work, rwork,
                       info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_ctrrfs(uplo, trans, diag, &n_64, &nrhs_64, a, &lda_64, b, &ldb_64, x, &ldx_64, ferr,
                       berr, work, rwork, &info_64);

    *info = (aocl_int_t)info_64;
#endif
}

void aocl_lapack_ctrrfs(char *uplo, char *trans, char *diag, aocl_int64_t *n, aocl_int64_t *nrhs,
                        scomplex *a, aocl_int64_t *lda, scomplex *b, aocl_int64_t *ldb, scomplex *x,
                        ...
                        aocl_int64_t *info)
{
...
#if FLA_ENABLE_ILP64
    snprintf(buffer, 256,
             "ctrrfs inputs: uplo %c, trans %c, diag %c, n %lld, nrhs %lld, lda %lld, ldb %lld, "
             "ldx %lld",
             *uplo, *trans, *diag, *n, *nrhs, *lda, *ldb, *ldx);
#else
    snprintf(buffer, 256,
             "ctrrfs inputs: uplo %c, trans %c, diag %c, n %d, nrhs %d, lda %d, ldb %d, ldx %d",
             *uplo, *trans, *diag, *n, *nrhs, *lda, *ldb, *ldx);
#endif
    AOCL_DTL_LOG(AOCL_DTL_LEVEL_TRACE_5, buffer);
#endif
    /* System generated locals */
    aocl_int64_t a_dim1, a_offset, b_dim1, b_offset, x_dim1, x_offset, i__1, i__2, i__3, i__4, i__5;
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("CTRRFS", &i__1, (ftnlen)6);
    ...
    for(j = 1; j <= i__1; ++j)
    {
        ...
        aocl_blas_ctrmv(uplo, trans, diag, n, &a[a_offset], lda, &work[1], &c__1);
        q__1.real = -1.f;
        ...
        else
        {
            ...
            if(upper)
            {
                ...
                else
                {
                    ...
                    {
                        i__3 = k + j * x_dim1;
                        s = (r__1 = x[i__3].real, f2c_abs(r__1))
                            + (r__2 = r_imag(&x[k + j * x_dim1]), f2c_abs(r__2));
                        i__3 = k - 1;
                        for(i__ = 1; i__ <= i__3; ++i__)
                        {
    ...
    L210:
        ...
        if(kase != 0)
        {
            if(kase == 1)
            {
                ...
                aocl_blas_ctrsv(uplo, transt, diag, n, &a[a_offset], lda, &work[1], &c__1);
                i__2 = *n;
                for(i__ = 1; i__ <= i__2; ++i__)
                {
                    i__3 = i__;
                    i__4 = i__;
                    i__5 = i__;
                    q__1.real = rwork[i__4] * work[i__5].real;
                    q__1.imag = rwork[i__4] * work[i__5].imag; // , expr subst
            ...
            else
            {
                ...
                aocl_blas_ctrsv(uplo, transn, diag, n, &a[a_offset], lda, &work[1], &c__1);
            }
            goto L210;
        }
        /* Normalize error. */
