/* ../netlib/sla_syrcond.f -- translated by f2c (version 20160102). You must link the resulting
...
real sla_syrcond_(char *uplo, aocl_int_t *n, real *a, aocl_int_t *lda, real *af, aocl_int_t *ldaf,
                  aocl_int_t *ipiv, aocl_int_t *cmode, real *c__, aocl_int_t *info, real *work,
                  aocl_int_t *iwork)
{
#if FLA_ENABLE_ILP64
    return aocl_lapack_sla_syrcond(uplo, n, a, lda, af, ldaf, ipiv, cmode, c__, info, work, iwork);
#else
    aocl_int64_t n_64 = *n;
    ...
    real ret_val = aocl_lapack_sla_syrcond(uplo, &n_64, a, &lda_64, af, &ldaf_64, ipiv, &cmode_64,
...
real aocl_lapack_sla_syrcond(char *uplo, aocl_int64_t *n, real *a, aocl_int64_t *lda, real *af,
                             ...
                             aocl_int64_t *info, real *work, aocl_int_t *iwork)
{
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("SLA_SYRCOND", &i__1, (ftnlen)11);
        return ret_val;
    }
    if(*n == 0)
    {
...
L10:
    ...
    if(kase != 0)
    {
        if(kase == 2)
        {
            ...
            if(up)
            {
                aocl_lapack_ssytrs("U", n, &c__1, &af[af_offset], ldaf, &ipiv[1], &work[1], n,
                                   info);
            }
            else
            {
                aocl_lapack_ssytrs("L", n, &c__1, &af[af_offset], ldaf, &ipiv[1], &work[1], n,
                                   info);
            }
            /* Multiply by inv(C). */
            if(*cmode == 1)
