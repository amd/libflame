/* ../netlib/sgegv.f -- translated by f2c (version 20100827). You must link the resulting object
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/sgegv.f
 * "> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/sgegv.f
 * "> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/sgegv.f
 * "> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
void sgegv_(char *jobvl, char *jobvr, aocl_int_t *n, real *a, aocl_int_t *lda, real *b, aocl_int_t *ldb, real *alphar, real *alphai, real *beta, real *vl, aocl_int_t *ldvl, real *vr, aocl_int_t *ldvr, real *work, aocl_int_t *lwork, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
            ...
            real *vr, aocl_int64_t *ldvr, real *work, aocl_int64_t *lwork, aocl_int64_t *info)
{
    ...
    *info = 0;
    ...
    if(anrm > 0.f)
    {
        aocl_lapack_slascl("G", &c_n1, &c_n1, &anrm, &c_b27, n, n, &a[a_offset], lda, &iinfo);
        if(iinfo != 0)
        {
            *info = *n + 10;
            AOCL_DTL_TRACE_LOG_EXIT
    ...
    if(bnrm > 0.f)
    {
        aocl_lapack_slascl("G", &c_n1, &c_n1, &bnrm, &c_b27, n, n, &b[b_offset], ldb, &iinfo);
        if(iinfo != 0)
        {
            *info = *n + 10;
            AOCL_DTL_TRACE_LOG_EXIT
    ...
    if(ilvl)
    {
        aocl_lapack_slaset("Full", n, n, &c_b38, &c_b27, &vl[vl_offset], ldvl);
        i__1 = irows - 1;
        i__2 = irows - 1;
    ...
    if(ilvr)
    {
        aocl_lapack_slaset("Full", n, n, &c_b38, &c_b27, &vr[vr_offset], ldvr);
    }
