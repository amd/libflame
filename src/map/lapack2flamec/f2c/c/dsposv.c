...
void dsposv_(char *uplo, aocl_int_t *n, aocl_int_t *nrhs, doublereal *a, aocl_int_t *lda,
             doublereal *b, aocl_int_t *ldb, doublereal *x, aocl_int_t *ldx, doublereal *work,
             real *swork, aocl_int_t *iter, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_dsposv(uplo, n, nrhs, a, lda, b, ldb, x, ldx, work, swork, iter, info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_dsposv(uplo, &n_64, &nrhs_64, a, &lda_64, b, &ldb_64, x, &ldx_64, work, swork,
                       &iter_64, &info_64);
...
void aocl_lapack_dsposv(char *uplo, aocl_int64_t *n, aocl_int64_t *nrhs, doublereal *a,
                        aocl_int64_t *lda, doublereal *b, aocl_int64_t *ldb, doublereal *x,
                        ...
                        aocl_int64_t *info)
{
    ...
    *iter = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("DSPOSV", &i__1, (ftnlen)6);
    ...
    anrm = aocl_lapack_dlansy("I", uplo, n, &a[a_offset], lda, &work[work_offset]);
    eps = dlamch_("Epsilon");
    ...
    /* result in SA. */
    aocl_lapack_dlat2s(uplo, n, &a[a_offset], lda, &swork[ptsa], n, info);
    if(*info != 0)
    {
        *iter = -2;
        goto L40;
    ...
    aocl_lapack_spotrf(uplo, n, &swork[ptsa], n, info);
    if(*info != 0)
    {
        *iter = -3;
        goto L40;
    ...
    aocl_lapack_spotrs(uplo, n, nrhs, &swork[ptsa], n, &swork[ptsx], n, info);
    ...
    aocl_blas_dsymm("Left", uplo, n, nrhs, &c_b10, &a[a_offset], lda, &x[x_offset], ldx, &c_b11,
                    &work[work_offset], n);
...
L10:
    for(iiter = 1; iiter <= 30; ++iiter)
    {
        ...
        /* Compute R = B - AX (R is WORK). */
        aocl_lapack_dlacpy("All", n, nrhs, &b[b_offset], ldb, &work[work_offset], n);
        aocl_blas_dsymm("L", uplo, n, nrhs, &c_b10, &a[a_offset], lda, &x[x_offset], ldx, &c_b11,
                        &work[work_offset], n);
        /* Check whether the NRHS normwise backward errors satisfy the */
...
L40: /* Single-precision iterative refinement failed to converge to a */
    ...
    aocl_lapack_dpotrs(uplo, n, nrhs, &a[a_offset], lda, &x[x_offset], ldx, info);
    AOCL_DTL_TRACE_LOG_EXIT
    return;
    /* End of DSPOSV. */
}
