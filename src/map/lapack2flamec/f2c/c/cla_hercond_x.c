/* ../netlib/cla_hercond_x.f -- translated by f2c (version 20100827). You must link the resulting
                    ...
                    integer *ipiv, complex *x, integer *info, complex *work, real *rwork)
{
    ...
    char buffer[256];
#if FLA_ENABLE_ILP64
    snprintf(buffer, 256, "cla_hercond_x inputs: uplo %c, n %lld, lda %lld, ldaf %lld", *uplo, *n,
             *lda, *ldaf);
#else
    snprintf(buffer, 256, "cla_hercond_x inputs: uplo %c, n %d, lda %d, ldaf %d", *uplo, *n, *lda,
             *ldaf);
#endif
