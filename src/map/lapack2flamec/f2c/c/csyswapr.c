/* csyswapr.f -- translated by f2c (version 20190311). You must link the resulting object file with
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/csyswap
 * r.f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/csyswap
 * r.f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/csyswap
 * r.f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
void csyswapr_(char *uplo, aocl_int_t *n, scomplex *a, aocl_int_t *lda, aocl_int_t *i1,
               aocl_int_t *i2)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_csyswapr(uplo, n, a, lda, i1, i2);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_csyswapr(uplo, &n_64, a, &lda_64, &i1_64, &i2_64);
#endif
}

void aocl_lapack_csyswapr(char *uplo, aocl_int64_t *n, scomplex *a, aocl_int64_t *lda,
                          aocl_int64_t *i1, aocl_int64_t *i2)
{
    AOCL_DTL_TRACE_LOG_INIT
    AOCL_DTL_SNPRINTF("csyswapr inputs: uplo %c, n %" FLA_IS ", lda %" FLA_IS ", i1 %" FLA_IS
                      ", i2 %" FLA_IS "",
                      *uplo, *n, *lda, *i1, *i2);
    ...
    if(upper)
    {
        ...
        {
            i__1 = *n - *i2;
            aocl_blas_cswap(&i__1, &a[*i1 + (*i2 + 1) * a_dim1], lda, &a[*i2 + (*i2 + 1) * a_dim1],
                            lda);
        }
    }
    else
    {
        ...
        {
            i__1 = *n - *i2;
            aocl_blas_cswap(&i__1, &a[*i2 + 1 + *i1 * a_dim1], &c__1, &a[*i2 + 1 + *i2 * a_dim1],
                            &c__1);
        }
    }
