...
#include "FLA_f2c.h" /* > \brief \b ZSPMV computes a matrix-vector product for scomplex vectors using a scomplex symmetric packed mat rix */
/* =========== DOCUMENTATION =========== */
...
{
#if FLA_ENABLE_ILP64
    aocl_lapack_zspmv(uplo, n, alpha, ap, x, incx, beta, y, incy);
#else
    aocl_int64_t n_64 = *n;
    aocl_int64_t incx_64 = *incx;
    aocl_int64_t incy_64 = *incy;

    aocl_lapack_zspmv(uplo, &n_64, alpha, ap, x, &incx_64, beta, y, &incy_64);
#endif
}

void aocl_lapack_zspmv(char *uplo, aocl_int64_t *n, dcomplex *alpha, dcomplex *ap,
                       dcomplex *x, aocl_int64_t *incx, dcomplex *beta, dcomplex *y,
                       aocl_int64_t *incy)
{
    ...
    if(info != 0)
    {
        aocl_blas_xerbla("ZSPMV ", &info, (ftnlen)6);
    ...
    if(lsame_(uplo, "U", 1, 1))
    {
        ...
        if(*incx == 1 && *incy == 1)
        {
            ...
            for(j = 1; j <= i__1; ++j)
            {
                ...
                i__3 = j;
                i__4 = kk + j - 1;
                z__3.real = temp1.real * ap[i__4].real - temp1.imag * ap[i__4].imag;
                z__3.imag = temp1.real * ap[i__4].imag + temp1.imag * ap[i__4].real; // , expr subst
    ...
    else
    {
        ...
        if(*incx == 1 && *incy == 1)
        {
            ...
            for(j = 1; j <= i__1; ++j)
            {
                ...
                i__3 = j;
                i__4 = kk;
                z__2.real = temp1.real * ap[i__4].real - temp1.imag * ap[i__4].imag;
                z__2.imag = temp1.real * ap[i__4].imag + temp1.imag * ap[i__4].real; // , expr subst
