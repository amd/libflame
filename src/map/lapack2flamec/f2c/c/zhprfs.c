/* ../netlib/zhprfs.f -- translated by f2c (version 20100827). You must link the resulting object
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/zhprfs.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/zhprfs.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/zhprfs.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
void zhprfs_(char *uplo, aocl_int_t *n, aocl_int_t *nrhs, dcomplex *ap, dcomplex *afp,
...
{
#if FLA_ENABLE_ILP64
    aocl_lapack_zhprfs(uplo, n, nrhs, ap, afp, ipiv, b, ldb, x, ldx, ferr, berr, work, rwork, info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_zhprfs(uplo, &n_64, &nrhs_64, ap, afp, ipiv, b, &ldb_64, x, &ldx_64, ferr, berr,
                       work, rwork, &info_64);
...
void aocl_lapack_zhprfs(char *uplo, aocl_int64_t *n, aocl_int64_t *nrhs, dcomplex *ap,
                        dcomplex *afp, aocl_int_t *ipiv, dcomplex *b, aocl_int64_t *ldb,
                        dcomplex *x, aocl_int64_t *ldx, doublereal *ferr, doublereal *berr,
                        dcomplex *work, doublereal *rwork, aocl_int64_t *info)
{
    AOCL_DTL_TRACE_LOG_INIT
    AOCL_DTL_SNPRINTF("zhprfs inputs: uplo %c, n %" FLA_IS ", nrhs %" FLA_IS ", ldb %" FLA_IS
                      ", ldx %" FLA_IS "",
                      *uplo, *n, *nrhs, *ldb, *ldx);
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("ZHPRFS", &i__1, (ftnlen)6);
    ...
    L20: /* Loop until stopping criterion is satisfied. */
        ...
        if(upper)
        {
            ...
            for(k = 1; k <= i__2; ++k)
            {
                ...
                i__3 = kk + k - 1;
                rwork[k] = rwork[k] + (d__1 = ap[i__3].real, f2c_dabs(d__1)) * xk + s;
                kk += k;
                /* L50: */
            }
        }
        else
        {
            ...
            for(k = 1; k <= i__2; ++k)
            {
                ...
                xk = (d__1 = x[i__3].real, f2c_dabs(d__1))
                     + (d__2 = d_imag(&x[k + j * x_dim1]), f2c_dabs(d__2));
                i__3 = kk;
                rwork[k] += (d__1 = ap[i__3].real, f2c_dabs(d__1)) * xk;
                ik = kk + 1;
                i__3 = *n;
        ...
        if(berr[j] > eps && berr[j] * 2. <= lstres && count <= 5)
        {
            ...
            aocl_lapack_zhptrs(uplo, n, &c__1, &afp[1], &ipiv[1], &work[1], n, info);
