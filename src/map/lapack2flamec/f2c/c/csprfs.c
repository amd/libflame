/* ../netlib/csprfs.f -- translated by f2c (version 20100827). You must link the resulting object
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/csprfs.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/csprfs.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/csprfs.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
    ...
    L20: /* Loop until stopping criterion is satisfied. */
        ...
        if(upper)
        {
            ...
            for(k = 1; k <= i__2; ++k)
            {
                ...
                i__3 = kk + k - 1;
                rwork[k] = rwork[k]
                           + ((r__1 = ap[i__3].real, f2c_abs(r__1))
                              + (r__2 = r_imag(&ap[kk + k - 1]), f2c_abs(r__2)))
                                 * xk
                           + s;
                kk += k;
                /* L50: */
        ...
        else
        {
            ...
            for(k = 1; k <= i__2; ++k)
            {
                ...
                xk = (r__1 = x[i__3].real, f2c_abs(r__1))
                     + (r__2 = r_imag(&x[k + j * x_dim1]), f2c_abs(r__2));
                i__3 = kk;
                rwork[k] += ((r__1 = ap[i__3].real, f2c_abs(r__1))
                             + (r__2 = r_imag(&ap[kk]), f2c_abs(r__2)))
                            * xk;
                ik = kk + 1;
                i__3 = *n;
        ...
        if(berr[j] > eps && berr[j] * 2.f <= lstres && count <= 5)
        {
            ...
            aocl_lapack_csptrs(uplo, n, &c__1, &afp[1], &ipiv[1], &work[1], n, info);
