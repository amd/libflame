...
integer row_major_geqrf_lda;

/* Local prototypes. */
void fla_test_geqrf_experiment(char *tst_api, test_params_t *params, integer datatype,
    ...
    if(argc >= 8 && argc <= 9)
    {
        ...
        if(n_repeats > 0)
        {
            ...
            for(i = 0; i < num_types; i++)
            {
                ...
                /* Call the test code */
                fla_test_geqrf_experiment(front_str, params, datatype, M, N, 0, n_repeats, einfo);
                tests_not_run = 0;
            }
        }
    }
                       ...
                       int layout)
{
       ...
       && (g_lwork <= 0))
    {
        ...
        {
            invoke_cpp_geqrf(datatype, &m_A, &n_A, NULL, &lda, NULL, work, &lwork, info);
        }
        else
#endif
        {
    ...
    *info = 0;
    for(i = 0; i < n_repeats && *info == 0; ++i)
    {
        ...
        else if(interfacetype == LAPACK_CPP_TEST)
        {
            ...
            /* Call CPP geqrf API */
            invoke_cpp_geqrf(datatype, &m_A, &n_A, A, &lda, T_test, work, &lwork, info);
