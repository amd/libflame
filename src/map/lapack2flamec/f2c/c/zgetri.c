/* ../netlib/zgetri.f -- translated by f2c (version 20100827). You must link the resulting object
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/zgetri.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/zgetri.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/zgetri.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
{
#if FLA_ENABLE_ILP64
    aocl_lapack_zgetri(n, a, lda, ipiv, work, lwork, info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_zgetri(&n_64, a, &lda_64, ipiv, work, &lwork_64, &info_64);

    *info = (aocl_int_t)info_64;
#endif
}

void aocl_lapack_zgetri(aocl_int64_t *n, dcomplex *a, aocl_int64_t *lda, aocl_int_t *ipiv,
                        dcomplex *work, aocl_int64_t *lwork, aocl_int64_t *info)
{
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("ZGETRI", &i__1, (ftnlen)6);
    ...
    for(j = *n - 1; j >= 1; --j)
    {
        ...
        if(jp != j)
        {
            aocl_blas_zswap(n, &a[j * a_dim1 + 1], &c__1, &a[jp * a_dim1 + 1], &c__1);
        }
        /* L60: */
    }
