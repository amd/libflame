...
/* ../netlib/cgbrfsx.f -- translated by f2c (version 20160102). You must link the resulting object
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/cgbrfsx
 * .f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/cgbrfsx
 * .f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/cgbrfsx
 * .f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
              ...
              integer *nparams, real *params, complex *work, real *rwork, integer *info)
{
...
#if FLA_ENABLE_ILP64
    snprintf(buffer, 256,
             "cgbrfsx inputs: trans %c, equed %c, n %lld, kl %lld, ku %lld, nrhs %lld, ldab %lld, "
             "ldafb %lld, ldb %lld, ldx %lld, n_err_bnds__ %lld, nparams %lld",
             *trans, *equed, *n, *kl, *ku, *nrhs, *ldab, *ldafb, *ldb, *ldx, *n_err_bnds__,
             *nparams);
#else
    snprintf(buffer, 256,
             "cgbrfsx inputs: trans %c, equed %c n %d, kl %d, ku %d, nrhs %d, ldab %d, ldafb %d "
             "ldb %d, ldx %d, n_err_bnds__ %d, nparams %d",
             *trans, *equed, *n, *kl, *ku, *nrhs, *ldab, *ldafb, *ldb, *ldx, *n_err_bnds__,
             *nparams);
#endif
    ...
    *rcond = 0.f;
    ...
    if(ref_type__ != 0 && *info == 0)
    {
        ...
        if(notran)
        {
            cla_gbrfsx_extended_(
                &prec_type__, &trans_type__, n, kl, ku, nrhs, &ab[ab_offset], ldab,
                &afb[afb_offset], ldafb, &ipiv[1], &colequ, &c__[1], &b[b_offset], ldb,
                &x[x_offset], ldx, &berr[1], &n_norms__, &err_bnds_norm__[err_bnds_norm_offset],
        ...
        else
        {
            cla_gbrfsx_extended_(
                &prec_type__, &trans_type__, n, kl, ku, nrhs, &ab[ab_offset], ldab,
                &afb[afb_offset], ldafb, &ipiv[1], &rowequ, &r__[1], &b[b_offset], ldb,
                &x[x_offset], ldx, &berr[1], &n_norms__, &err_bnds_norm__[err_bnds_norm_offset],
                &err_bnds_comp__[err_bnds_comp_offset], &work[1], &rwork[1], &work[*n + 1],
                &rwork[1], rcond, &ithresh, &rthresh, &unstable_thresh__, &ignore_cwise__, info);
        }
    ...
    if(*n_err_bnds__ >= 1 && n_norms__ >= 1)
    {
        ...
        {
            rcond_tmp__
                = cla_gbrcond_c_(trans, n, kl, ku, &ab[ab_offset], ldab, &afb[afb_offset], ldafb,
    ...
    if(*n_err_bnds__ >= 1 && n_norms__ >= 2)
    {
        ...
        for(j = 1; j <= i__1; ++j)
        {
            ...
            {
                rcond_tmp__ = cla_gbrcond_x_(trans, n, kl, ku, &ab[ab_offset], ldab,
                                             &afb[afb_offset], ldafb, &ipiv[1], &x[j * x_dim1 + 1],
                                             info, &work[1], &rwork[1]);
            }
            else
            {
