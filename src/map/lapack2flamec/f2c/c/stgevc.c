...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/stgevc.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/stgevc.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/stgevc.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
/* > scomplex eigenvalue, the corresponding scomplex eigenvector */
...
void stgevc_(char *side, char *howmny, logical *select, aocl_int_t *n, real *s, aocl_int_t *lds,
             real *p, aocl_int_t *ldp, real *vl, aocl_int_t *ldvl, real *vr, aocl_int_t *ldvr,
             aocl_int_t *mm, aocl_int_t *m, real *work, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_stgevc(side, howmny, select, n, s, lds, p, ldp, vl, ldvl, vr, ldvr, mm, m, work,
                       info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_stgevc(side, howmny, select, &n_64, s, &lds_64, p, &ldp_64, vl, &ldvl_64, vr,
...
void aocl_lapack_stgevc(char *side, char *howmny, logical *select, aocl_int64_t *n, real *s,
                        aocl_int64_t *lds, real *p, aocl_int64_t *ldp, real *vl, aocl_int64_t *ldvl,
                        real *vr, aocl_int64_t *ldvr, aocl_int64_t *mm, aocl_int64_t *m, real *work,
                        aocl_int64_t *info)
{
    AOCL_DTL_TRACE_LOG_INIT
    AOCL_DTL_SNPRINTF("stgevc inputs: side %c, howmny %c, n %" FLA_IS ", lds %" FLA_IS
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("STGEVC", &i__1, (ftnlen)6);
    ...
    for(j = 1; j <= i__1; ++j)
    {
        if(s[j + 1 + j * s_dim1] != 0.f)
        {
            if(p[j + j * p_dim1] == 0.f || p[j + 1 + (j + 1) * p_dim1] == 0.f
               || p[j + (j + 1) * p_dim1] != 0.f)
            {
                ilbbad = TRUE_;
            }
    ...
    if(compl )
    {
        ...
        for(je = 1; je <= i__1; ++je)
        {
            ...
            else
            {
                /* Complex eigenvalue */
                r__1 = safmin * 100.f;
                aocl_lapack_slag2(&s[je + je * s_dim1], lds, &p[je + je * p_dim1], ldp, &r__1,
                ...
                else
                {
                    ...
                    work[(*n << 1) + je] = (bcoefr * p[je + 1 + (je + 1) * p_dim1]
                                            - acoef * s[je + 1 + (je + 1) * s_dim1])
                                           / temp;
            ...
            for(j = je + nw; j <= i__2; ++j)
            {
                ...
                {
                    /* Computing MAX */
                    r__1 = temp, r__2 = work[j + 1], r__1 = fla_max(r__1, r__2),
                    r__2 = work[*n + j + 1];
                    r__1 = fla_max(r__1, r__2);
                ...
                for(jw = 1; jw <= i__3; ++jw)
                {
                    ...
                    for(ja = 1; ja <= i__4; ++ja)
                    {
                        ...
                        {
                            sums[ja + (jw << 1) - 3]
                                += s[jr + (j + ja - 1) * s_dim1] * work[(jw + 1) * *n + jr];
                            sump[ja + (jw << 1) - 3]
                                += p[jr + (j + ja - 1) * p_dim1] * work[(jw + 1) * *n + jr];
                            /* L100: */
                        }
                        /* L110: */
                    }
                ...
                for(ja = 1; ja <= i__3; ++ja)
                {
                    ...
                    {
                        sum[ja - 1]
                            = -acoef * sums[ja - 1] + bcoefr * sump[ja - 1] - bcoefi * sump[ja + 1];
                        sum[ja + 1]
                            = -acoef * sums[ja + 1] + bcoefr * sump[ja + 1] + bcoefi * sump[ja - 1];
                    }
                    else
            ...
            if(ilback)
            {
                ...
                {
                    i__3 = *n + 1 - je;
                    aocl_blas_sgemv("N", n, &i__3, &c_b34, &vl[je * vl_dim1 + 1], ldvl,
                                    &work[(jw + 2) * *n + je], &c__1, &c_b36,
                                    &work[(jw + 4) * *n + 1], &c__1);
                    /* L170: */
                }
                aocl_lapack_slacpy(" ", n, &nw, &work[(*n << 2) + 1], n, &vl[je * vl_dim1 + 1],
                                   ldvl);
                ibeg = 1;
            }
            else
            {
                aocl_lapack_slacpy(" ", n, &nw, &work[(*n << 1) + 1], n, &vl[ieig * vl_dim1 + 1],
                                   ldvl);
                ibeg = je;
            }
            /* Scale eigenvector */
    ...
    if(compr)
    {
        ...
        for(je = *n; je >= 1; --je)
        {
            ...
            if(!ilcplx)
            {
                ...
                {
                    work[(*n << 1) + jr]
                        = bcoefr * p[jr + je * p_dim1] - acoef * s[jr + je * s_dim1];
                    /* L260: */
                }
            }
            else
            {
                /* Complex eigenvalue */
                r__1 = safmin * 100.f;
                aocl_lapack_slag2(&s[je - 1 + (je - 1) * s_dim1], lds,
                                  &p[je - 1 + (je - 1) * p_dim1], ldp, &r__1, &acoef, &temp,
                                  &bcoefr, &temp2, &bcoefi);
                if(bcoefi == 0.f)
                {
                    *info = je - 1;
                ...
                {
                    work[(*n << 1) + jr]
                        = -creala * s[jr + (je - 1) * s_dim1] + crealb * p[jr + (je - 1) * p_dim1]
                          - cre2a * s[jr + je * s_dim1] + cre2b * p[jr + je * p_dim1];
                    work[*n * 3 + jr] = -cimaga * s[jr + (je - 1) * s_dim1]
                                        + cimagb * p[jr + (je - 1) * p_dim1]
                                        - cim2a * s[jr + je * s_dim1] + cim2b * p[jr + je * p_dim1];
                    /* L270: */
                }
            }
            /* Computing MAX */
            ...
            for(j = je - nw; j >= 1; --j)
            {
                ...
                aocl_lapack_slaln2(&c_false, &na, &nw, &dmin__, &acoef, &s[j + j * s_dim1], lds,
                        ...
                        if(ilcplx)
                        {
                            ...
                            for(jr = 1; jr <= i__2; ++jr)
                            {
                                work[(*n << 1) + jr] = work[(*n << 1) + jr]
                                                       ...
                                                       + crealb * p[jr + (j + ja - 1) * p_dim1];
                                work[*n * 3 + jr] = work[*n * 3 + jr]
                                                    - cimaga * s[jr + (j + ja - 1) * s_dim1]
                                                    + cimagb * p[jr + (j + ja - 1) * p_dim1];
                                /* L340: */
                            }
                        }
                        else
            ...
            if(ilback)
            {
                ...
                for(jw = 0; jw <= i__1; ++jw)
                {
                    ...
                    for(jc = 2; jc <= i__2; ++jc)
                    {
                        ...
                        {
                            work[(jw + 4) * *n + jr]
                                += work[(jw + 2) * *n + jc] * vr[jr + jc * vr_dim1];
                            /* L390: */
                        }
                        /* L400: */
                    }
