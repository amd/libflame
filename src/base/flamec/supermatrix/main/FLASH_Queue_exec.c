...
----------------------------------------------------------------------------*/
{
   fla_dim_t          n_tasks    = FLASH_Queue_get_num_tasks();
   fla_dim_t          n_threads  = FLASH_Queue_get_num_threads();
   fla_dim_t          n_queues;
   fla_dim_t          n_caches;
   fla_dim_t          size;
   fla_dim_t          i;
...
----------------------------------------------------------------------------*/
{
   ...
   fla_dim_t            i, j, k;
   fla_dim_t            n_tasks    = FLASH_Queue_get_num_tasks();
   fla_dim_t            n_queues   = args->n_queues;
   fla_dim_t            n_prefetch = 0;
   fla_dim_t            n_ready    = 0;
   fla_dim_t            length     = 0;
   fla_dim_t            width      = 0;
   fla_dim_t            height     = 0;
   fla_dim_t            size       = args->size;
...
FLASH_Task* FLASH_Queue_wait_dequeue( fla_dim_t queue, fla_dim_t cache, void* arg )
/*----------------------------------------------------------------------------
...
FLASH_Task* FLASH_Queue_wait_dequeue_block( fla_dim_t queue, fla_dim_t cache, void* arg )
/*----------------------------------------------------------------------------
...
----------------------------------------------------------------------------*/
{
   ...
   fla_dim_t         i, j, k;
   fla_dim_t         size    = args->size;
   fla_dim_t         n_tasks = args->wait_queue[queue].n_tasks;
   FLA_Bool    enabled = FALSE;
   FLASH_Task* t;
   FLA_Obj     obj;
   FLA_Obj     mem;
...
----------------------------------------------------------------------------*/
{
   FLASH_Queue_vars* args = ( FLASH_Queue_vars* ) arg;
   fla_dim_t i, j, k;
   fla_dim_t n_caches = args->n_caches;
   fla_dim_t size     = args->size;
...
void FLASH_Queue_prefetch( fla_dim_t cache, void* arg )
/*----------------------------------------------------------------------------
...
{
   fla_dim_t          i, inc;
   fla_dim_t          line_size = FLASH_Queue_get_cache_line_size();
   fla_dim_t          elem_size = FLA_Obj_elem_size( obj );
   fla_dim_t          length    = FLA_Obj_length( obj );
   fla_dim_t          width     = FLA_Obj_width( obj );
   ...
   switch ( datatype )
   {
      ...
      case FLA_INT:
      {
         fla_dim_t *buffer = ( fla_dim_t * ) FLA_INT_PTR( obj );
         fla_dim_t access;
...
FLASH_Task* FLASH_Queue_work_stealing( fla_dim_t queue, void *arg )
/*----------------------------------------------------------------------------
...
void FLASH_Queue_create_gpu( fla_dim_t thread, void *arg )
/*----------------------------------------------------------------------------
...
void FLASH_Queue_destroy_gpu( fla_dim_t thread, void *arg )
/*----------------------------------------------------------------------------
...
----------------------------------------------------------------------------*/
{
   ...
   {
      fla_dim_t i, j, k;
      fla_dim_t thread        = t->thread;
      fla_dim_t n_input_args  = t->n_input_args;
      fla_dim_t n_output_args = t->n_output_args;
      fla_dim_t n_threads     = FLASH_Queue_get_num_threads();
      FLA_Bool duplicate;
      FLA_Obj  obj;
...
FLA_Bool FLASH_Queue_check_block_gpu( FLA_Obj obj, fla_dim_t thread, void *arg )
/*----------------------------------------------------------------------------
...
void FLASH_Queue_invalidate_block_gpu( FLA_Obj obj, fla_dim_t thread, void *arg )
/*----------------------------------------------------------------------------
...
void FLASH_Queue_flush_block_gpu( FLA_Obj obj, fla_dim_t thread, void *arg )
/*----------------------------------------------------------------------------
...
void FLASH_Queue_flush_gpu( fla_dim_t thread, void *arg )
/*----------------------------------------------------------------------------
...
----------------------------------------------------------------------------*/
{
   ...
   for ( i = 1; i < n_threads; i++ )
   {
      ...
      // an fla_dim_t. Declaring it as a long or void* appears to force the
...
----------------------------------------------------------------------------*/
{
   ...
   fla_dim_t           n_tasks   = FLASH_Queue_get_num_tasks();
   fla_dim_t           n_threads = FLASH_Queue_get_num_threads();
   fla_dim_t           n_cores   = FLASH_Queue_get_cores_per_cache();
...
fla_dim_t RCCE_acquire_lock(fla_dim_t);
fla_dim_t RCCE_release_lock(fla_dim_t);
...
----------------------------------------------------------------------------*/
{
   ...
   args.n_ready = ( fla_dim_t* ) FLA_shmalloc( n_tasks * sizeof( fla_dim_t ) );
   args.wait_queue = ( fla_dim_t* ) FLA_shmalloc( n_tasks * sizeof( fla_dim_t ) );
   args.n_wait = ( fla_dim_t* ) FLA_shmalloc( sizeof( fla_dim_t ) );
   args.pc = ( fla_dim_t* ) FLA_shmalloc( sizeof( fla_dim_t ) );
...
{  
   FLASH_Queue_vars* args = ( FLASH_Queue_vars* ) arg;
   fla_dim_t i = args->n_wait[0] + args->pc[0];
