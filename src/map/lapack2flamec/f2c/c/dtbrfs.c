/* ../netlib/dtbrfs.f -- translated by f2c (version 20100827). You must link the resulting object
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/dtbrfs.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/dtbrfs.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/dtbrfs.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
void dtbrfs_(char *uplo, char *trans, char *diag, aocl_int_t *n, aocl_int_t *kd, aocl_int_t *nrhs,
             doublereal *ab, aocl_int_t *ldab, doublereal *b, aocl_int_t *ldb, doublereal *x,
             aocl_int_t *ldx, doublereal *ferr, doublereal *berr, doublereal *work,
             aocl_int_t *iwork, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_dtbrfs(uplo, trans, diag, n, kd, nrhs, ab, ldab, b, ldb, x, ldx, ferr, berr, work,
                       iwork, info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_dtbrfs(uplo, trans, diag, &n_64, &kd_64, &nrhs_64, ab, &ldab_64, b, &ldb_64, x,
...
void aocl_lapack_dtbrfs(char *uplo, char *trans, char *diag, aocl_int64_t *n, aocl_int64_t *kd,
                        aocl_int64_t *nrhs, doublereal *ab, aocl_int64_t *ldab, doublereal *b,
                        ...
                        doublereal *berr, doublereal *work, aocl_int_t *iwork, aocl_int64_t *info)
{
    ...
    AOCL_DTL_SNPRINTF("dtbrfs inputs: uplo %c, trans %c, diag %c, n %" FLA_IS ", kd %" FLA_IS
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("DTBRFS", &i__1, (ftnlen)6);
    ...
    for(j = 1; j <= i__1; ++j)
    {
        ...
        aocl_blas_dcopy(n, &x[j * x_dim1 + 1], &c__1, &work[*n + 1], &c__1);
        aocl_blas_dtbmv(uplo, trans, diag, n, kd, &ab[ab_offset], ldab, &work[*n + 1], &c__1);
        aocl_blas_daxpy(n, &c_b19, &b[j * b_dim1 + 1], &c__1, &work[*n + 1], &c__1);
            ...
            if(upper)
            {
                if(nounit)
                {
                    ...
                    for(k = 1; k <= i__2; ++k)
                    {
                        ...
                        {
                            work[i__]
                                += (d__1 = ab[*kd + 1 + i__ - k + k * ab_dim1], f2c_dabs(d__1))
                                   * xk;
                            /* L30: */
                        }
                        /* L40: */
            ...
            else
            {
                if(nounit)
                {
                    ...
                    for(k = 1; k <= i__2; ++k)
                    {
                        ...
                        {
                            work[i__]
                                += (d__1 = ab[i__ + 1 - k + k * ab_dim1], f2c_dabs(d__1)) * xk;
                            /* L70: */
                        }
                        /* L80: */
                    }
            ...
            if(upper)
            {
                if(nounit)
                {
                    ...
                    for(k = 1; k <= i__2; ++k)
                    {
                        ...
                        {
                            s += (d__1 = ab[*kd + 1 + i__ - k + k * ab_dim1], f2c_dabs(d__1))
            ...
            else
            {
                if(nounit)
                {
                    ...
                    for(k = 1; k <= i__2; ++k)
                    {
                        ...
                        {
                            s += (d__1 = ab[i__ + 1 - k + k * ab_dim1], f2c_dabs(d__1))
                                 * (d__2 = x[i__ + j * x_dim1], f2c_dabs(d__2));
                            /* L150: */
                        }
                        work[k] += s;
    ...
    L210:
        ...
        if(kase != 0)
        {
            if(kase == 1)
            {
                ...
                aocl_blas_dtbsv(uplo, transt, diag, n, kd, &ab[ab_offset], ldab, &work[*n + 1],
                                &c__1);
                i__2 = *n;
                for(i__ = 1; i__ <= i__2; ++i__)
                {
            ...
            else
            {
                ...
                aocl_blas_dtbsv(uplo, trans, diag, n, kd, &ab[ab_offset], ldab, &work[*n + 1],
                                &c__1);
            }
            goto L210;
        }
