...
/* ../netlib/cgebrd.f -- translated by f2c (version 20160102). You must link the resulting object
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/cgebrd.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/cgebrd.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/cgebrd.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
void cgebrd_(aocl_int_t *m, aocl_int_t *n, scomplex *a, aocl_int_t *lda, real *d__, real *e,
             scomplex *tauq, scomplex *taup, scomplex *work, aocl_int_t *lwork, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_cgebrd(m, n, a, lda, d__, e, tauq, taup, work, lwork, info);
#else
    aocl_int64_t m_64 = *m;
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_cgebrd(&m_64, &n_64, a, &lda_64, d__, e, tauq, taup, work, &lwork_64, &info_64);

    *info = (aocl_int_t)info_64;
#endif
}

void aocl_lapack_cgebrd(aocl_int64_t *m, aocl_int64_t *n, scomplex *a, aocl_int64_t *lda, real *d__,
                        real *e, scomplex *tauq, scomplex *taup, scomplex *work, aocl_int64_t *lwork,
                        aocl_int64_t *info)
{
    ...
    char buffer[256];
#if FLA_ENABLE_ILP64
    snprintf(buffer, 256, "cgebrd inputs: m %lld, n %lld, lda %lld, lwork %lld", *m, *n, *lda,
             *lwork);
#else
    ...
    *info = 0;
    ...
    if(nb > 1 && nb < minmn)
    {
        ...
        /* Computing MAX */
        i__1 = nb;
        i__2 = aocl_lapack_ilaenv(&c__3, "CGEBRD", " ", m, n, &c_n1, &c_n1); // , expr subst
        ...
        if(nx < minmn)
        {
            ...
            if(*lwork < ws)
            {
                ...
                nbmin = aocl_lapack_ilaenv(&c__2, "CGEBRD", " ", m, n, &c_n1, &c_n1);
                if(*lwork >= (*m + *n) * nbmin)
                {
                    nb = *lwork / (*m + *n);
                }
    ...
    for(i__ = 1; i__2 < 0 ? i__ >= i__1 : i__ <= i__1; i__ += i__2)
    {
        ...
        i__3 = *m - i__ + 1;
        i__4 = *n - i__ + 1;
        aocl_lapack_clabrd(&i__3, &i__4, &nb, &a[i__ + i__ * a_dim1], lda, &d__[i__], &e[i__],
    ...
    r__1 = aocl_lapack_sroundup_lwork(&ws);
    work[1].real = r__1;
    work[1].imag = 0.f; // , expr subst
    AOCL_DTL_TRACE_EXIT(AOCL_DTL_LEVEL_TRACE_5);
    return;
