/* ../netlib/v3.9.0/chetrs_aa_2stage.f -- translated by f2c (version 20160102). You must link the
...
void chetrs_aa_2stage_(char *uplo, aocl_int_t *n, aocl_int_t *nrhs, scomplex *a, aocl_int_t *lda,
                       scomplex *tb, aocl_int_t *ltb, aocl_int_t *ipiv, aocl_int_t *ipiv2,
                       scomplex *b, aocl_int_t *ldb, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_chetrs_aa_2stage(uplo, n, nrhs, a, lda, tb, ltb, ipiv, ipiv2, b, ldb, info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_chetrs_aa_2stage(uplo, &n_64, &nrhs_64, a, &lda_64, tb, &ltb_64, ipiv, ipiv2, b,
                                 &ldb_64, &info_64);
...
void aocl_lapack_chetrs_aa_2stage(char *uplo, aocl_int64_t *n, aocl_int64_t *nrhs, scomplex *a,
                                  ...
                                  aocl_int64_t *ldb, aocl_int64_t *info)
{
...
#if FLA_ENABLE_ILP64
    snprintf(buffer, 256,
             "chetrs_aa_2stage inputs: uplo %c, n %lld, nrhs %lld, lda %lld, ltb %lld, ldb %lld",
             *uplo, *n, *nrhs, *lda, *ltb, *ldb);
#else
    snprintf(buffer, 256, "chetrs_aa_2stage inputs: uplo %c, n %d, nrhs %d, lda %d, ltb %d, ldb %d",
             *uplo, *n, *nrhs, *lda, *ltb, *ldb);
#endif
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("CHETRS_AA_2STAGE", &i__1, (ftnlen)16);
    ...
    if(upper)
    {
        ...
        if(*n > nb)
        {
            ...
            i__1 = *n - nb;
            aocl_blas_ctrsm("L", "U", "C", "U", &i__1, nrhs, &c_b1, &a[(nb + 1) * a_dim1 + 1], lda,
                            &b[nb + 1 + b_dim1], ldb);
        }
    ...
    else
    {
        ...
        if(*n > nb)
        {
            ...
            i__1 = *n - nb;
            aocl_blas_ctrsm("L", "L", "C", "U", &i__1, nrhs, &c_b1, &a[nb + 1 + a_dim1], lda,
                            &b[nb + 1 + b_dim1], ldb);
