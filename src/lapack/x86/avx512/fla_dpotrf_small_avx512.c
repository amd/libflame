...
int fla_dpotrf_small_avx512(char *uplo, aocl_int64_t *n, doublereal *a, aocl_int64_t *lda, aocl_int64_t *info)
{
    *info = 0;
    ...
    aocl_int64_t i = 1, i_1 = 1, j = 1, ni = 1, k = 1;
    doublereal *acur, p_val = 1;
    __m128d acur_i1_vec_128, acur_j_vec_128, acur_update_vec_128;
    __m256d p_val_vec_256, acur_vec_256, acur_i1_vec_256, acur_j_vec_256, acur_update_vec_256;
    __m512d acur_i1_vec_512, acur_j_vec_512, acur_update_vec_512;
    if(*n == 1)
    {
        /*
        ...
        for(i = 0; i < *n; i++)
        {
            ...
            *acur = sqrt(*acur);
            ...
            for(i_1 = 1; i_1 < ni; i_1++)
            {
                ...
                {
                    /* Broadcast acur[i_1] */
                    acur_i1_vec_512 = _mm512_broadcastsd_pd(_mm_load_sd(&acur[i_1]));
                    for(j = i_1; j < (ni - 7); j += 8)
                    {
                        /* Load 8 elements of acur[j] */
                        acur_j_vec_512 = _mm512_loadu_pd(&acur[j]);
                        /* Load 8 elements of acur[i_1 + (j * *lda)] */
                        acur_update_vec_512 = _mm512_loadu_pd(&acur[j + i_1xlda]);
                        /* Perform fused multiply-subtract */
                        acur_update_vec_512 = _mm512_fnmadd_pd(acur_j_vec_512, acur_i1_vec_512,
                                                               acur_update_vec_512);
                        /* Store the result back */
                        _mm512_storeu_pd(&acur[j + i_1xlda], acur_update_vec_512);
                    }
                }
                /* Broadcast acur[i_1] */
                ...
                /* Handle remaining elements (if any) with scalar  */
                /* Broadcast acur[i_1] */
                acur_i1_vec_128 = _mm_load1_pd(&acur[i_1]);
                for(; j < (ni - 1); j += 2)
                {
                    /* Load 2 elements of acur[j] */
                    acur_j_vec_128 = _mm_loadu_pd(&acur[j]);
                    /* Load 2 elements of acur[i_1 + (j * *lda)] */
                    acur_update_vec_128 = _mm_loadu_pd(&acur[j + i_1xlda]);
                    /* Perform fused multiply-subtract */
                    acur_update_vec_128
                        = _mm_fnmadd_pd(acur_j_vec_128, acur_i1_vec_128, acur_update_vec_128);
                    /* Store the result back */
                    _mm_storeu_pd(&acur[j + i_1xlda], acur_update_vec_128);
                }
                for(; j < ni; j++)
                {
        ...
        for(i = 0; i < *n; i++)
        {
            ...
            *acur = sqrt(*acur);
            ...
            for(i_1 = 1; i_1 < ni; i_1++)
            {
                ...
                {
                    acur[j + i_1xlda]
                        = acur[j + i_1xlda] - acur[j * *lda] * acur[i_1xlda];
                }
            }
        }
        return 0;
