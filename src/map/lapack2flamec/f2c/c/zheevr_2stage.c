...
static aocl_int64_t c__10 = 10;
static aocl_int64_t c__1 = 1;
static aocl_int64_t c__2 = 2;
static aocl_int64_t c__3 = 3;
static aocl_int64_t c__4 = 4;
static aocl_int64_t c_n1 = -1;
/* > \brief <b> ZHEEVR_2STAGE computes the eigenvalues and, optionally, the left and/or right
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/zheevr_
 * 2stage.f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/zheevr_
 * 2stage.f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/zheevr_
 * 2stage.f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
/* > of a scomplex Hermitian matrix A using the 2stage technique for */
...
void zheevr_2stage_(char *jobz, char *range, char *uplo, aocl_int_t *n, dcomplex *a,
                    aocl_int_t *lda, doublereal *vl, doublereal *vu, aocl_int_t *il, aocl_int_t *iu,
                    doublereal *abstol, aocl_int_t *m, doublereal *w, dcomplex *z__,
                    aocl_int_t *ldz, aocl_int_t *isuppz, dcomplex *work, aocl_int_t *lwork,
                    doublereal *rwork, aocl_int_t *lrwork, aocl_int_t *iwork, aocl_int_t *liwork,
                    aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_zheevr_2stage(jobz, range, uplo, n, a, lda, vl, vu, il, iu, abstol, m, w, z__, ldz,
    ...
    aocl_lapack_zheevr_2stage(jobz, range, uplo, &n_64, a, &lda_64, vl, vu, &il_64, &iu_64, abstol,
                              &m_64, w, z__, &ldz_64, isuppz, work, &lwork_64, rwork, &lrwork_64,
...
void aocl_lapack_zheevr_2stage(char *jobz, char *range, char *uplo, aocl_int64_t *n,
                               dcomplex *a, aocl_int64_t *lda, doublereal *vl, doublereal *vu,
                               aocl_int64_t *il, aocl_int64_t *iu, doublereal *abstol,
                               aocl_int64_t *m, doublereal *w, dcomplex *z__,
                               aocl_int64_t *ldz, aocl_int_t *isuppz, dcomplex *work,
                               aocl_int64_t *lwork, doublereal *rwork, aocl_int64_t *lrwork,
                               aocl_int_t *iwork, aocl_int64_t *liwork, aocl_int64_t *info)
{
    AOCL_DTL_TRACE_LOG_INIT
    AOCL_DTL_SNPRINTF("zheevr_2stage inputs: jobz %c, range %c, uplo %c, n %" FLA_IS
                      ", lda %" FLA_IS ", il %" FLA_IS ", iu %" FLA_IS
                      ", vl %lf, vu %lf, abstol %lf, ldz %" FLA_IS ", lwork %" FLA_IS
    ...
    logical test;
    aocl_int64_t itmp1;
    aocl_int64_t indrd, indre;
    doublereal sigma;
    extern logical lsame_(char *, char *, aocl_int64_t, aocl_int64_t);
    aocl_int64_t iinfo;
    char order[1];
    aocl_int64_t indwk, lhtrd;
    aocl_int64_t lwmin;
    logical lower;
    aocl_int64_t lwtrd;
    logical wantz;
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("ZHEEVR_2STAGE", &i__1, (ftnlen)13);
    ...
    *m = 0;
    ...
    anrm = aocl_lapack_zlansy("M", uplo, n, &a[a_offset], lda, &rwork[1]);
    if(anrm > 0. && anrm < rmin)
    {
        iscale = 1;
        sigma = rmin / anrm;
    ...
    if(iscale == 1)
    {
        if(lower)
        {
            ...
            {
                i__2 = *n - j + 1;
                aocl_blas_zdscal(&i__2, &sigma, &a[j + j * a_dim1], &c__1);
                /* L10: */
            }
        }
        else
        {
            ...
            for(j = 1; j <= i__1; ++j)
            {
                aocl_blas_zdscal(&j, &sigma, &a[j * a_dim1 + 1], &c__1);
                /* L20: */
            }
        }
        if(*abstol > 0.)
    ...
    /* WORK(INDTAU:INDTAU+N-1) stores the scomplex scalar factors of the */
    /* elementary reflectors used in ZHETRD. */
    indtau = 1;
    /* INDWK is the starting offset of the remaining scomplex workspace, */
    /* and LLWORK is the remaining scomplex workspace size. */
    indhous = indtau + *n;
    indwk = indhous + lhtrd;
    ...
    aocl_lapack_zhetrd_2stage(jobz, uplo, n, &a[a_offset], lda, &rwork[indrd], &rwork[indre],
                              &work[indtau], &work[indhous], &lhtrd, &work[indwk], &llwork, &iinfo);
    ...
    if((alleig || test) && ieeeok == 1)
    {
        if(!wantz)
        {
            aocl_blas_dcopy(n, &rwork[indrd], &c__1, &w[1], &c__1);
            i__1 = *n - 1;
            aocl_blas_dcopy(&i__1, &rwork[indre], &c__1, &rwork[indree], &c__1);
            aocl_lapack_dsterf(n, &w[1], &rwork[indree], info);
        }
        else
        {
            i__1 = *n - 1;
            aocl_blas_dcopy(&i__1, &rwork[indre], &c__1, &rwork[indree], &c__1);
            aocl_blas_dcopy(n, &rwork[indrd], &c__1, &rwork[indrdd], &c__1);
            if(*abstol <= *n * 2. * eps)
            {
                tryrac = TRUE_;
            }
            ...
            aocl_lapack_zstemr(jobz, "A", n, &rwork[indrdd], &rwork[indree], vl, vu, il, iu, m,
                               &w[1], &z__[z_offset], ldz, n, &isuppz[1], &tryrac, &rwork[indrwk],
    ...
    aocl_lapack_dstebz(range, order, n, &vll, &vuu, il, iu, &abstll, &rwork[indrd], &rwork[indre],
                       m, &nsplit, &w[1], &iwork[indibl], &iwork[indisp], &rwork[indrwk],
                       &iwork[indiwo], info);
    if(wantz)
    {
        aocl_lapack_zstein(n, &rwork[indrd], &rwork[indre], m, &w[1], &iwork[indibl],
                           &iwork[indisp], &z__[z_offset], ldz, &rwork[indrwk], &iwork[indiwo],
                           &iwork[indifl], info);
        /* Apply unitary matrix used in reduction to tridiagonal */
        /* form to eigenvectors returned by ZSTEIN. */
        indwkn = indwk;
        llwrkn = *lwork - indwkn + 1;
        aocl_lapack_zunmtr("L", uplo, "N", n, m, &a[a_offset], lda, &work[indtau], &z__[z_offset],
