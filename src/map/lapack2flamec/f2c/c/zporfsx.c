...
/* ../netlib/zporfsx.f -- translated by f2c (version 20160102). You must link the resulting object
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/zporfsx
 * .f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/zporfsx
 * .f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/zporfsx
 * .f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
              ...
              integer *info)
{
    ...
    AOCL_DTL_SNPRINTF("zporfsx inputs: uplo %c, equed %c, n %" FLA_IS ", nrhs %" FLA_IS
    ...
    *(unsigned char *)norm = 'I';
    ...
    if(ref_type__ != 0)
    {
        ...
        zla_porfsx_extended_(&prec_type__, uplo, n, nrhs, &a[a_offset], lda, &af[af_offset], ldaf,
    ...
    if(*n_err_bnds__ >= 1 && n_norms__ >= 1)
    {
        ...
        if(rcequ)
        {
            rcond_tmp__ = zla_porcond_c_(uplo, n, &a[a_offset], lda, &af[af_offset], ldaf, &s[1],
    ...
    if(*n_err_bnds__ >= 1 && n_norms__ >= 2)
    {
        ...
        for(j = 1; j <= i__1; ++j)
        {
            ...
            {
                rcond_tmp__ = zla_porcond_x_(uplo, n, &a[a_offset], lda, &af[af_offset], ldaf,
