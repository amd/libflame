/* ../netlib/v3.9.0/dgeqr.f -- translated by f2c (version 20160102). You must link the resulting
...
void dgeqr_(aocl_int_t *m, aocl_int_t *n, doublereal *a, aocl_int_t *lda, doublereal *t,
            aocl_int_t *tsize, doublereal *work, aocl_int_t *lwork, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_dgeqr(m, n, a, lda, t, tsize, work, lwork, info);
#else
    aocl_int64_t m_64 = *m;
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_dgeqr(&m_64, &n_64, a, &lda_64, t, &tsize_64, work, &lwork_64, &info_64);

    *info = (aocl_int_t)info_64;
#endif
}

void aocl_lapack_dgeqr(aocl_int64_t *m, aocl_int64_t *n, doublereal *a, aocl_int64_t *lda,
                       doublereal *t, aocl_int64_t *tsize, doublereal *work, aocl_int64_t *lwork,
                       aocl_int64_t *info)
{
    AOCL_DTL_TRACE_LOG_INIT
    AOCL_DTL_SNPRINTF("dgeqr inputs: m %" FLA_IS ", n %" FLA_IS ", lda %" FLA_IS ", tsize %" FLA_IS
                      ", lwork %" FLA_IS "",
                      *m, *n, *lda, *tsize, *lwork);
    ...
    *info = 0;
    ...
    if(fla_min(*m, *n) > 0)
    {
        mb = aocl_lapack_ilaenv(&c__1, "DGEQR ", " ", m, n, &c__1, &c_n1);
        nb = aocl_lapack_ilaenv(&c__1, "DGEQR ", " ", m, n, &c__2, &c_n1);
    }
    else
    {
        mb = *m;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("DGEQR", &i__1, (ftnlen)5);
    ...
    if(*m <= *n || mb <= *n || mb >= *m)
    {
        aocl_lapack_dgeqrt(m, n, &nb, &a[a_offset], lda, &t[6], &nb, &work[1], info);
    }
    else
    {
        aocl_lapack_dlatsqr(m, n, &mb, &nb, &a[a_offset], lda, &t[6], &nb, &work[1], lwork, info);
    }
    /* Computing MAX */
    i__1 = 1;
    i__2 = nb * *n; // , expr subst
