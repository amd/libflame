...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/zgeqrfp
 * .f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/zgeqrfp
 * .f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/zgeqrfp
 * .f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
/* > \verbatim */
/* > */
/* > ZGEQRFP computes a QR factorization of a scomplex M-by-N matrix A: */
/* > A = Q * R. */
/* > \endverbatim */
/* Arguments: */
/* ========== */
...
void zgeqrfp_(aocl_int_t *m, aocl_int_t *n, dcomplex *a, aocl_int_t *lda, dcomplex *tau,
              dcomplex *work, aocl_int_t *lwork, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_zgeqrfp(m, n, a, lda, tau, work, lwork, info);
#else
    aocl_int64_t m_64 = *m;
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_zgeqrfp(&m_64, &n_64, a, &lda_64, tau, work, &lwork_64, &info_64);

    *info = (aocl_int_t)info_64;
#endif
}

void aocl_lapack_zgeqrfp(aocl_int64_t *m, aocl_int64_t *n, dcomplex *a, aocl_int64_t *lda,
                         dcomplex *tau, dcomplex *work, aocl_int64_t *lwork,
                         aocl_int64_t *info)
{
    AOCL_DTL_TRACE_LOG_INIT
    AOCL_DTL_SNPRINTF("zgeqrfp inputs: m %" FLA_IS ", n %" FLA_IS ", lda %" FLA_IS
                      ", lwork %" FLA_IS "",
                      *m, *n, *lda, *lwork);
    ...
    *info = 0;
    ...
    if(nb >= nbmin && nb < k && nx < k)
    {
        ...
        for(i__ = 1; i__2 < 0 ? i__ >= i__1 : i__ <= i__1; i__ += i__2)
        {
            ...
            /* A(i:m,i:i+ib-1) */
            i__3 = *m - i__ + 1;
            aocl_lapack_zgeqr2p(&i__3, &ib, &a[i__ + i__ * a_dim1], lda, &tau[i__], &work[1],
                                &iinfo);
            if(i__ + ib <= *n)
            {
    ...
    if(i__ <= k)
    {
        i__2 = *m - i__ + 1;
        i__1 = *n - i__ + 1;
        aocl_lapack_zgeqr2p(&i__2, &i__1, &a[i__ + i__ * a_dim1], lda, &tau[i__], &work[1], &iinfo);
    }
