...
/* > \verbatim */
/* > */
/* > ZHETRF computes the factorization of a scomplex Hermitian matrix A */
    ...
    *info = 0;
    ...
    L10: /* If K < 1, exit from loop */
        ...
        if(k > nb)
        {
            ...
            aocl_lapack_zlahef(uplo, &k, &nb, &kb, &a[a_offset], lda, &ipiv[1], &work[1], n,
                               &iinfo);
        }
        else
        {
            /* Use unblocked code to factorize columns 1:k of A */
            aocl_lapack_zhetf2(uplo, &k, &a[a_offset], lda, &ipiv[1], &iinfo);
            kb = k;
        }
    ...
    L20: /* If K > N, exit from loop */
        ...
        if(k <= *n - nb)
        {
            ...
            i__1 = *n - k + 1;
            aocl_lapack_zlahef(uplo, &i__1, &nb, &kb, &a[k + k * a_dim1], lda, &ipiv[k], &work[1],
                               n, &iinfo);
        }
        else
        {
            ...
            i__1 = *n - k + 1;
            aocl_lapack_zhetf2(uplo, &i__1, &a[k + k * a_dim1], lda, &ipiv[k], &iinfo);
            kb = *n - k + 1;
        }
        /* Set INFO on the first occurrence of a zero pivot */
        if(*info == 0 && iinfo > 0)
