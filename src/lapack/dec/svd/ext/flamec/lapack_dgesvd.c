  ...
  *info = 0;
    ...
    if (*m >= *n && minmn > 0) {
      ...
      if (*m >= mnthr) {
        if (wntun) {
          ...
          /* 'A') */
          if (*m <= FLA_SVD_SMALL_SIZE_THRESH2
              && FLA_IS_MIN_ARCH_ID(FLA_ARCH_AVX2)) {
            i__2 = *n * 3 + *m;
            maxwrk = fla_max(i__2, bdspac);
            minwrk = *n << 2;
        ...
        if(!(wntuo || wntvo) && (((wntun && wntvn)
           && (*m < FLA_SVD_SMALL_SIZE_THRESH3)))
           || ((wntuas || wntvas)
           && (*m < FLA_SVD_SMALL_SIZE_THRESH1))
           && FLA_IS_MIN_ARCH_ID(FLA_ARCH_AVX2)) {
          i__2 = *n * 3 + *m;
          maxwrk = fla_max(i__2, bdspac);
          minwrk = *n * 3;
      ...
      if (*n >= mnthr) {
        if (wntvn) {
          ...
          /* JOBVT='S') */
          if (*n <= FLA_SVD_SMALL_SIZE_THRESH2
              && FLA_IS_MIN_ARCH_ID(FLA_ARCH_AVX2)) {
            i__2 = *m * 3 + *n;
            minwrk = fla_max(i__2, bdspac);
  ...
  if (*m >= *n) {
    ...
    if (*m >= mnthr) {
      ...
      {
        if (wntua) {
          fla_dgesvd_xx_small10(wntuas, wntvas, m, n, m, &a[a_offset], lda, &s[1],
