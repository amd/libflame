...
void slals0_(aocl_int_t *icompq, aocl_int_t *nl, aocl_int_t *nr, aocl_int_t *sqre, aocl_int_t *nrhs,
             real *b, aocl_int_t *ldb, real *bx, aocl_int_t *ldbx, aocl_int_t *perm,
             ...
             real *c__, real *s, real *work, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_slals0(icompq, nl, nr, sqre, nrhs, b, ldb, bx, ldbx, perm, givptr, givcol, ldgcol,
    ...
    aocl_lapack_slals0(&icompq_64, &nl_64, &nr_64, &sqre_64, &nrhs_64, b, &ldb_64, bx, &ldbx_64,
...
void aocl_lapack_slals0(aocl_int64_t *icompq, aocl_int64_t *nl, aocl_int64_t *nr,
                        aocl_int64_t *sqre, aocl_int64_t *nrhs, real *b, aocl_int64_t *ldb,
                        real *bx, aocl_int64_t *ldbx, aocl_int_t *perm, aocl_int64_t *givptr,
                        aocl_int_t *givcol, aocl_int64_t *ldgcol, real *givnum,
                        aocl_int64_t *ldgnum, real *poles, real *difl, real *difr, real *z__,
                        aocl_int64_t *k, real *c__, real *s, real *work, aocl_int64_t *info)
{
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("SLALS0", &i__1, (ftnlen)6);
    ...
    if(*icompq == 0)
    {
        ...
        {
            aocl_blas_srot(nrhs, &b[givcol[i__ + (givcol_dim1 << 1)] + b_dim1], ldb,
                           &b[givcol[i__ + givcol_dim1] + b_dim1], ldb,
                           &givnum[i__ + (givnum_dim1 << 1)], &givnum[i__ + givnum_dim1]);
            /* L10: */
        }
        /* Step (2L): permute rows of B. */
        aocl_blas_scopy(nrhs, &b[nlp1 + b_dim1], ldb, &bx[bx_dim1 + 1], ldbx);
        i__1 = n;
        for(i__ = 2; i__ <= i__1; ++i__)
        {
            aocl_blas_scopy(nrhs, &b[perm[i__] + b_dim1], ldb, &bx[i__ + bx_dim1], ldbx);
            /* L20: */
        }
        ...
        if(*k == 1)
        {
            aocl_blas_scopy(nrhs, &bx[bx_offset], ldbx, &b[b_offset], ldb);
            if(z__[1] < 0.f)
            {
                aocl_blas_sscal(nrhs, &c_b5, &b[b_offset], ldb);
            }
        }
        else
        {
            ...
            for(j = 1; j <= i__1; ++j)
            {
                ...
                aocl_blas_sgemv("T", k, nrhs, &c_b11, &bx[bx_offset], ldbx, &work[1], &c__1, &c_b13,
                                &b[j + b_dim1], ldb);
                aocl_lapack_slascl("G", &c__0, &c__0, &temp, &c_b11, &c__1, nrhs, &b[j + b_dim1],
                                   ldb, info);
                /* L50: */
            }
        }
        ...
        {
            i__1 = n - *k;
            aocl_lapack_slacpy("A", &i__1, nrhs, &bx[*k + 1 + bx_dim1], ldbx, &b[*k + 1 + b_dim1],
                               ldb);
        }
    }
    else
    {
        ...
        if(*k == 1)
        {
            aocl_blas_scopy(nrhs, &b[b_offset], ldb, &bx[bx_offset], ldbx);
        }
        else
        {
            i__1 = *k;
            for(j = 1; j <= i__1; ++j)
            {
                ...
                aocl_blas_sgemv("T", k, nrhs, &c_b11, &b[b_offset], ldb, &work[1], &c__1, &c_b13,
                                &bx[j + bx_dim1], ldbx);
                /* L80: */
            }
        }
        ...
        if(*sqre == 1)
        {
            aocl_blas_scopy(nrhs, &b[m + b_dim1], ldb, &bx[m + bx_dim1], ldbx);
            aocl_blas_srot(nrhs, &bx[bx_dim1 + 1], ldbx, &bx[m + bx_dim1], ldbx, c__, s);
        }
        if(*k < fla_max(m, n))
        {
            i__1 = n - *k;
            aocl_lapack_slacpy("A", &i__1, nrhs, &b[*k + 1 + b_dim1], ldb, &bx[*k + 1 + bx_dim1],
                               ldbx);
        }
        /* Step (3R): permute rows of B. */
        aocl_blas_scopy(nrhs, &bx[bx_dim1 + 1], ldbx, &b[nlp1 + b_dim1], ldb);
        if(*sqre == 1)
        {
            aocl_blas_scopy(nrhs, &bx[m + bx_dim1], ldbx, &b[m + b_dim1], ldb);
        }
        i__1 = n;
        for(i__ = 2; i__ <= i__1; ++i__)
        {
            aocl_blas_scopy(nrhs, &bx[i__ + bx_dim1], ldbx, &b[perm[i__] + b_dim1], ldb);
            /* L90: */
        }
