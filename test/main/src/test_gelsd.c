...
/* Local prototypes */
void fla_test_gelsd_experiment(test_params_t *params, integer  datatype,
                               integer  p_cur, integer  q_cur, integer pci,
                               integer n_repeats, integer einfo,
                               double* perf, double* t, double* residual);
void prepare_gelsd_run(integer m_A, integer n_A, integer nrhs, void *A,
                       integer lda, void *B, integer ldb, void *s, void *rcond,
                       integer *rank, integer datatype, integer n_repeats,
                       double* time_min_, integer* info);
void invoke_gelsd(integer datatype, integer *m, integer *n, integer *nrhs,
                  void * a, integer *lda, void *b, integer *ldb, void *s,
                  void *rcond, integer *rank, void *work, integer *lwork,
                  void *rwork, integer * iwork, integer *info);
    ...
    if(argc >= 11 && argc <= 12)
    {
        ...
        if(n_repeats > 0)
        {
            ...
            for(i = 0; i < num_types; i++)
            {
                ...
                /* Call the test code */
                fla_test_gelsd_experiment(params, datatype,
                                          M, N,
                                          0,
                                          n_repeats, einfo,
...
void fla_test_gelsd_experiment(test_params_t *params,
    integer  datatype,
    integer  p_cur,
    integer  q_cur,
    integer pci,
    ...
    double* residual)
{
    ...
    *residual = params->lin_solver_paramslist[pci].solver_threshold;
    ...
    /* call to API */
    prepare_gelsd_run(m, n, NRHS, A_save, lda, B_save, ldb, S, rcond, &rank, datatype,
    ...
    *t = time_min;
    ...
    if(m >= n)
    {
        *perf = (double)(4.0 * m * n * (n + NRHS) + NRHS)
                / *t / FLOPS_PER_UNIT_PERF;
    }
    else
    {
        *perf = (double)((2.0 * m * NRHS * (m + n) + m * (4.0 * n * n + 1.0) + NRHS))
                 / *t / FLOPS_PER_UNIT_PERF;
    }
    ...
    if(info == 0)
        validate_gelsd(m, n, NRHS, A, lda, B, ldb, S, B_save,
                       rcond, &rank, datatype, residual);
...
void prepare_gelsd_run(integer m_A,
                       integer n_A,
                       integer nrhs,
                       void *A,
                       integer lda,
                       ...
                       integer* info)
{
    ...
    if(g_lwork <= 0)
    {
        ...
        /* call to  gelsd API */
        invoke_gelsd(datatype, &m_A, &n_A, &nrhs, NULL, &lda, NULL, &ldb,
                     NULL, rcond, rank, work, &lwork, rwork, iwork, info);
        /* Get work size */
        if(*info == 0)
        {
    ...
    *info = 0;
    for (i = 0; i < n_repeats && *info == 0; ++i)
    {
        ...
        /* call to API */
        invoke_gelsd(datatype, &m_A, &n_A, &nrhs, A_test, &lda, B_test, &ldb,
                  ...
                  void *rwork, integer *iwork, integer *info)
{
    switch(datatype)
    {
        case FLOAT:
        {
            fla_lapack_sgelsd(m, n, nrhs, a, lda, b, ldb, s, rcond, rank,
        ...
        case DOUBLE:
        {
            fla_lapack_dgelsd(m, n, nrhs, a, lda, b, ldb, s, rcond, rank,
                              work, lwork, iwork, info);
            break;
        }
        ...
        case COMPLEX:
        {
            fla_lapack_cgelsd(m, n, nrhs, a, lda, b, ldb, s, rcond, rank,
        ...
        case DOUBLE_COMPLEX:
        {
            fla_lapack_zgelsd(m, n, nrhs, a, lda, b, ldb, s, rcond, rank,
                              work, lwork, rwork, iwork, info);
            break;
        }
    }
