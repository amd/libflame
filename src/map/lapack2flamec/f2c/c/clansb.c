...
real clansb_(char *norm, char *uplo, aocl_int_t *n, aocl_int_t *k, scomplex *ab, aocl_int_t *ldab,
             real *work)
{
#if FLA_ENABLE_ILP64
    return aocl_lapack_clansb(norm, uplo, n, k, ab, ldab, work);
#else
    aocl_int64_t n_64 = *n;
    aocl_int64_t k_64 = *k;
    aocl_int64_t ldab_64 = *ldab;

    return aocl_lapack_clansb(norm, uplo, &n_64, &k_64, ab, &ldab_64, work);
#endif
}

real aocl_lapack_clansb(char *norm, char *uplo, aocl_int64_t *n, aocl_int64_t *k, scomplex *ab,
                        aocl_int64_t *ldab, real *work)
{
    ...
    else if(lsame_(norm, "F", 1, 1) || lsame_(norm, "E", 1, 1))
    {
        ...
        if(*k > 0)
        {
            if(lsame_(uplo, "U", 1, 1))
            {
                ...
                for(j = 2; j <= i__1; ++j)
                {
                    ...
                    /* Computing MAX */
                    i__2 = *k + 2 - j;
                    aocl_lapack_classq(&i__4, &ab[fla_max(i__2, 1) + j * ab_dim1], &c__1, &scale, &sum);
                    /* L110: */
                }
                l = *k + 1;
            }
            else
            {
                ...
                for(j = 1; j <= i__1; ++j)
                {
                    ...
                    i__3 = *n - j;
                    i__4 = fla_min(i__3, *k);
                    aocl_lapack_classq(&i__4, &ab[j * ab_dim1 + 2], &c__1, &scale, &sum);
                    /* L120: */
                }
                l = 1;
            }
        ...
        aocl_lapack_classq(n, &ab[l + ab_dim1], ldab, &scale, &sum);
