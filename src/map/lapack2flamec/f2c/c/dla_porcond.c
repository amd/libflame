...
doublereal dla_porcond_(char *uplo, aocl_int_t *n, doublereal *a, aocl_int_t *lda, doublereal *af,
                        aocl_int_t *ldaf, aocl_int_t *cmode, doublereal *c__, aocl_int_t *info,
                        doublereal *work, aocl_int_t *iwork)
{
#if FLA_ENABLE_ILP64
    return aocl_lapack_dla_porcond(uplo, n, a, lda, af, ldaf, cmode, c__, info, work, iwork);
#else
    aocl_int64_t n_64 = *n;
    ...
    doublereal ret_val = aocl_lapack_dla_porcond(uplo, &n_64, a, &lda_64, af, &ldaf_64, &cmode_64,
...
doublereal aocl_lapack_dla_porcond(char *uplo, aocl_int64_t *n, doublereal *a, aocl_int64_t *lda,
                                   doublereal *af, aocl_int64_t *ldaf, aocl_int64_t *cmode,
                                   ...
                                   aocl_int_t *iwork)
{
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("DLA_PORCOND", &i__1, (ftnlen)11);
...
L10:
    ...
    if(kase != 0)
    {
        if(kase == 2)
        {
            ...
            if(up)
            {
                aocl_lapack_dpotrs("Upper", n, &c__1, &af[af_offset], ldaf, &work[1], n, info);
            }
            else
            {
                aocl_lapack_dpotrs("Lower", n, &c__1, &af[af_offset], ldaf, &work[1], n, info);
            }
            /* Multiply by inv(C). */
            if(*cmode == 1)
            {
