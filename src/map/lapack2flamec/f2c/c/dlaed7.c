...
void dlaed7_(aocl_int_t *icompq, aocl_int_t *n, aocl_int_t *qsiz, aocl_int_t *tlvls,
             aocl_int_t *curlvl, aocl_int_t *curpbm, doublereal *d__, doublereal *q,
             aocl_int_t *ldq, aocl_int_t *indxq, doublereal *rho, aocl_int_t *cutpnt,
             doublereal *qstore, aocl_int_t *qptr, aocl_int_t *prmptr, aocl_int_t *perm,
             aocl_int_t *givptr, aocl_int_t *givcol, doublereal *givnum, doublereal *work,
             aocl_int_t *iwork, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_dlaed7(icompq, n, qsiz, tlvls, curlvl, curpbm, d__, q, ldq, indxq, rho, cutpnt,
    ...
    aocl_lapack_dlaed7(&icompq_64, &n_64, &qsiz_64, &tlvls_64, &curlvl_64, &curpbm_64, d__, q,
                       &ldq_64, indxq, rho, &cutpnt_64, qstore, qptr, prmptr, perm, givptr, givcol,
                       givnum, work, iwork, &info_64);

    *info = (aocl_int_t)info_64;
#endif
}

void aocl_lapack_dlaed7(aocl_int64_t *icompq, aocl_int64_t *n, aocl_int64_t *qsiz,
                        aocl_int64_t *tlvls, aocl_int64_t *curlvl, aocl_int64_t *curpbm,
                        doublereal *d__, doublereal *q, aocl_int64_t *ldq, aocl_int_t *indxq,
                        doublereal *rho, aocl_int64_t *cutpnt, doublereal *qstore, aocl_int_t *qptr,
                        aocl_int_t *prmptr, aocl_int_t *perm, aocl_int_t *givptr,
                        aocl_int_t *givcol, doublereal *givnum, doublereal *work, aocl_int_t *iwork,
                        aocl_int64_t *info)
{
    ...
    /* Local variables */
    aocl_int64_t i__, k, n1, n2, is, iw, iz, iq2, ptr, ldq2, indx, curr;
    aocl_int64_t indxc, indxp;
    aocl_int64_t idlmda;
    aocl_int64_t coltyp;
    int64_t givptr_sca;
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("DLAED7", &i__1, (ftnlen)6);
    ...
    curr = ptr + *curpbm;
    aocl_lapack_dlaeda(n, tlvls, curlvl, curpbm, &prmptr[1], &perm[1], &givptr[1], &givcol[3],
                       &givnum[3], &qstore[1], &qptr[1], &work[iz], &work[iz + *n], info);
    ...
    givptr_sca = givptr[curr + 1];
    aocl_lapack_dlaed8(icompq, &k, n, qsiz, &d__[1], &q[q_offset], ldq, &indxq[1], rho, cutpnt,
                       &work[iz], &work[idlmda], &work[iq2], &ldq2, &work[iw], &perm[prmptr[curr]],
                       &givptr_sca, &givcol[(givptr[curr] << 1) + 1],
                       &givnum[(givptr[curr] << 1) + 1], &iwork[indxp], &iwork[indx], info);
    givptr[curr + 1] = (aocl_int_t)givptr_sca;
    prmptr[curr + 1] = (aocl_int_t)(prmptr[curr] + *n);
    givptr[curr + 1] += givptr[curr];
    /* Solve Secular Equation. */
    if(k != 0)
    {
        aocl_lapack_dlaed9(&k, &c__1, &k, n, &d__[1], &work[is], &k, rho, &work[idlmda], &work[iw],
        ...
        if(*icompq == 1)
        {
            aocl_blas_dgemm("N", "N", qsiz, &k, &k, &c_b10, &work[iq2], &ldq2, &qstore[qptr[curr]],
                            &k, &c_b11, &q[q_offset], ldq);
        }
        /* Computing 2nd power */
        i__1 = k;
