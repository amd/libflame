...
void chetri2_(char *uplo, aocl_int_t *n, scomplex *a, aocl_int_t *lda, aocl_int_t *ipiv,
              scomplex *work, aocl_int_t *lwork, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_chetri2(uplo, n, a, lda, ipiv, work, lwork, info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_chetri2(uplo, &n_64, a, &lda_64, ipiv, work, &lwork_64, &info_64);

    *info = (aocl_int_t)info_64;
#endif
}

void aocl_lapack_chetri2(char *uplo, aocl_int64_t *n, scomplex *a, aocl_int64_t *lda,
                         aocl_int_t *ipiv, scomplex *work, aocl_int64_t *lwork, aocl_int64_t *info)
{
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("CHETRI2", &i__1, (ftnlen)7);
    ...
    if(nbmax >= *n)
    {
        aocl_lapack_chetri(uplo, n, &a[a_offset], lda, &ipiv[1], &work[1], info);
    }
    else
    {
        aocl_lapack_chetri2x(uplo, n, &a[a_offset], lda, &ipiv[1], &work[1], &nbmax, info);
    }
    AOCL_DTL_TRACE_EXIT(AOCL_DTL_LEVEL_TRACE_5);
    return;
    /* End of CHETRI2 */
