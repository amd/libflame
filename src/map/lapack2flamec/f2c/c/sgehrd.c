...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/sgehrd.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/sgehrd.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/sgehrd.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
void sgehrd_(aocl_int_t *n, aocl_int_t *ilo, aocl_int_t *ihi, real *a, aocl_int_t *lda, real *tau,
             real *work, aocl_int_t *lwork, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_sgehrd(n, ilo, ihi, a, lda, tau, work, lwork, info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_sgehrd(&n_64, &ilo_64, &ihi_64, a, &lda_64, tau, work, &lwork_64, &info_64);

    *info = (aocl_int_t)info_64;
#endif
}

void aocl_lapack_sgehrd(aocl_int64_t *n, aocl_int64_t *ilo, aocl_int64_t *ihi, real *a,
                        ...
                        aocl_int64_t *info)
{
    ...
    real ei;
    aocl_int64_t nb, nh, nx, iwt, nbmin, iinfo;
    ...
    *info = 0;
    ...
    if(nb > 1 && nb < nh)
    {
        ...
        /* Computing MAX */
        i__1 = nb;
        i__2 = aocl_lapack_ilaenv(&c__3, "SGEHRD", " ", n, ilo, ihi, &c_n1); // , expr subst
        ...
        if(nx < nh)
        {
            ...
            if(*lwork < *n * nb + 4160)
            {
                ...
                /* Computing MAX */
                i__1 = 2;
                i__2 = aocl_lapack_ilaenv(&c__2, "SGEHRD", " ", n, ilo, ihi, &c_n1); // , expr subst
    ...
    else
    {
        ...
        for(i__ = *ilo; i__2 < 0 ? i__ >= i__1 : i__ <= i__1; i__ += i__2)
        {
            ...
            aocl_lapack_slahr2(ihi, &i__, &ib, &a[i__ * a_dim1 + 1], lda, &tau[i__], &work[iwt],
                               &c__65, &work[1], &ldwork);
            ...
            i__3 = *ihi - i__ - ib + 1;
            aocl_blas_sgemm("No transpose", "Transpose", ihi, &i__3, &ib, &c_b25, &work[1], &ldwork,
            ...
            /* right */
            i__3 = ib - 1;
            aocl_blas_strmm("Right", "Lower", "Transpose", "Unit", &i__, &i__3, &c_b26,
                            &a[i__ + 1 + i__ * a_dim1], lda, &work[1], &ldwork);
            i__3 = ib - 2;
            for(j = 0; j <= i__3; ++j)
            {
                aocl_blas_saxpy(&i__, &c_b25, &work[ldwork * j + 1], &c__1,
    ...
    aocl_lapack_sgehd2(n, &i__, ihi, &a[a_offset], lda, &tau[1], &work[1], &iinfo);
    work[1] = aocl_lapack_sroundup_lwork(&lwkopt);
    AOCL_DTL_TRACE_LOG_EXIT
    return;
    /* End of SGEHRD */
