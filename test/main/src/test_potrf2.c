...
{
    char *op_str = "Cholesky factorization (recursive algorithm)";
    char *front_str = "POTRF2";
    ...
    if(argc == 1)
    {
        ...
        fla_test_op_driver(front_str, SQUARE_INPUT, params, LIN, fla_test_potrf2_experiment);
        tests_not_run = 0;
    }
    if(argc == 8)
    {
    ...
    if(argc >= 7 && argc <= 8)
    {
        ...
        if(n_repeats > 0)
        {
            ...
            for(i = 0; i < num_types; i++)
            {
                ...
                /* Call the test code */
                fla_test_potrf2_experiment(front_str, params, datatype, N, N, 0, n_repeats, einfo);
                tests_not_run = 0;
            }
        }
    }
    ...
    if(tests_not_run)
    {
        printf("\nIllegal arguments for potrf2\n");
        printf("./<EXE> potrf2 <precisions - sdcz> <Uplo> <N> <LDA> <repeats>\n");
    }
    if(invalid_dtype)
    {
                                ...
                                integer einfo)
{
    ...
    prepare_potrf2_run(&uplo, m, A_test, lda, datatype, &info, params);
                        ...
                        test_params_t *params)
{
    ...
    *info = 0;
    FLA_EXEC_LOOP_BEGIN
    {
        ...
        /* Call LAPACK potrf2 API */
        invoke_potrf2(uplo, datatype, &m, A, &lda, info);
...
void invoke_potrf2(char *uplo, integer datatype, integer *m, void *a, integer *lda, integer *info)
{
    switch(datatype)
    {
        case FLOAT:
        {
            fla_lapack_spotrf2(uplo, m, a, lda, info);
            break;
        }
        case DOUBLE:
        {
            fla_lapack_dpotrf2(uplo, m, a, lda, info);
            break;
        }
        case COMPLEX:
        {
            fla_lapack_cpotrf2(uplo, m, a, lda, info);
            break;
        }
        case DOUBLE_COMPLEX:
        {
            fla_lapack_zpotrf2(uplo, m, a, lda, info);
            break;
        }
    }
}