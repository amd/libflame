/* ../netlib/zgeqr2p.f -- translated by f2c (version 20100827). You must link the resulting object
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/zgeqr2p
 * .f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/zgeqr2p
 * .f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/zgeqr2p
 * .f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
/* > \verbatim */
/* > */
/* > ZGEQR2P computes a QR factorization of a scomplex m by n matrix A: */
/* > A = Q * R. */
/* > \endverbatim */
/* Arguments: */
/* ========== */
...
void zgeqr2p_(aocl_int_t *m, aocl_int_t *n, dcomplex *a, aocl_int_t *lda, dcomplex *tau,
              dcomplex *work, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_zgeqr2p(m, n, a, lda, tau, work, info);
#else
    aocl_int64_t m_64 = *m;
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_zgeqr2p(&m_64, &n_64, a, &lda_64, tau, work, &info_64);

    *info = (aocl_int_t)info_64;
#endif
}

void aocl_lapack_zgeqr2p(aocl_int64_t *m, aocl_int64_t *n, dcomplex *a, aocl_int64_t *lda,
                         dcomplex *tau, dcomplex *work, aocl_int64_t *info)
{
    ...
    *info = 0;
    ...
    for(i__ = 1; i__ <= i__1; ++i__)
    {
        ...
        /* Computing MIN */
        i__3 = i__ + 1;
        aocl_lapack_zlarfgp(&i__2, &a[i__ + i__ * a_dim1], &a[fla_min(i__3, *m) + i__ * a_dim1],
                            &c__1, &tau[i__]);
        if(i__ < *n)
        {
