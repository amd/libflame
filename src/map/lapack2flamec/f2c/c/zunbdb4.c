...
/* ../netlib/zunbdb4.f -- translated by f2c (version 20160102). You must link the resulting object
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/zunbdb4
 * .f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/zunbdb4
 * .f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/zunbdb4
 * .f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
void zunbdb4_(aocl_int_t *m, aocl_int_t *p, aocl_int_t *q, dcomplex *x11, aocl_int_t *ldx11,
              dcomplex *x21, aocl_int_t *ldx21, doublereal *theta, doublereal *phi,
              dcomplex *taup1, dcomplex *taup2, dcomplex *tauq1,
              dcomplex *phantom, dcomplex *work, aocl_int_t *lwork, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_zunbdb4(m, p, q, x11, ldx11, x21, ldx21, theta, phi, taup1, taup2, tauq1, phantom,
                        work, lwork, info);
#else
    aocl_int64_t m_64 = *m;
    aocl_int64_t p_64 = *p;
    ...
    aocl_lapack_zunbdb4(&m_64, &p_64, &q_64, x11, &ldx11_64, x21, &ldx21_64, theta, phi, taup1,
...
void aocl_lapack_zunbdb4(aocl_int64_t *m, aocl_int64_t *p, aocl_int64_t *q, dcomplex *x11,
                         aocl_int64_t *ldx11, dcomplex *x21, aocl_int64_t *ldx21,
                         doublereal *theta, doublereal *phi, dcomplex *taup1,
                         dcomplex *taup2, dcomplex *tauq1, dcomplex *phantom,
                         dcomplex *work, aocl_int64_t *lwork, aocl_int64_t *info)
{
    AOCL_DTL_TRACE_LOG_INIT
    AOCL_DTL_SNPRINTF("zunbdb4 inputs: m %" FLA_IS ", p %" FLA_IS ", q %" FLA_IS ", ldx11 %" FLA_IS
    ...
    aocl_int64_t i__, j;
    doublereal s;
    aocl_int64_t childinfo, ilarf, llarf;
    logical lquery;
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("ZUNBDB4", &i__1, (ftnlen)7);
    ...
    for(i__ = 1; i__ <= i__1; ++i__)
    {
        if(i__ == 1)
        {
            ...
            aocl_lapack_zunbdb5(p, &i__2, q, &phantom[1], &c__1, &phantom[*p + 1], &c__1,
                                ...
                                &lorbdb5, &childinfo);
            aocl_blas_zscal(p, &c_b1, &phantom[1], &c__1);
            ...
            s = sin(theta[i__]);
            d_cnjg(&z__1, &taup1[1]);
            aocl_lapack_zlarf1f("L", p, q, &phantom[1], &c__1, &z__1, &x11[x11_offset], ldx11,
                              &work[ilarf]);
            i__2 = *m - *p;
            d_cnjg(&z__1, &taup2[1]);
            aocl_lapack_zlarf1f("L", &i__2, q, &phantom[*p + 1], &c__1, &z__1, &x21[x21_offset],
                              ldx21, &work[ilarf]);
        }
        else
        {
            ...
            i__3 = *m - *p - i__ + 1;
            i__4 = *q - i__ + 1;
            aocl_lapack_zunbdb5(&i__2, &i__3, &i__4, &x11[i__ + (i__ - 1) * x11_dim1], &c__1,
                                &x21[i__ + (i__ - 1) * x21_dim1], &c__1, &x11[i__ + i__ * x11_dim1],
                                ldx11, &x21[i__ + i__ * x21_dim1], ldx21, &work[iorbdb5], &lorbdb5,
                                &childinfo);
            i__2 = *p - i__ + 1;
            aocl_blas_zscal(&i__2, &c_b1, &x11[i__ + (i__ - 1) * x11_dim1], &c__1);
            i__2 = *p - i__ + 1;
            ...
            aocl_lapack_zlarf1f("L", &i__2, &i__3, &x11[i__ + (i__ - 1) * x11_dim1], &c__1, &z__1,
                              &x11[i__ + i__ * x11_dim1], ldx11, &work[ilarf]);
            i__2 = *m - *p - i__ + 1;
            i__3 = *q - i__ + 1;
            d_cnjg(&z__1, &taup2[i__]);
            aocl_lapack_zlarf1f("L", &i__2, &i__3, &x21[i__ + (i__ - 1) * x21_dim1], &c__1, &z__1,
                              &x21[i__ + i__ * x21_dim1], ldx21, &work[ilarf]);
        }
        i__2 = *q - i__ + 1;
        d__1 = -c__;
        aocl_blas_zdrot(&i__2, &x11[i__ + i__ * x11_dim1], ldx11, &x21[i__ + i__ * x21_dim1], ldx21,
                        &s, &d__1);
        i__2 = *q - i__ + 1;
        ...
        if(i__ < *m - *q)
        {
            i__2 = *p - i__;
            /* Computing 2nd power */
            d__1 = aocl_blas_dznrm2(&i__2, &x11[i__ + 1 + i__ * x11_dim1], &c__1);
            i__3 = *m - *p - i__;
            /* Computing 2nd power */
            d__2 = aocl_blas_dznrm2(&i__3, &x21[i__ + 1 + i__ * x21_dim1], &c__1);
    ...
    for(i__ = *m - *q + 1; i__ <= i__1; ++i__)
    {
        ...
        i__2 = *p - i__;
        i__3 = *q - i__ + 1;
        aocl_lapack_zlarf1f("R", &i__2, &i__3, &x11[i__ + i__ * x11_dim1], ldx11, &tauq1[i__],
                          &x11[i__ + 1 + i__ * x11_dim1], ldx11, &work[ilarf]);
        i__2 = *q - *p;
        i__3 = *q - i__ + 1;
        aocl_lapack_zlarf1f("R", &i__2, &i__3, &x11[i__ + i__ * x11_dim1], ldx11, &tauq1[i__],
    ...
    {
        i__2 = *q - i__ + 1;
        aocl_lapack_zlacgv(&i__2, &x21[*m - *q + i__ - *p + i__ * x21_dim1], ldx21);
        i__2 = *q - i__ + 1;
        aocl_lapack_zlarfgp(&i__2, &x21[*m - *q + i__ - *p + i__ * x21_dim1],
                            &x21[*m - *q + i__ - *p + (i__ + 1) * x21_dim1], ldx21, &tauq1[i__]);
        i__2 = *q - i__;
        i__3 = *q - i__ + 1;
        aocl_lapack_zlarf1f("R", &i__2, &i__3, &x21[*m - *q + i__ - *p + i__ * x21_dim1], ldx21,
