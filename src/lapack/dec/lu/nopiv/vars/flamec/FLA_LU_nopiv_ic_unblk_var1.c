...
FLA_Error FLA_LU_nopiv_ic_unblk_var1( fla_dim_t m_A, fla_dim_t n_A, scomplex* A , fla_dim_t nfact, fla_dim_t rs_A, fla_dim_t cs_A )
{
  ...
  for( i = 0; i < nfact - 1; i++ )
  {
    ...
    aocl_blas_cscal( &diff, &alpha_inv, (A + i + 1 + i*cs_A), &rs_A );
    inc_x = (i == nfact - 2) ? cs_A : rs_A;
    inc_y = cs_A;
    aocl_blas_cgeru( &diff, &diff, Minusone, (A + i + 1 + i * cs_A), &inc_x, (A + i + (i + 1) * cs_A), &inc_y, (A + i + 1 + (i + 1) * cs_A), &cs_A );
  ...
  if( e_val == FLA_SUCCESS )
  {
    ...
    if( tr_m != 0 )                             // if size of ATR is 0 then trsm is invalid
    {
       ...
       {
          aocl_blas_ctrsm( "L", "L", "N", "U", &nfact, &tr_m, One, A, &cs_A, (A + cs_A * nfact), &cs_A );
       }
    }
    ...
    {
      aocl_blas_ctrsm( "R", "U", "N", "N", &tr_n, &nfact, One, A, &cs_A, (A + nfact), &cs_A );
    }

    // ABR = ABR - new ABL * new ATR
    if( (tr_m != 0) && (tr_n != 0) )      // base invalid cases check
    {
      aocl_blas_cgemm( "N", "N", &tr_n, &tr_m,  &nfact, Minusone, (A + nfact), &cs_A, (A + nfact * cs_A), &cs_A, One, (A + nfact + nfact * cs_A), &cs_A );
    }
  }
  else                                                // setting matrix to last known valid state when encountering singular matrix
  {
    ...
    if( tr_m != 0 )                             // if size of ATR is 0 then trsm is invalid
    {
      aocl_blas_ctrsm( "L", "L", "N", "U", (fla_dim_t *) &e_val, &tr_m, One, A, &cs_A, (A + cs_A * nfact), &cs_A );
    }

    // L2 valid ABL calculation = U1^-1 * valid ABL   // ABL size to be updated is (n_A - nfact) * e_val
    if( tr_n != 0 )                             // base invalid cases when size of  ABL is 0, nfact=1 case handled seperately
    {
       aocl_blas_ctrsm( "R", "U", "N", "N", &tr_n, (fla_dim_t *) &e_val, One, A, &cs_A, (A + nfact), &cs_A );
    }
    ...
    {
     tr_nfe = nfact - e_val;
     aocl_blas_cgemm( "N", "N", &tr_n, &(tr_nfe), (fla_dim_t *) &e_val, Minusone, (A + nfact), &cs_A, (A + e_val * cs_A), &cs_A, One, (A + nfact + e_val * cs_A), &cs_A );
    }
    ...
    {
       tr_ne = n_A - e_val;
       aocl_blas_cgemm( "N", "N", &(tr_ne), &tr_m, (fla_dim_t *)  &e_val, Minusone, (A + e_val), &cs_A, (A + nfact * cs_A), &cs_A, One, (A + e_val + nfact * cs_A), &cs_A );
    }
