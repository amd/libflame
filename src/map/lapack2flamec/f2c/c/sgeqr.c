...
void sgeqr_(aocl_int_t *m, aocl_int_t *n, real *a, aocl_int_t *lda, real *t, aocl_int_t *tsize,
            real *work, aocl_int_t *lwork, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_sgeqr(m, n, a, lda, t, tsize, work, lwork, info);
#else
    aocl_int64_t m_64 = *m;
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_sgeqr(&m_64, &n_64, a, &lda_64, t, &tsize_64, work, &lwork_64, &info_64);

    *info = (aocl_int_t)info_64;
#endif
}

void aocl_lapack_sgeqr(aocl_int64_t *m, aocl_int64_t *n, real *a, aocl_int64_t *lda, real *t,
                       aocl_int64_t *tsize, real *work, aocl_int64_t *lwork, aocl_int64_t *info)
{
    ...
    *info = 0;
    ...
    if(fla_min(*m, *n) > 0)
    {
        mb = aocl_lapack_ilaenv(&c__1, "SGEQR ", " ", m, n, &c__1, &c_n1);
        nb = aocl_lapack_ilaenv(&c__1, "SGEQR ", " ", m, n, &c__2, &c_n1);
    }
    else
    {
        mb = *m;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("SGEQR", &i__1, (ftnlen)5);
    ...
    if(*m <= *n || mb <= *n || mb >= *m)
    {
        aocl_lapack_sgeqrt(m, n, &nb, &a[a_offset], lda, &t[6], &nb, &work[1], info);
    }
    else
    {
