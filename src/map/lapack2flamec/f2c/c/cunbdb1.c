...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/cunbdb1
 * .f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/cunbdb1
 * .f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/cunbdb1
 * .f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
#if FLA_ENABLE_ILP64
    snprintf(buffer, 256,
             "cunbdb1 inputs: m %lld, p %lld, q %lld, ldx11 %lld, ldx21 %lld, lwork %lld", *m, *p,
             *q, *ldx11, *ldx21, *lwork);
#else
    snprintf(buffer, 256, "cunbdb1 inputs: m %d, p %d, q %d, ldx11 %d, ldx21 %d, lwork %d", *m, *p,
             *q, *ldx11, *ldx21, *lwork);
#endif
    ...
    *info = 0;
    ...
    for(i__ = 1; i__ <= i__1; ++i__)
    {
        ...
        aocl_lapack_clarfgp(&i__2, &x21[i__ + i__ * x21_dim1], &x21[i__ + 1 + i__ * x21_dim1],
                            &c__1, &taup2[i__]);
        theta[i__] = atan2((real)x21[i__ + i__ * x21_dim1].real, (real)x11[i__ + i__ * x11_dim1].real);
        c__ = cos(theta[i__]);
        s = sin(theta[i__]);
        i__2 = *p - i__ + 1;
        i__3 = *q - i__;
        ...
        if(i__ < *q)
        {
            ...
            i__2 = *p - i__;
            i__3 = *q - i__;
            aocl_lapack_clarf1f("R", &i__2, &i__3, &x21[i__ + (i__ + 1) * x21_dim1], ldx21,
