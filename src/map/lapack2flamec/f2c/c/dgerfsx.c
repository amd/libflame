...
/* ../netlib/dgerfsx.f -- translated by f2c (version 20160102). You must link the resulting object
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/dgerfsx
 * .f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/dgerfsx
 * .f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/dgerfsx
 * .f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
/* ===================================================================== */
/* Subroutine */
void dgerfsx_(char *trans, char *equed, integer *n, integer *nrhs, doublereal *a, integer *lda,
              doublereal *af, integer *ldaf, integer *ipiv, doublereal *r__, doublereal *c__,
              ...
              integer *iwork, integer *info)
{
    ...
    AOCL_DTL_SNPRINTF("dgerfsx inputs: trans %c, n %" FLA_IS ", nrhs %" FLA_IS ", lda %" FLA_IS
                      ", ldaf %" FLA_IS ", ldb %" FLA_IS ", ldx %" FLA_IS ", n_err_bnds__ %" FLA_IS
                      ", nparams %" FLA_IS "",
    ...
    extern doublereal dla_gercond_(char *, integer *, doublereal *, integer *, doublereal *,
                                   integer *, integer *, integer *, doublereal *, integer *,
                                   doublereal *, integer *);
    doublereal cwise_wrong__;
    extern /* Subroutine */
        void
        dla_gerfsx_extended_(integer *, integer *, integer *, integer *, doublereal *, integer *,
    ...
    *rcond = 0.;
    ...
    if(ref_type__ != 0)
    {
        ...
        if(notran)
        {
            dla_gerfsx_extended_(&prec_type__, &trans_type__, n, nrhs, &a[a_offset], lda,
    ...
    if(*n_err_bnds__ >= 1 && n_norms__ >= 1)
    {
        ...
        if(colequ && notran)
        {
            rcond_tmp__ = dla_gercond_(trans, n, &a[a_offset], lda, &af[af_offset], ldaf, &ipiv[1],
    ...
    if(*n_err_bnds__ >= 1 && n_norms__ >= 2)
    {
        ...
        for(j = 1; j <= i__1; ++j)
        {
            ...
            {
                rcond_tmp__
                    = dla_gercond_(trans, n, &a[a_offset], lda, &af[af_offset], ldaf, &ipiv[1],
