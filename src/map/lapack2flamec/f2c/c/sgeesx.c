...
/* > \brief <b> SGEESX computes the eigenvalues, the Schur form, and, optionally, the matrix of
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/sgeesx.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/sgeesx.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/sgeesx.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
void sgeesx_(char *jobvs, char *sort, L_fps2 select, char *sense, aocl_int_t *n, real *a,
             aocl_int_t *lda, aocl_int_t *sdim, real *wr, real *wi, real *vs, aocl_int_t *ldvs,
             real *rconde, real *rcondv, real *work, aocl_int_t *lwork, aocl_int_t *iwork,
             aocl_int_t *liwork, logical *bwork, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_sgeesx(jobvs, sort, select, sense, n, a, lda, sdim, wr, wi, vs, ldvs, rconde,
    ...
    aocl_lapack_sgeesx(jobvs, sort, select, sense, &n_64, a, &lda_64, &sdim_64, wr, wi, vs,
...
void aocl_lapack_sgeesx(char *jobvs, char *sort, L_fps2 select, char *sense, aocl_int64_t *n,
                        real *a, aocl_int64_t *lda, aocl_int64_t *sdim, real *wr, real *wi,
                        real *vs, aocl_int64_t *ldvs, real *rconde, real *rcondv, real *work,
                        aocl_int64_t *lwork, aocl_int_t *iwork, aocl_int64_t *liwork,
                        logical *bwork, aocl_int64_t *info)
{
    AOCL_DTL_TRACE_LOG_INIT
    AOCL_DTL_SNPRINTF("sgeesx inputs: jobvs %c, sort %c, sense %c, n %" FLA_IS ", lda %" FLA_IS
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("SGEESX", &i__1, (ftnlen)6);
    ...
    *sdim = 0;
    ...
    if(wantst && *info == 0)
    {
        ...
        /* otherwise, need 0 ) */
        i__1 = *lwork - iwrk + 1;
        aocl_lapack_strsen(sense, jobvs, &bwork[1], n, &a[a_offset], lda, &vs[vs_offset], ldvs,
    ...
    if(scalea)
    {
        ...
        if(cscale == smlnum)
        {
            ...
            if(ieval > 0)
            {
                ...
                i2 = ihi - 1;
                i__1 = ilo - 1;
                aocl_lapack_slascl("G", &c__0, &c__0, &cscale, &anrm, &i__1, &c__1, &wi[1], n,
                                   &ierr);
            }
            else if(wantst)
            {
    ...
    if(wantsv || wantsb)
    {
        iwork[1] = (aocl_int_t)(*sdim * (*n - *sdim));
    }
    else
    {
        iwork[1] = 1;
