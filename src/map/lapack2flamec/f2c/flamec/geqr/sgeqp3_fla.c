...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/sgeqp3.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/sgeqp3.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/sgeqp3.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
                ...
                real *tau, real *work, aocl_int64_t *lwork, aocl_int64_t *info)
{
    ...
    aocl_int64_t nbmin, minmn, minws;
    ...
    *info = 0;
    ...
    for(j = 1; j <= i__1; ++j)
    {
        if(jpvt[j] != 0)
        {
            if(j != nfxd)
            {
                aocl_blas_sswap(m, &a[j * a_dim1 + 1], &c__1, &a[nfxd * a_dim1 + 1], &c__1);
                jpvt[j] = jpvt[nfxd];
    ...
    if(nfxd > 0)
    {
        ...
        aocl_lapack_sgeqrf(m, &na, &a[a_offset], lda, &tau[1], &work[1], lwork, info);
        /* Computing MAX */
        i__1 = iws;
        ...
        if(na < *n)
        {
            ...
            i__1 = *n - na;
            aocl_lapack_sormqr("Left", "Transpose", m, &i__1, &na, &a[a_offset], lda, &tau[1],
    ...
    if(nfxd < minmn)
    {
        ...
        nb = aocl_lapack_ilaenv(&c__1, "SGEQRF", " ", &sm, &sn, &c_n1, &c_n1);
        nbmin = 2;
        nx = 0;
        if(nb > 1 && nb < sminmn)
        {
            ...
            /* Computing MAX */
            i__1 = 0;
            i__2 = aocl_lapack_ilaenv(&c__3, "SGEQRF", " ", &sm, &sn, &c_n1, &c_n1); // , expr subst
            ...
            if(nx < sminmn)
            {
                ...
                if(*lwork < minws)
                {
                    ...
                    /* Computing MAX */
                    i__1 = 2;
                    i__2 = aocl_lapack_ilaenv(&c__2, "SGEQRF", " ", &sm, &sn, &c_n1,
                                              &c_n1); // , expr subst
        ...
        {
            work[j] = aocl_blas_snrm2(&sm, &a[nfxd + 1 + j * a_dim1], &c__1);
            work[*n + j] = work[j];
            /* L20: */
        }
        ...
        L30:
            if(j <= topbmn)
            {
                ...
                i__2 = j - 1;
                i__3 = *n - j + 1;
                aocl_lapack_slaqps(m, &i__1, &i__2, &jb, &fjb, &a[j * a_dim1 + 1], lda, &jpvt[j],
        ...
        if(j <= minmn)
        {
            i__1 = *n - j + 1;
            i__2 = j - 1;
            aocl_lapack_slaqp2(m, &i__1, &i__2, &a[j * a_dim1 + 1], lda, &jpvt[j], &tau[j],
