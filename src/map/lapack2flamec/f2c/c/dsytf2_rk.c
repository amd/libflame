/* ../netlib/v3.9.0/dsytf2_rk.f -- translated by f2c (version 20160102). You must link the resulting
...
/* > \brief \b DSYTF2_RK computes the factorization of a real symmetric indefinite matrix using the
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/dsytf2_
 * rk.f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/dsytf2_
 * rk.f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/dsytf2_
 * rk.f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
void dsytf2_rk_(char *uplo, aocl_int_t *n, doublereal *a, aocl_int_t *lda, doublereal *e,
                aocl_int_t *ipiv, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_dsytf2_rk(uplo, n, a, lda, e, ipiv, info);
#else
    aocl_int64_t n_64 = *n;
    aocl_int64_t lda_64 = *lda;
    aocl_int64_t info_64 = *info;

    aocl_lapack_dsytf2_rk(uplo, &n_64, a, &lda_64, e, ipiv, &info_64);

    *info = (aocl_int_t)info_64;
#endif
}

void aocl_lapack_dsytf2_rk(char *uplo, aocl_int64_t *n, doublereal *a, aocl_int64_t *lda,
                           doublereal *e, aocl_int_t *ipiv, aocl_int64_t *info)
{
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("DSYTF2_RK", &i__1, (ftnlen)9);
    ...
    L10: /* If K < 1, exit from loop */
        ...
        else
        {
            ...
            L12: /* Begin pivot search loop body */
                ...
                {
                    i__1 = imax - 1;
                    itemp = aocl_blas_idamax(&i__1, &a[imax * a_dim1 + 1], &c__1);
            ...
            if(kstep == 2 && p != k)
            {
                ...
                {
                    i__1 = p - 1;
                    aocl_blas_dswap(&i__1, &a[k * a_dim1 + 1], &c__1, &a[p * a_dim1 + 1], &c__1);
                }
                if(p < k - 1)
                {
                    i__1 = k - p - 1;
                    aocl_blas_dswap(&i__1, &a[p + 1 + k * a_dim1], &c__1, &a[p + (p + 1) * a_dim1],
                                    lda);
                }
                t = a[k + k * a_dim1];
                ...
                {
                    i__1 = *n - k;
                    aocl_blas_dswap(&i__1, &a[k + (k + 1) * a_dim1], lda, &a[p + (k + 1) * a_dim1],
                                    lda);
                }
            }
            /* Second swap */
                    ...
                    for(j = k - 2; j >= 1; --j)
                    {
                        ...
                        for(i__ = j; i__ >= 1; --i__)
                        {
                            a[i__ + j * a_dim1] = a[i__ + j * a_dim1]
                                                  - a[i__ + k * a_dim1] / d12 * wk
                                                  - a[i__ + (k - 1) * a_dim1] / d12 * wkm1;
                            /* L20: */
                        }
    ...
    L40: /* If K > N, exit from loop */
        ...
        else
        {
            ...
            L42: /* Begin pivot search loop body */
                ...
                {
                    i__1 = *n - imax;
                    itemp = imax + aocl_blas_idamax(&i__1, &a[imax + 1 + imax * a_dim1], &c__1);
                    dtemp = (d__1 = a[itemp + imax * a_dim1], f2c_dabs(d__1));
                    if(dtemp > rowmax)
                    {
                        rowmax = dtemp;
            ...
            if(kstep == 2 && p != k)
            {
                ...
                {
                    i__1 = *n - p;
                    aocl_blas_dswap(&i__1, &a[p + 1 + k * a_dim1], &c__1, &a[p + 1 + p * a_dim1],
                                    &c__1);
                }
                if(p > k + 1)
                {
                    i__1 = p - k - 1;
                    aocl_blas_dswap(&i__1, &a[k + 1 + k * a_dim1], &c__1, &a[p + (k + 1) * a_dim1],
                                    lda);
                }
                t = a[k + k * a_dim1];
                a[k + k * a_dim1] = a[p + p * a_dim1];
                ...
                {
                    i__1 = k - 1;
                    aocl_blas_dswap(&i__1, &a[k + a_dim1], lda, &a[p + a_dim1], lda);
                }
            }
            /* Second swap */
            kk = k + kstep - 1;
            ...
            else
            {
                ...
                if(k < *n - 1)
                {
                    ...
                    for(j = k + 2; j <= i__1; ++j)
                    {
                        ...
                        {
                            a[i__ + j * a_dim1] = a[i__ + j * a_dim1]
                                                  - a[i__ + k * a_dim1] / d21 * wk
                                                  - a[i__ + (k + 1) * a_dim1] / d21 * wkp1;
                            /* L50: */
                        }
