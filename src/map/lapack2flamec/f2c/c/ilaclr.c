...
aocl_int_t ilaclr_(aocl_int_t *m, aocl_int_t *n, scomplex *a, aocl_int_t *lda)
{
#if FLA_ENABLE_ILP64
    return aocl_lapack_ilaclr(m, n, a, lda);
#else
    aocl_int64_t m_64 = *m;
    aocl_int64_t n_64 = *n;
    aocl_int64_t lda_64 = *lda;

    return aocl_lapack_ilaclr(&m_64, &n_64, a, &lda_64);
#endif
}

aocl_int64_t aocl_lapack_ilaclr(aocl_int64_t *m, aocl_int64_t *n, scomplex *a, aocl_int64_t *lda)
{
        ...
        else
        {
            ...
            for(j = 1; j <= i__1; ++j)
            {
                ...
                for(;;)
                {
                    ...
                    if(!(a[i__2].real == 0.f && a[i__2].imag == 0.f && i__ >= 1))
                        break;
                    --i__;
                }
