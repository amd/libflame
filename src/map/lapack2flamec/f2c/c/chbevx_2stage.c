/* ./chbevx_2stage.f -- translated by f2c (version 20190311). You must link the resulting object
...
/* > \brief <b> CHBEVX_2STAGE computes the eigenvalues and, optionally, the left and/or right
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/chbevx_
 * 2stage.f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/chbevx_
 * 2stage.f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/chbevx_
 * 2stage.f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
/* > of a scomplex Hermitian band matrix A using the 2stage technique for */
...
void chbevx_2stage_(char *jobz, char *range, char *uplo, aocl_int_t *n, aocl_int_t *kd, scomplex *ab,
                    aocl_int_t *ldab, scomplex *q, aocl_int_t *ldq, real *vl, real *vu,
                    aocl_int_t *il, aocl_int_t *iu, real *abstol, aocl_int_t *m, real *w,
                    scomplex *z__, aocl_int_t *ldz, scomplex *work, aocl_int_t *lwork, real *rwork,
                    aocl_int_t *iwork, aocl_int_t *ifail, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_chbevx_2stage(jobz, range, uplo, n, kd, ab, ldab, q, ldq, vl, vu, il, iu, abstol, m,
                              w, z__, ldz, work, lwork, rwork, iwork, ifail, info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_chbevx_2stage(jobz, range, uplo, &n_64, &kd_64, ab, &ldab_64, q, &ldq_64, vl, vu,
                              &il_64, &iu_64, abstol, &m_64, w, z__, &ldz_64, work, &lwork_64,
                              rwork, iwork, ifail, &info_64);
...
void aocl_lapack_chbevx_2stage(char *jobz, char *range, char *uplo, aocl_int64_t *n,
                               aocl_int64_t *kd, scomplex *ab, aocl_int64_t *ldab, scomplex *q,
                               aocl_int64_t *ldq, real *vl, real *vu, aocl_int64_t *il,
                               aocl_int64_t *iu, real *abstol, aocl_int64_t *m, real *w,
                               scomplex *z__, aocl_int64_t *ldz, scomplex *work, aocl_int64_t *lwork,
                               real *rwork, aocl_int_t *iwork, aocl_int_t *ifail,
                               aocl_int64_t *info)
{
...
#if FLA_ENABLE_ILP64
    snprintf(buffer, 256,
             "chbevx_2stage inputs: jobz %c, range %c, uplo %c, n %lld, kd %lld, ldab %lld, ldq "
             "%lld, il %lld, iu %lld, m %lld, ldz %lld, lwork %lld",
             *jobz, *range, *uplo, *n, *kd, *ldab, *ldq, *il, *iu, *m, *ldz, *lwork);
#else
    snprintf(buffer, 256,
             "chbevx_2stage inputs: jobz %c, range %c, uplo %c, n %d, kd %d, ldab %d, ldq %d, il "
             "%d, iu %d, m %d, ldz %d, lwork %d",
             *jobz, *range, *uplo, *n, *kd, *ldab, *ldq, *il, *iu, *m, *ldz, *lwork);
#endif
    ...
    *info = 0;
    ...
    if(*info == 0)
    {
        ...
        {
            ib = aocl_lapack_ilaenv2stage(&c__2, "CHETRD_HB2ST", jobz, n, kd, &c_n1, &c_n1);
            lhtrd = aocl_lapack_ilaenv2stage(&c__3, "CHETRD_HB2ST", jobz, n, kd, &ib, &c_n1);
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("CHBEVX_2STAGE", &i__1, (ftnlen)13);
    ...
    *m = 0;
    ...
    if(iscale == 1)
    {
        if(lower)
        {
            aocl_lapack_clascl("B", kd, kd, &c_b26, &sigma, n, n, &ab[ab_offset], ldab, info);
        }
        else
        {
            aocl_lapack_clascl("Q", kd, kd, &c_b26, &sigma, n, n, &ab[ab_offset], ldab, info);
        }
        if(*abstol > 0.f)
        {
            abstll = *abstol * sigma;
    ...
    aocl_lapack_chetrd_hb2st("N", jobz, uplo, n, kd, &ab[ab_offset], ldab, &rwork[indd], &rwork[inde],
