...
/* > \verbatim */
/* > */
/* > ZUNM22 overwrites the general scomplex M-by-N matrix C with */
/* > */
/* > SIDE = 'L' SIDE = 'R' */
/* > TRANS = 'N': Q * C C * Q */
/* > TRANS = 'C': Q**H * C C * Q**H */
...
void zunm22_(char *side, char *trans, aocl_int_t *m, aocl_int_t *n, aocl_int_t *n1, aocl_int_t *n2,
             dcomplex *q, aocl_int_t *ldq, dcomplex *c__, aocl_int_t *ldc,
             dcomplex *work, aocl_int_t *lwork, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_zunm22(side, trans, m, n, n1, n2, q, ldq, c__, ldc, work, lwork, info);
#else
    aocl_int64_t m_64 = *m;
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_zunm22(side, trans, &m_64, &n_64, &n1_64, &n2_64, q, &ldq_64, c__, &ldc_64, work,
                       &lwork_64, &info_64);
...
void aocl_lapack_zunm22(char *side, char *trans, aocl_int64_t *m, aocl_int64_t *n, aocl_int64_t *n1,
                        aocl_int64_t *n2, dcomplex *q, aocl_int64_t *ldq, dcomplex *c__,
                        ...
                        aocl_int64_t *info)
{
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("ZUNM22", &i__1, (ftnlen)6);
    ...
    if(left)
    {
        if(notran)
        {
            ...
            for(i__ = 1; i__2 < 0 ? i__ >= i__1 : i__ <= i__1; i__ += i__2)
            {
                ...
                aocl_lapack_zlacpy("All", n1, &len, &c__[*n2 + 1 + i__ * c_dim1], ldc, &work[1],
                                   &ldwork);
                aocl_blas_ztrmm("Left", "Lower", "No Transpose", "Non-Unit", n1, &len, &c_b1,
                                &q[(*n2 + 1) * q_dim1 + 1], ldq, &work[1], &ldwork);
                /* Multiply top part of C by Q11. */
                aocl_blas_zgemm("No Transpose", "No Transpose", n1, &len, n2, &c_b1, &q[q_offset],
                                ldq, &c__[i__ * c_dim1 + 1], ldc, &c_b1, &work[1], &ldwork);
                /* Multiply top part of C by Q21. */
                aocl_lapack_zlacpy("All", n2, &len, &c__[i__ * c_dim1 + 1], ldc, &work[*n1 + 1],
                                   &ldwork);
                aocl_blas_ztrmm("Left", "Upper", "No Transpose", "Non-Unit", n2, &len, &c_b1,
                                &q[*n1 + 1 + q_dim1], ldq, &work[*n1 + 1], &ldwork);
                /* Multiply bottom part of C by Q22. */
                aocl_blas_zgemm("No Transpose", "No Transpose", n2, &len, n1, &c_b1,
                                &q[*n1 + 1 + (*n2 + 1) * q_dim1], ldq, &c__[*n2 + 1 + i__ * c_dim1],
                                ldc, &c_b1, &work[*n1 + 1], &ldwork);
                /* Copy everything back. */
                aocl_lapack_zlacpy("All", m, &len, &work[1], &ldwork, &c__[i__ * c_dim1 + 1], ldc);
            }
        }
        else
        {
            ...
            for(i__ = 1; i__1 < 0 ? i__ >= i__2 : i__ <= i__2; i__ += i__1)
            {
                ...
                aocl_lapack_zlacpy("All", n2, &len, &c__[*n1 + 1 + i__ * c_dim1], ldc, &work[1],
                                   &ldwork);
                aocl_blas_ztrmm("Left", "Upper", "Conjugate", "Non-Unit", n2, &len, &c_b1,
                                &q[*n1 + 1 + q_dim1], ldq, &work[1], &ldwork);
                /* Multiply top part of C by Q11**H. */
                aocl_blas_zgemm("Conjugate", "No Transpose", n2, &len, n1, &c_b1, &q[q_offset], ldq,
                                &c__[i__ * c_dim1 + 1], ldc, &c_b1, &work[1], &ldwork);
                /* Multiply top part of C by Q12**H. */
                aocl_lapack_zlacpy("All", n1, &len, &c__[i__ * c_dim1 + 1], ldc, &work[*n2 + 1],
                                   &ldwork);
                aocl_blas_ztrmm("Left", "Lower", "Conjugate", "Non-Unit", n1, &len, &c_b1,
                                &q[(*n2 + 1) * q_dim1 + 1], ldq, &work[*n2 + 1], &ldwork);
                /* Multiply bottom part of C by Q22**H. */
                aocl_blas_zgemm("Conjugate", "No Transpose", n1, &len, n2, &c_b1,
    ...
    else
    {
        if(notran)
        {
            ...
            for(i__ = 1; i__2 < 0 ? i__ >= i__1 : i__ <= i__1; i__ += i__2)
            {
                ...
                aocl_lapack_zlacpy("All", &len, n2, &c__[i__ + (*n1 + 1) * c_dim1], ldc, &work[1],
                                   &ldwork);
                aocl_blas_ztrmm("Right", "Upper", "No Transpose", "Non-Unit", &len, n2, &c_b1,
                                &q[*n1 + 1 + q_dim1], ldq, &work[1], &ldwork);
                /* Multiply left part of C by Q11. */
                aocl_blas_zgemm("No Transpose", "No Transpose", &len, n2, n1, &c_b1,
                                &c__[i__ + c_dim1], ldc, &q[q_offset], ldq, &c_b1, &work[1],
                                &ldwork);
                /* Multiply left part of C by Q12. */
                aocl_lapack_zlacpy("All", &len, n1, &c__[i__ + c_dim1], ldc,
                                   &work[*n2 * ldwork + 1], &ldwork);
                aocl_blas_ztrmm("Right", "Lower", "No Transpose", "Non-Unit", &len, n1, &c_b1,
                                &q[(*n2 + 1) * q_dim1 + 1], ldq, &work[*n2 * ldwork + 1], &ldwork);
                /* Multiply right part of C by Q22. */
                aocl_blas_zgemm("No Transpose", "No Transpose", &len, n1, n2, &c_b1,
                                &c__[i__ + (*n1 + 1) * c_dim1], ldc,
                                &q[*n1 + 1 + (*n2 + 1) * q_dim1], ldq, &c_b1,
                                &work[*n2 * ldwork + 1], &ldwork);
                /* Copy everything back. */
                aocl_lapack_zlacpy("All", &len, n, &work[1], &ldwork, &c__[i__ + c_dim1], ldc);
            }
        }
        else
        {
            ...
            for(i__ = 1; i__1 < 0 ? i__ >= i__2 : i__ <= i__2; i__ += i__1)
            {
                ...
                aocl_lapack_zlacpy("All", &len, n1, &c__[i__ + (*n2 + 1) * c_dim1], ldc, &work[1],
                                   &ldwork);
                aocl_blas_ztrmm("Right", "Lower", "Conjugate", "Non-Unit", &len, n1, &c_b1,
                                &q[(*n2 + 1) * q_dim1 + 1], ldq, &work[1], &ldwork);
                /* Multiply left part of C by Q11**H. */
                aocl_blas_zgemm("No Transpose", "Conjugate", &len, n1, n2, &c_b1,
                                &c__[i__ + c_dim1], ldc, &q[q_offset], ldq, &c_b1, &work[1],
                                &ldwork);
                /* Multiply left part of C by Q21**H. */
                aocl_lapack_zlacpy("All", &len, n2, &c__[i__ + c_dim1], ldc,
                                   &work[*n1 * ldwork + 1], &ldwork);
                aocl_blas_ztrmm("Right", "Upper", "Conjugate", "Non-Unit", &len, n2, &c_b1,
                                &q[*n1 + 1 + q_dim1], ldq, &work[*n1 * ldwork + 1], &ldwork);
                /* Multiply right part of C by Q22**H. */
                aocl_blas_zgemm("No Transpose", "Conjugate", &len, n2, n1, &c_b1,
