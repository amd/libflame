...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/dbbcsd.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/dbbcsd.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/dbbcsd.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
void dbbcsd_(char *jobu1, char *jobu2, char *jobv1t, char *jobv2t, char *trans, aocl_int_t *m,
             aocl_int_t *p, aocl_int_t *q, doublereal *theta, doublereal *phi, doublereal *u1,
             aocl_int_t *ldu1, doublereal *u2, aocl_int_t *ldu2, doublereal *v1t, aocl_int_t *ldv1t,
             doublereal *v2t, aocl_int_t *ldv2t, doublereal *b11d, doublereal *b11e,
                        ...
                        doublereal *b22e, doublereal *work, aocl_int64_t *lwork, aocl_int64_t *info)
{
    ...
    *info = 0;
    ...
    while(imax > 1)
    {
        ...
        {
            dlartgp_(&b11bulge, &b11d[imin], &work[iu1sn + imin - 1], &work[iu1cs + imin - 1],
                     &r__);
        }
        else if(mu <= nu)
        {
            dlartgs_(&b11e[imin], &b11d[imin + 1], &mu, &work[iu1cs + imin - 1],
                     &work[iu1sn + imin - 1]);
        }
        else
        {
        ...
        if(d__1 * d__1 + d__2 * d__2 > d__3 * d__3)
        {
            dlartgp_(&b21bulge, &b21d[imin], &work[iu2sn + imin - 1], &work[iu2cs + imin - 1],
                     &r__);
        }
        else if(nu < mu)
        {
            dlartgs_(&b21e[imin], &b21d[imin + 1], &nu, &work[iu2cs + imin - 1],
                     &work[iu2sn + imin - 1]);
        }
        else
        {
        ...
        for(i__ = imin + 1; i__ <= i__1; ++i__)
        {
            ...
            {
                dlartgs_(&b12e[i__ - 1], &b12d[i__], &nu, &work[iv2tcs + i__ - 2],
                         &work[iv2tsn + i__ - 2]);
            }
            else
            {
                dlartgs_(&b22e[i__ - 1], &b22d[i__], &mu, &work[iv2tcs + i__ - 2],
                         &work[iv2tsn + i__ - 2]);
            }
            ...
            {
                dlartgp_(&b11bulge, &b11d[i__], &work[iu1sn + i__ - 1], &work[iu1cs + i__ - 1],
                         &r__);
            }
            else if(restart11 && !restart12)
            {
                dlartgp_(&b12bulge, &b12e[i__ - 1], &work[iu1sn + i__ - 1], &work[iu1cs + i__ - 1],
                         &r__);
            }
            else if(mu <= nu)
            {
                dlartgs_(&b11e[i__], &b11d[i__ + 1], &mu, &work[iu1cs + i__ - 1],
                         &work[iu1sn + i__ - 1]);
            }
            else
            {
            ...
            {
                dlartgp_(&b21bulge, &b21d[i__], &work[iu2sn + i__ - 1], &work[iu2cs + i__ - 1],
                         &r__);
            }
            else if(restart21 && !restart22)
            {
                dlartgp_(&b22bulge, &b22e[i__ - 1], &work[iu2sn + i__ - 1], &work[iu2cs + i__ - 1],
                         &r__);
            }
            else if(nu < mu)
            {
                dlartgs_(&b21e[i__], &b21e[i__ + 1], &nu, &work[iu2cs + i__ - 1],
                         &work[iu2sn + i__ - 1]);
            }
            else
            {
        ...
        {
            dlartgp_(&b12bulge, &b12d[imax - 1], &work[iv2tsn + imax - 2], &work[iv2tcs + imax - 2],
                     &r__);
        }
        else if(restart12 && !restart22)
        {
            dlartgp_(&b22bulge, &b22d[imax - 1], &work[iv2tsn + imax - 2], &work[iv2tcs + imax - 2],
                     &r__);
        }
        else if(nu < mu)
        {
            dlartgs_(&b12e[imax - 1], &b12d[imax], &nu, &work[iv2tcs + imax - 2],
                     &work[iv2tsn + imax - 2]);
        }
        else
        {
            dlartgs_(&b22e[imax - 1], &b22d[imax], &mu, &work[iv2tcs + imax - 2],
                     &work[iv2tsn + imax - 2]);
        }
        ...
        if(b11e[imax - 1] + b21e[imax - 1] > 0.)
        {
            ...
            if(wantv1t)
            {
                if(colmajor)
                {
                    aocl_blas_dscal(q, &c_b35, &v1t[imax + v1t_dim1], ldv1t);
                }
                else
                {
                    aocl_blas_dscal(q, &c_b35, &v1t[imax * v1t_dim1 + 1], &c__1);
                }
            }
        }
        /* Compute THETA(IMAX) */
        ...
        if(b11d[imax] + b12e[imax - 1] < 0.)
        {
            ...
            if(wantu1)
            {
                if(colmajor)
                {
                    aocl_blas_dscal(p, &c_b35, &u1[imax * u1_dim1 + 1], &c__1);
                }
                else
                {
                    aocl_blas_dscal(p, &c_b35, &u1[imax + u1_dim1], ldu1);
                }
            }
        }
        if(b21d[imax] + b22e[imax - 1] > 0.)
        {
            ...
            if(wantu2)
            {
                ...
                {
                    i__1 = *m - *p;
                    aocl_blas_dscal(&i__1, &c_b35, &u2[imax * u2_dim1 + 1], &c__1);
                }
                else
                {
                    i__1 = *m - *p;
                    aocl_blas_dscal(&i__1, &c_b35, &u2[imax + u2_dim1], ldu2);
                }
            }
        }
        ...
        if(b12d[imax] + b22d[imax] < 0.)
        {
            if(wantv2t)
            {
                ...
                {
                    i__1 = *m - *q;
                    aocl_blas_dscal(&i__1, &c_b35, &v2t[imax + v2t_dim1], ldv2t);
                }
                else
                {
                    i__1 = *m - *q;
                    aocl_blas_dscal(&i__1, &c_b35, &v2t[imax * v2t_dim1 + 1], &c__1);
                }
            }
        }
        /* Test for negligible sines or cosines */
    ...
    for(i__ = 1; i__ <= i__1; ++i__)
    {
        ...
        if(mini != i__)
        {
            ...
            if(colmajor)
            {
                if(wantu1)
                {
                    aocl_blas_dswap(p, &u1[i__ * u1_dim1 + 1], &c__1, &u1[mini * u1_dim1 + 1],
                                    &c__1);
                }
                if(wantu2)
                {
                    i__2 = *m - *p;
                    aocl_blas_dswap(&i__2, &u2[i__ * u2_dim1 + 1], &c__1, &u2[mini * u2_dim1 + 1],
                                    &c__1);
                }
                if(wantv1t)
                {
                    aocl_blas_dswap(q, &v1t[i__ + v1t_dim1], ldv1t, &v1t[mini + v1t_dim1], ldv1t);
                }
                if(wantv2t)
                {
                    i__2 = *m - *q;
                    aocl_blas_dswap(&i__2, &v2t[i__ + v2t_dim1], ldv2t, &v2t[mini + v2t_dim1],
                                    ldv2t);
                }
            }
            else
            {
                if(wantu1)
                {
                    aocl_blas_dswap(p, &u1[i__ + u1_dim1], ldu1, &u1[mini + u1_dim1], ldu1);
                }
                if(wantu2)
                {
                    i__2 = *m - *p;
                    aocl_blas_dswap(&i__2, &u2[i__ + u2_dim1], ldu2, &u2[mini + u2_dim1], ldu2);
                }
                if(wantv1t)
                {
                    aocl_blas_dswap(q, &v1t[i__ * v1t_dim1 + 1], &c__1, &v1t[mini * v1t_dim1 + 1],
                                    &c__1);
                }
                if(wantv2t)
                {
