                 ...
                 float *work, aocl_int64_t *lwork, aocl_int64_t *info)
{
    ...
    aocl_int64_t iinfo;
    logical wantq;
    extern int sorglq_check(aocl_int64_t *, aocl_int64_t *, aocl_int64_t *, float *, aocl_int64_t *, float *, float *,
                            aocl_int64_t *, aocl_int64_t *),
        sorgqr_check(aocl_int64_t *, aocl_int64_t *, aocl_int64_t *, float *, aocl_int64_t *, float *, float *,
                     aocl_int64_t *, aocl_int64_t *);
    aocl_int64_t lwkopt;
    logical lquery;
        ...
        if(wantq)
        {
            ...
            else
            {
                if(*m > 1)
                {
                    ...
                    i__2 = *m - 1;
                    i__3 = *m - 1;
                    sorgqr_check(&i__1, &i__2, &i__3, &a[(a_dim1 << 1) + 2], lda, &tau[1], &work[1],
                                 &c_n1, &iinfo);
                }
            }
        }
        else
        {
            ...
            else
            {
                if(*n > 1)
                {
                    ...
                    i__2 = *n - 1;
                    i__3 = *n - 1;
                    sorglq_check(&i__1, &i__2, &i__3, &a[(a_dim1 << 1) + 2], lda, &tau[1], &work[1],
                                 &c_n1, &iinfo);
                }
            }
        }
