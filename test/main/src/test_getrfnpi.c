...
void fla_test_getrfnpi(integer argc, char **argv, test_params_t *params)
{
    ...
    if(tests_not_run)
    {
        ...
        printf("./<EXE> getrfnpi <precisions - sdcz> <M> <N> <nfact> <LDA> <repeats>\n");
    }
    if(invalid_dtype)
    {
...
/*
 *  GETRFNPI_API calls LAPACK interface of
 *  */
void invoke_getrfnpi(integer datatype, integer *m, integer *n, integer *nfact, void *a,
                     integer *lda, integer *info)
{
    switch(datatype)
    {
        case FLOAT:
        {
            fla_lapack_sgetrfnpi(m, n, nfact, a, lda, info);
            break;
        }

        case DOUBLE:
        {
            fla_lapack_dgetrfnpi(m, n, nfact, a, lda, info);
            break;
        }

        case COMPLEX:
        {
            fla_lapack_cgetrfnpi(m, n, nfact, a, lda, info);
            break;
        }

        case DOUBLE_COMPLEX:
        {
            fla_lapack_zgetrfnpi(m, n, nfact, a, lda, info);
            break;
        }
    }
}
