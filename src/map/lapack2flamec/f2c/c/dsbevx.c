/* ../netlib/dsbevx.f -- translated by f2c (version 20100827). You must link the resulting object
...
/* > \brief <b> DSBEVX computes the eigenvalues and, optionally, the left and/or right eigenvectors
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/dsbevx.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/dsbevx.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/dsbevx.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
{
#if FLA_ENABLE_ILP64
    aocl_lapack_dsbevx(jobz, range, uplo, n, kd, ab, ldab, q, ldq, vl, vu, il, iu, abstol, m, w,
                       z__, ldz, work, iwork, ifail, info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_dsbevx(jobz, range, uplo, &n_64, &kd_64, ab, &ldab_64, q, &ldq_64, vl, vu, &il_64,
                       &iu_64, abstol, &m_64, w, z__, &ldz_64, work, iwork, ifail, &info_64);
...
void aocl_lapack_dsbevx(char *jobz, char *range, char *uplo, aocl_int64_t *n, aocl_int64_t *kd,
                        doublereal *ab, aocl_int64_t *ldab, doublereal *q, aocl_int64_t *ldq,
                        ...
                        aocl_int64_t *info)
{
    ...
    AOCL_DTL_SNPRINTF("dsbevx inputs: jobz %c, range %c, uplo %c, n %" FLA_IS ", kd %" FLA_IS
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("DSBEVX", &i__1, (ftnlen)6);
    ...
    *m = 0;
    ...
    if(iscale == 1)
    {
        if(lower)
        {
            aocl_lapack_dlascl("B", kd, kd, &c_b14, &sigma, n, n, &ab[ab_offset], ldab, info);
        }
        else
        {
            aocl_lapack_dlascl("Q", kd, kd, &c_b14, &sigma, n, n, &ab[ab_offset], ldab, info);
        }
        if(*abstol > 0.)
        {
            abstll = *abstol * sigma;
    ...
    inde = indd + *n;
    indwrk = inde + *n;
    aocl_lapack_dsbtrd(jobz, uplo, n, kd, &ab[ab_offset], ldab, &work[indd], &work[inde],
    ...
    if(wantz)
    {
        ...
        for(j = 1; j <= i__1; ++j)
        {
            ...
            aocl_blas_dgemv("N", n, n, &c_b14, &q[q_offset], ldq, &work[1], &c__1, &c_b34,
                            &z__[j * z_dim1 + 1], &c__1);
            /* L20: */
        }
    }
