/* ../netlib/v3.9.0/cgesvdq.f -- translated by f2c (version 20160102). You must link the resulting
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/cgesvdq
 * .f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/cgesvdq
 * .f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/cgesvdq
 * .f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
#if FLA_ENABLE_ILP64
    snprintf(buffer, 256,
             "cgesvdq inputs: joba %c, jobp %c, jobr %c, jobu %c, jobv %c, m %lld, n %lld, lda "
             "%lld, ldu %lld, ldv %lld, liwork %lld, lrwork %lld",
             *joba, *jobp, *jobr, *jobu, *jobv, *m, *n, *lda, *ldu, *ldv, *liwork, *lrwork);
#else
    snprintf(buffer, 256,
             "cgesvdq inputs: joba %c, jobp %c, jobr %c, jobu %c, jobv %c, m %d, n %d, lda %d, ldu "
             "%d, ldv %d, liwork %d, lrwork %d",
             *joba, *jobp, *jobr, *jobu, *jobv, *m, *n, *lda, *ldu, *ldv, *liwork, *lrwork);
#endif
    ...
    if(*info == 0)
    {
        ...
        if(!(lsvec || rsvec))
        {
            ...
            if(lquery)
            {
                ...
                lwrk_cgesvd__ = (integer)cdummy[0].real;
                if(conda)
                {
                    /* Computing MAX */
    ...
    else if(lsvec && !rsvec)
    {
        ...
        if(rtrans)
        {
            ...
            for(p = 1; p <= i__1; ++p)
            {
                ...
                u[i__2].real = q__1.real;
                u[i__2].imag = q__1.imag; // , expr subst
                i__2 = nr;
                for(q = p + 1; q <= i__2; ++q)
                {
                    ...
                    u[i__3].imag = q__1.imag; // , expr subst
                    i__3 = p + q * u_dim1;
                    u[i__3].real = ctmp.real;
                    u[i__3].imag = ctmp.imag; // , expr subst
                    /* L1120: */
                }
                /* L1119: */
            }
            ...
            if(wntvr || nr == *n)
            {
                ...
                for(p = 1; p <= i__1; ++p)
                {
                    ...
                    for(q = p + 1; q <= i__2; ++q)
                    {
                        ...
                        v[i__3].imag = q__1.imag; // , expr subst
                        i__3 = p + q * v_dim1;
                        v[i__3].real = ctmp.real;
                        v[i__3].imag = ctmp.imag; // , expr subst
                        /* L1122: */
                    }
                    /* L1121: */
                }
