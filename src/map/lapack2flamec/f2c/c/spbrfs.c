/* ../netlib/spbrfs.f -- translated by f2c (version 20100827). You must link the resulting object
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/spbrfs.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/spbrfs.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/spbrfs.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
void spbrfs_(char *uplo, aocl_int_t *n, aocl_int_t *kd, aocl_int_t *nrhs, real *ab,
             aocl_int_t *ldab, real *afb, aocl_int_t *ldafb, real *b, aocl_int_t *ldb, real *x,
             aocl_int_t *ldx, real *ferr, real *berr, real *work, aocl_int_t *iwork,
             aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_spbrfs(uplo, n, kd, nrhs, ab, ldab, afb, ldafb, b, ldb, x, ldx, ferr, berr, work,
                       iwork, info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_spbrfs(uplo, &n_64, &kd_64, &nrhs_64, ab, &ldab_64, afb, &ldafb_64, b, &ldb_64, x,
...
void aocl_lapack_spbrfs(char *uplo, aocl_int64_t *n, aocl_int64_t *kd, aocl_int64_t *nrhs, real *ab,
                        aocl_int64_t *ldab, real *afb, aocl_int64_t *ldafb, real *b,
                        aocl_int64_t *ldb, real *x, aocl_int64_t *ldx, real *ferr, real *berr,
                        real *work, aocl_int_t *iwork, aocl_int64_t *info)
{
    AOCL_DTL_TRACE_LOG_INIT
    AOCL_DTL_SNPRINTF("spbrfs inputs: uplo %c, n %" FLA_IS ", kd %" FLA_IS ", nrhs %" FLA_IS
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("SPBRFS", &i__1, (ftnlen)6);
    ...
    L20: /* Loop until stopping criterion is satisfied. */
        ...
        aocl_blas_ssbmv(uplo, n, kd, &c_b12, &ab[ab_offset], ldab, &x[j * x_dim1 + 1], &c__1,
                        &c_b14, &work[*n + 1], &c__1);
        ...
        if(upper)
        {
            ...
            for(k = 1; k <= i__2; ++k)
            {
                ...
                for(i__ = fla_max(i__3, i__4); i__ <= i__5; ++i__)
                {
                    ...
                    s += (r__1 = ab[l + i__ + k * ab_dim1], f2c_abs(r__1))
        ...
        if(berr[j] > eps && berr[j] * 2.f <= lstres && count <= 5)
        {
            ...
            aocl_lapack_spbtrs(uplo, n, kd, &c__1, &afb[afb_offset], ldafb, &work[*n + 1], n, info);
    ...
    L100:
        ...
        if(kase != 0)
        {
            if(kase == 1)
            {
                ...
                aocl_lapack_spbtrs(uplo, n, kd, &c__1, &afb[afb_offset], ldafb, &work[*n + 1], n,
                                   info);
                i__2 = *n;
                for(i__ = 1; i__ <= i__2; ++i__)
                {
