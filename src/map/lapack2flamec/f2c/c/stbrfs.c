/* ../netlib/stbrfs.f -- translated by f2c (version 20100827). You must link the resulting object
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/stbrfs.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/stbrfs.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/stbrfs.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
                        ...
                        real *work, aocl_int_t *iwork, aocl_int64_t *info)
{
    ...
    *info = 0;
    ...
    for(j = 1; j <= i__1; ++j)
    {
        ...
        aocl_blas_stbmv(uplo, trans, diag, n, kd, &ab[ab_offset], ldab, &work[*n + 1], &c__1);
            ...
            if(upper)
            {
                if(nounit)
                {
                    ...
                    for(k = 1; k <= i__2; ++k)
                    {
                        ...
                        {
                            work[i__]
                                += (r__1 = ab[*kd + 1 + i__ - k + k * ab_dim1], f2c_abs(r__1)) * xk;
                            /* L30: */
                        }
                        /* L40: */
                    }
            ...
            if(upper)
            {
                if(nounit)
                {
                    ...
                    for(k = 1; k <= i__2; ++k)
                    {
                        ...
                        {
                            s += (r__1 = ab[*kd + 1 + i__ - k + k * ab_dim1], f2c_abs(r__1))
            ...
            else
            {
                if(nounit)
                {
                    ...
                    for(k = 1; k <= i__2; ++k)
                    {
                        ...
                        {
                            s += (r__1 = ab[i__ + 1 - k + k * ab_dim1], f2c_abs(r__1))
                                 * (r__2 = x[i__ + j * x_dim1], f2c_abs(r__2));
                            /* L150: */
                        }
                        work[k] += s;
