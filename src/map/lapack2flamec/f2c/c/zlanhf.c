/* ../netlib/zlanhf.f -- translated by f2c (version 20160102). You must link the resulting object
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/zlanhf.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/zlanhf.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/zlanhf.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
                              ...
                              dcomplex *a, doublereal *work)
{
    ...
    {
        ret_val = (d__1 = a[0].real, f2c_abs(d__1));
        AOCL_DTL_TRACE_LOG_EXIT
        return ret_val;
    }
        ...
        if(noe == 1)
        {
            ...
            if(ifm == 1)
            {
                ...
                if(ilu == 1)
                {
                    ...
                    /* -> L(0,0) */
                    i__1 = j + j * lda;
                    temp = (d__1 = a[i__1].real, f2c_abs(d__1));
                    ...
                    for(j = 1; j <= i__1; ++j)
                    {
                        ...
                        /* L(k+j,k+j) */
                        i__2 = i__ + j * lda;
                        temp = (d__1 = a[i__2].real, f2c_abs(d__1));
                        if(value < temp || disnan_(&temp))
                        {
                            value = temp;
                        }
            ...
            if(noe == 1)
            {
                ...
                if(ilu == 0)
                {
                    ...
                    for(j = 0; j <= i__1; ++j)
                    {
                        ...
                        i__2 = i__ + j * lda;
                        aa = (d__1 = a[i__2].real, f2c_abs(d__1));
                        /* -> A(j+k,j+k) */
                        work[j + k] = s + aa;
                        if(i__ == k + k)
                        {
                ...
                else
                {
                    ...
                    for(j = k - 1; j >= 0; --j)
                    {
                        ...
                        {
                            i__1 = i__ + j * lda;
                            aa = (d__1 = a[i__1].real, f2c_abs(d__1));
                            /* -> A(j+k,j+k) */
                            s += aa;
                            work[i__ + k] += s;
                            /* i=j */
        ...
        else
        {
            ...
            if(noe == 1)
            {
                ...
                if(ilu == 0)
                {
                    ...
                    /* j=n1=k-1 is special */
                    i__1 = j * lda;
                    s = (d__1 = a[i__1].real, f2c_abs(d__1));
                    /* A(k-1,k-1) */
                    i__1 = k - 1;
                    for(i__ = 1; i__ <= i__1; ++i__)
                    {
                    ...
                    for(j = k; j <= i__1; ++j)
                    {
                        ...
                        ++i__;
                        i__2 = i__ + j * lda;
                        s = (d__1 = a[i__2].real, f2c_abs(d__1));
                        /* A(j,j) */
                        i__2 = *n - 1;
                        for(l = j + 1; l <= i__2; ++l)
                        {
