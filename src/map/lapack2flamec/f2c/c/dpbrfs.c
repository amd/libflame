/* ../netlib/dpbrfs.f -- translated by f2c (version 20100827). You must link the resulting object
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/dpbrfs.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/dpbrfs.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/dpbrfs.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
{
#if FLA_ENABLE_ILP64
    aocl_lapack_dpbrfs(uplo, n, kd, nrhs, ab, ldab, afb, ldafb, b, ldb, x, ldx, ferr, berr, work,
                       iwork, info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_dpbrfs(uplo, &n_64, &kd_64, &nrhs_64, ab, &ldab_64, afb, &ldafb_64, b, &ldb_64, x,
...
void aocl_lapack_dpbrfs(char *uplo, aocl_int64_t *n, aocl_int64_t *kd, aocl_int64_t *nrhs,
                        doublereal *ab, aocl_int64_t *ldab, doublereal *afb, aocl_int64_t *ldafb,
                        ...
                        aocl_int64_t *info)
{
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("DPBRFS", &i__1, (ftnlen)6);
    ...
    L20: /* Loop until stopping criterion is satisfied. */
        ...
        if(berr[j] > eps && berr[j] * 2. <= lstres && count <= 5)
        {
            ...
            aocl_lapack_dpbtrs(uplo, n, kd, &c__1, &afb[afb_offset], ldafb, &work[*n + 1], n, info);
    ...
    L100:
        ...
        if(kase != 0)
        {
            if(kase == 1)
            {
                ...
                aocl_lapack_dpbtrs(uplo, n, kd, &c__1, &afb[afb_offset], ldafb, &work[*n + 1], n,
                                   info);
                i__2 = *n;
                for(i__ = 1; i__ <= i__2; ++i__)
                {
