...
/* > \verbatim */
/* > */
/* > ZUNMQL overwrites the general scomplex M-by-N matrix C with */
/* > */
/* > SIDE = 'L' SIDE = 'R' */
/* > TRANS = 'N': Q * C C * Q */
/* > TRANS = 'C': Q**H * C C * Q**H */
...
void zunmql_(char *side, char *trans, aocl_int_t *m, aocl_int_t *n, aocl_int_t *k, dcomplex *a,
             aocl_int_t *lda, dcomplex *tau, dcomplex *c__, aocl_int_t *ldc,
             dcomplex *work, aocl_int_t *lwork, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_zunmql(side, trans, m, n, k, a, lda, tau, c__, ldc, work, lwork, info);
#else
    aocl_int64_t m_64 = *m;
    aocl_int64_t n_64 = *n;
    aocl_int64_t k_64 = *k;
    ...
    aocl_lapack_zunmql(side, trans, &m_64, &n_64, &k_64, a, &lda_64, tau, c__, &ldc_64, work,
                       &lwork_64, &info_64);
...
void aocl_lapack_zunmql(char *side, char *trans, aocl_int64_t *m, aocl_int64_t *n, aocl_int64_t *k,
    ...
    *info = 0;
    ...
    if(*info == 0)
    {
        ...
        else
        {
            /* Computing MIN */
            i__1 = 64;
            i__2 = aocl_lapack_ilaenv(&c__1, "ZUNMQL", ch__1, m, n, k, &c_n1); // , expr subst
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("ZUNMQL", &i__1, (ftnlen)6);
    ...
    if(nb > 1 && nb < *k)
    {
        if(*lwork < nw * nb + 4160)
        {
            ...
            /* Computing MAX */
            i__1 = 2;
            i__2 = aocl_lapack_ilaenv(&c__2, "ZUNMQL", ch__1, m, n, k, &c_n1); // , expr subst
    ...
    {
        /* Use unblocked code */
        aocl_lapack_zunm2l(side, trans, m, n, k, &a[a_offset], lda, &tau[1], &c__[c_offset], ldc,
                           &work[1], &iinfo);
    }
    else
    {
        ...
        for(i__ = i1; i__2 < 0 ? i__ >= i__1 : i__ <= i__1; i__ += i__2)
        {
            ...
            i__4 = nq - *k + i__ + ib - 1;
            aocl_lapack_zlarft("Backward", "Columnwise", &i__4, &ib, &a[i__ * a_dim1 + 1], lda,
            ...
            /* Apply H or H**H */
            aocl_lapack_zlarfb(side, trans, "Backward", "Columnwise", &mi, &ni, &ib,
