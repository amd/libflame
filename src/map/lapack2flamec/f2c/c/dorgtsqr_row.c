...
void dorgtsqr_row_(aocl_int_t *m, aocl_int_t *n, aocl_int_t *mb, aocl_int_t *nb, doublereal *a,
                   aocl_int_t *lda, doublereal *t, aocl_int_t *ldt, doublereal *work,
                   aocl_int_t *lwork, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_dorgtsqr_row(m, n, mb, nb, a, lda, t, ldt, work, lwork, info);
#else
    aocl_int64_t m_64 = *m;
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_dorgtsqr_row(&m_64, &n_64, &mb_64, &nb_64, a, &lda_64, t, &ldt_64, work, &lwork_64,
                             &info_64);
...
void aocl_lapack_dorgtsqr_row(aocl_int64_t *m, aocl_int64_t *n, aocl_int64_t *mb, aocl_int64_t *nb,
                              ...
                              doublereal *work, aocl_int64_t *lwork, aocl_int64_t *info)
{
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("DORGTSQR_ROW", &i__1, (ftnlen)12);
    ...
    aocl_lapack_dlaset("U", m, n, &c_b4, &c_b5, &a[a_offset], lda);
    ...
    if(*mb < *m)
    {
        ...
        for(ib = ib_bottom__; i__2 < 0 ? ib >= i__1 : ib <= i__1; ib += i__2)
        {
            ...
            for(kb = kb_last__; i__3 < 0 ? kb >= 1 : kb <= 1; kb += i__3)
            {
                ...
                i__4 = *n - kb + 1;
                aocl_lapack_dlarfb_gett("I", &imb, &i__4, &knb, &t[(jb_t__ + kb - 1) * t_dim1 + 1],
    ...
    for(kb = kb_last__; i__2 < 0 ? kb >= 1 : kb <= 1; kb += i__2)
    {
        ...
        if(mb1 - kb - knb + 1 == 0)
        {
            ...
            i__1 = *n - kb + 1;
            aocl_lapack_dlarfb_gett("N", &c__0, &i__1, &knb, &t[kb * t_dim1 + 1], ldt,
        ...
        else
        {
            i__1 = mb1 - kb - knb + 1;
            i__3 = *n - kb + 1;
            aocl_lapack_dlarfb_gett("N", &i__1, &i__3, &knb, &t[kb * t_dim1 + 1], ldt,
