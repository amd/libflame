/* slaqr0.f -- translated by f2c (version 20190311). You must link the resulting object file with
...
/* > \brief \b SLAQR0 computes the eigenvalues of a Hessenberg matrix, and optionally the matrices
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/slaqr0.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/slaqr0.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/slaqr0.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
void slaqr0_(logical *wantt, logical *wantz, aocl_int_t *n, aocl_int_t *ilo, aocl_int_t *ihi,
...
{
#if FLA_ENABLE_ILP64
    aocl_lapack_slaqr0(wantt, wantz, n, ilo, ihi, h__, ldh, wr, wi, iloz, ihiz, z__, ldz, work,
                       lwork, info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_slaqr0(wantt, wantz, &n_64, &ilo_64, &ihi_64, h__, &ldh_64, wr, wi, &iloz_64,
...
void aocl_lapack_slaqr0(logical *wantt, logical *wantz, aocl_int64_t *n, aocl_int64_t *ilo,
                        ...
                        real *work, aocl_int64_t *lwork, aocl_int64_t *info)
{
    ...
    AOCL_DTL_SNPRINTF("slaqr0 inputs: n %" FLA_IS ", ilo %" FLA_IS ", ihi %" FLA_IS ", ldh %" FLA_IS
    ...
    *info = 0;
    ...
    if(*n <= 15)
    {
        ...
        if(*lwork != -1)
        {
            aocl_lapack_slahqr(wantt, wantz, n, ilo, ihi, &h__[h_offset], ldh, &wr[1], &wi[1], iloz,
    ...
    else
    {
        ...
        *info = 0;
        ...
        /* . NWR.GE.4.) ==== */
        nwr = aocl_lapack_ilaenv(&c__13, "SLAQR0", jbcmpz, n, ilo, ihi, lwork);
        nwr = fla_max(2, nwr);
        /* Computing MIN */
        i__1 = *ihi - *ilo + 1;
        i__2 = (*n - 1) / 3;
        ...
        nsr = aocl_lapack_ilaenv(&c__15, "SLAQR0", jbcmpz, n, ilo, ihi, lwork);
        /* Computing MIN */
        i__1 = nsr, i__2 = (*n - 3) / 6;
        ...
        i__1 = nwr + 1;
        aocl_lapack_slaqr3(wantt, wantz, n, ilo, ihi, &i__1, &h__[h_offset], ldh, iloz, ihiz,
        ...
        nmin = aocl_lapack_ilaenv(&c__12, "SLAQR0", jbcmpz, n, ilo, ihi, lwork);
        nmin = fla_max(15, nmin);
        /* ==== Nibble crossover point ==== */
        nibble = aocl_lapack_ilaenv(&c__14, "SLAQR0", jbcmpz, n, ilo, ihi, lwork);
        nibble = fla_max(0, nibble);
        /* ==== Accumulate reflections during ttswp? Use block */
        /* . 2-by-2 structure during matrix-matrix multiply? ==== */
        kacc22 = aocl_lapack_ilaenv(&c__16, "SLAQR0", jbcmpz, n, ilo, ihi, lwork);
        ...
        L20:
            ...
            nve = *n - nw - kwv + 1;
            /* ==== Aggressive early deflation ==== */
            aocl_lapack_slaqr3(wantt, wantz, n, &ktop, &kbot, &nw, &h__[h_offset], ldh, iloz, ihiz,
            ...
            if(ld == 0 || ld * 100 <= nw * nibble && kbot - ktop + 1 > fla_min(nmin, nwmax))
            {
                ...
                else
                {
                    ...
                    if(kbot - ks + 1 <= ns / 2)
                    {
                        ks = kbot - ns + 1;
                        kt = *n - ns + 1;
                        aocl_lapack_slacpy("A", &ns, &ns, &h__[ks + ks * h_dim1], ldh,
                                           &h__[kt + h_dim1], ldh);
                        if(ns > nmin)
                        {
                            aocl_lapack_slaqr4(&c_false, &c_false, &ns, &c__1, &ns,
                                               &h__[kt + h_dim1], ldh, &wr[ks], &wi[ks], &c__1,
                                               &c__1, zdum, &c__1, &work[1], lwork, &inf);
                        }
                        else
                        {
                            aocl_lapack_slahqr(&c_false, &c_false, &ns, &c__1, &ns,
                                               &h__[kt + h_dim1], ldh, &wr[ks], &wi[ks], &c__1,
                                               &c__1, zdum, &c__1, &inf);
                        }
                        ks += inf;
                ...
                nve = *n - kdu - kwv + 1;
                /* ==== Small-bulge multi-shift QR sweep ==== */
                aocl_lapack_slaqr5(wantt, wantz, &kacc22, n, &ktop, &kbot, &ns, &wr[ks], &wi[ks],
