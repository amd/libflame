/* ../netlib/v3.9.0/dorhr_col.f -- translated by f2c (version 20160102). You must link the resulting
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/dorhr_c
 * ol.f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/dorhr_c
 * ol.f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/dorhr_c
 * ol.f"> */
/* > [TXT]</a> */
/* > */
/* Definition: */
...
void dorhr_col_(aocl_int_t *m, aocl_int_t *n, aocl_int_t *nb, doublereal *a, aocl_int_t *lda,
                doublereal *t, aocl_int_t *ldt, doublereal *d__, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_dorhr_col(m, n, nb, a, lda, t, ldt, d__, info);
#else
    aocl_int64_t m_64 = *m;
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_dorhr_col(&m_64, &n_64, &nb_64, a, &lda_64, t, &ldt_64, d__, &info_64);

    *info = (aocl_int_t)info_64;
#endif
}

void aocl_lapack_dorhr_col(aocl_int64_t *m, aocl_int64_t *n, aocl_int64_t *nb, doublereal *a,
                           aocl_int64_t *lda, doublereal *t, aocl_int64_t *ldt, doublereal *d__,
                           aocl_int64_t *info)
{
    AOCL_DTL_TRACE_LOG_INIT
    AOCL_DTL_SNPRINTF("dorhr_col inputs: m %" FLA_IS ", n %" FLA_IS ", nb %" FLA_IS ", lda %" FLA_IS
                      ", ldt %" FLA_IS "",
                      *m, *n, *nb, *lda, *ldt);
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("DORHR_COL", &i__1, (ftnlen)9);
    ...
    /* (1-1) Factor V1 and U. */
    aocl_lapack_dlaorhr_col_getrfnp(n, n, &a[a_offset], lda, &d__[1], &iinfo);
    /* (1-2) Solve for V2. */
    if(*m > *n)
    {
        i__1 = *m - *n;
        aocl_blas_dtrsm("R", "U", "N", "N", &i__1, n, &c_b7, &a[a_offset], lda, &a[*n + 1 + a_dim1],
                        lda);
    }
    ...
    for(jb = 1; i__2 < 0 ? jb >= i__1 : jb <= i__1; jb += i__2)
    {
        ...
        {
            i__4 = j - jbtemp1;
            aocl_blas_dcopy(&i__4, &a[jb + j * a_dim1], &c__1, &t[j * t_dim1 + 1], &c__1);
        }
        ...
        for(j = jb; j <= i__3; ++j)
        {
            ...
            {
                i__4 = j - jbtemp1;
                aocl_blas_dscal(&i__4, &c_b10, &t[j * t_dim1 + 1], &c__1);
            }
        }
        ...
        aocl_blas_dtrsm("R", "L", "T", "U", &jnb, &jnb, &c_b7, &a[jb + jb * a_dim1], lda,
                        &t[jb * t_dim1 + 1], ldt);
    }
    AOCL_DTL_TRACE_LOG_EXIT
    return;
