...
/* > \brief <b> DSYSV_AA computes the solution to system of linear equations A * X = B for SY
 * matrices</b> */
/* =========== DOCUMENTATION =========== */
...
void dsysv_aa_(char *uplo, aocl_int_t *n, aocl_int_t *nrhs, doublereal *a, aocl_int_t *lda,
               aocl_int_t *ipiv, doublereal *b, aocl_int_t *ldb, doublereal *work,
               aocl_int_t *lwork, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_dsysv_aa(uplo, n, nrhs, a, lda, ipiv, b, ldb, work, lwork, info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_dsysv_aa(uplo, &n_64, &nrhs_64, a, &lda_64, ipiv, b, &ldb_64, work, &lwork_64,
                         &info_64);
...
void aocl_lapack_dsysv_aa(char *uplo, aocl_int64_t *n, aocl_int64_t *nrhs, doublereal *a,
                          aocl_int64_t *lda, aocl_int_t *ipiv, doublereal *b, aocl_int64_t *ldb,
                          doublereal *work, aocl_int64_t *lwork, aocl_int64_t *info)
{
    AOCL_DTL_TRACE_LOG_INIT
    AOCL_DTL_SNPRINTF("dsysv_aa inputs: uplo %c, n %" FLA_IS ", nrhs %" FLA_IS ", lda %" FLA_IS
    ...
    *info = 0;
    ...
    if(*info == 0)
    {
        aocl_lapack_dsytrf_aa(uplo, n, &a[a_offset], lda, &ipiv[1], &work[1], &c_n1, info);
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("DSYSV_AA ", &i__1, (ftnlen)9);
    ...
    aocl_lapack_dsytrf_aa(uplo, n, &a[a_offset], lda, &ipiv[1], &work[1], lwork, info);
    if(*info == 0)
    {
