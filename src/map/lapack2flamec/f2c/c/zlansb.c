...
doublereal zlansb_(char *norm, char *uplo, aocl_int_t *n, aocl_int_t *k, dcomplex *ab,
                   aocl_int_t *ldab, doublereal *work)
{
#if FLA_ENABLE_ILP64
    return aocl_lapack_zlansb(norm, uplo, n, k, ab, ldab, work);
#else
    aocl_int64_t n_64 = *n;
    aocl_int64_t k_64 = *k;
    aocl_int64_t ldab_64 = *ldab;

    return aocl_lapack_zlansb(norm, uplo, &n_64, &k_64, ab, &ldab_64, work);
#endif
}

doublereal aocl_lapack_zlansb(char *norm, char *uplo, aocl_int64_t *n, aocl_int64_t *k,
                              dcomplex *ab, aocl_int64_t *ldab, doublereal *work)
{
    ...
    else if(lsame_(norm, "F", 1, 1) || lsame_(norm, "E", 1, 1))
    {
        ...
        aocl_lapack_zlassq(n, &ab[l + ab_dim1], ldab, &scale, &sum);
