/* ../netlib/ssbgst.f -- translated by f2c (version 20100827). You must link the resulting object
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/ssbgst.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/ssbgst.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/ssbgst.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
void ssbgst_(char *vect, char *uplo, aocl_int_t *n, aocl_int_t *ka, aocl_int_t *kb, real *ab,
             aocl_int_t *ldab, real *bb, aocl_int_t *ldbb, real *x, aocl_int_t *ldx, real *work,
             aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_ssbgst(vect, uplo, n, ka, kb, ab, ldab, bb, ldbb, x, ldx, work, info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_ssbgst(vect, uplo, &n_64, &ka_64, &kb_64, ab, &ldab_64, bb, &ldbb_64, x, &ldx_64,
                       work, &info_64);
...
void aocl_lapack_ssbgst(char *vect, char *uplo, aocl_int64_t *n, aocl_int64_t *ka, aocl_int64_t *kb,
                        real *ab, aocl_int64_t *ldab, real *bb, aocl_int64_t *ldbb, real *x,
                        aocl_int64_t *ldx, real *work, aocl_int64_t *info)
{
    AOCL_DTL_TRACE_LOG_INIT
    AOCL_DTL_SNPRINTF("ssbgst inputs: vect %c, uplo %c, n %" FLA_IS ", ka %" FLA_IS ", kb %" FLA_IS
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("SSBGST", &i__1, (ftnlen)6);
    ...
    if(wantx)
    {
        aocl_lapack_slaset("Full", n, n, &c_b8, &c_b9, &x[x_offset], ldx);
    }
    ...
    else
    {
        ...
        for(k = 1; k <= i__4; ++k)
        {
            if(update)
            {
                ...
                if(i__ - k + *ka < *n && i__ - k > 1)
                {
                    ...
                    slartg_(&ab[ka1 - k + i__ * ab_dim1], &ra1, &work[*n + i__ - k + *ka - m],
                            &work[i__ - k + *ka - m], &ra);
        ...
        for(k = *kb; k >= 1; --k)
        {
            ...
            if(nr > 0)
            {
                /* generate rotations in 2nd set to annihilate elements */
                /* which have been created outside the band */
                aocl_lapack_slargv(&nr, &ab[ka1 + (j2 - *ka) * ab_dim1], &inca, &work[j2], &ka1,
                                   &work[*n + j2], &ka1);
    ...
    if(upper)
    {
        ...
        for(k = 1; k <= i__3; ++k)
        {
            if(update)
            {
                ...
                {
                    /* generate rotation to annihilate a(i+k-ka-1,i) */
                    slartg_(&ab[k + 1 + i__ * ab_dim1], &ra1, &work[*n + i__ + k - *ka],
                            &work[i__ + k - *ka], &ra);
                    /* create nonzero element a(i+k-ka-1,i+k) outside the */
    ...
    else
    {
        ...
        for(k = 1; k <= i__4; ++k)
        {
            ...
            for(l = *ka - 1; l >= i__1; --l)
            {
                ...
                if(nrt > 0)
                {
                    aocl_lapack_slartv(&nrt, &ab[ka1 - l + 1 + (j1t - ka1 + l) * ab_dim1], &inca,
