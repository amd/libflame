...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/dlatrs.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/dlatrs.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/dlatrs.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
    ...
    *scale = 1.;
    ...
    if(lsame_(normin, "N", 1, 1))
    {
        ...
        if(upper)
        {
            ...
            {
                i__2 = j - 1;
                cnorm[j] = aocl_blas_dasum(&i__2, &a[j * a_dim1 + 1], &c__1);
                /* L10: */
            }
        }
        else
        {
            ...
            {
                i__2 = *n - j;
                cnorm[j] = aocl_blas_dasum(&i__2, &a[j + 1 + j * a_dim1], &c__1);
                /* L20: */
            }
            cnorm[*n] = 0.;
        }
    ...
    else
    {
        ...
        if(xmax > bignum)
        {
            ...
            *scale = bignum / xmax;
            aocl_blas_dscal(n, scale, &x[1], &c__1);
            xmax = bignum;
        }
        if(notran)
        {
        ...
        else
        {
            ...
            for(j = jfirst; i__1 < 0 ? j >= i__2 : j <= i__2; j += i__1)
            {
                ...
                if(uscal == 1.)
                {
                    ...
                    {
                        i__3 = j - 1;
                        sumj = aocl_blas_ddot(&i__3, &a[j * a_dim1 + 1], &c__1, &x[1], &c__1);
                    }
                    else if(j < *n)
                    {
                        i__3 = *n - j;
