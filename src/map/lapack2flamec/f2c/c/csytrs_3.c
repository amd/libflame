...
/* > */
/* > \verbatim */
/* > CSYTRS_3 solves a system of linear equations A * X = B with a scomplex */
...
void csytrs_3_(char *uplo, aocl_int_t *n, aocl_int_t *nrhs, scomplex *a, aocl_int_t *lda, scomplex *e,
               aocl_int_t *ipiv, scomplex *b, aocl_int_t *ldb, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_csytrs_3(uplo, n, nrhs, a, lda, e, ipiv, b, ldb, info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_csytrs_3(uplo, &n_64, &nrhs_64, a, &lda_64, e, ipiv, b, &ldb_64, &info_64);

    *info = (aocl_int_t)info_64;
#endif
}

void aocl_lapack_csytrs_3(char *uplo, aocl_int64_t *n, aocl_int64_t *nrhs, scomplex *a,
                          ...
                          aocl_int64_t *ldb, aocl_int64_t *info)
{
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("CSYTRS_3", &i__1, (ftnlen)8);
    ...
    if(upper)
    {
        ...
        while(i__ >= 1)
        {
            if(ipiv[i__] > 0)
            {
                ...
                aocl_blas_cscal(nrhs, &q__1, &b[i__ + b_dim1], ldb);
            }
            else if(i__ > 1)
            {
                i__1 = i__;
        ...
        aocl_blas_ctrsm("L", "U", "T", "U", n, nrhs, &c_b1, &a[a_offset], lda, &b[b_offset], ldb);
        /* P * B [ P * (U**T \ (D \ (U \P**T * B) )) ] */
    ...
    else
    {
        ...
        aocl_blas_ctrsm("L", "L", "T", "U", n, nrhs, &c_b1, &a[a_offset], lda, &b[b_offset], ldb);
        /* P * B [ P * (L**T \ (D \ (L \P**T * B) )) ] */
        /* Interchange rows K and IPIV(K) of matrix B in reverse order */
