...
/* > \verbatim */
/* > */
/* > ZLAHR2 reduces the first NB columns of A scomplex general n-BY-(n-k+1) */
...
void zlahr2_(aocl_int_t *n, aocl_int_t *k, aocl_int_t *nb, dcomplex *a, aocl_int_t *lda,
             dcomplex *tau, dcomplex *t, aocl_int_t *ldt, dcomplex *y,
             aocl_int_t *ldy)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_zlahr2(n, k, nb, a, lda, tau, t, ldt, y, ldy);
#else
    aocl_int64_t n_64 = *n;
    aocl_int64_t k_64 = *k;
    ...
    aocl_lapack_zlahr2(&n_64, &k_64, &nb_64, a, &lda_64, tau, t, &ldt_64, y, &ldy_64);
#endif
}

void aocl_lapack_zlahr2(aocl_int64_t *n, aocl_int64_t *k, aocl_int64_t *nb, dcomplex *a,
                        ...
                        dcomplex *y, aocl_int64_t *ldy)
{
    ...
    for(i__ = 1; i__ <= i__1; ++i__)
    {
        if(i__ > 1)
        {
            ...
            z__1.r = -1.;
            z__1.i = -0.; // , expr subst
            aocl_blas_zgemv("NO TRANSPOSE", &i__2, &i__3, &z__1, &y[*k + 1 + y_dim1], ldy,
        ...
        /* Computing MIN */
        i__3 = *k + i__ + 1;
        aocl_lapack_zlarfg(&i__2, &a[*k + i__ + i__ * a_dim1], &a[fla_min(i__3, *n) + i__ * a_dim1],
                           &c__1, &tau[i__]);
        ...
        i__2 = *n - *k;
        i__3 = *n - *k - i__ + 1;
        aocl_blas_zgemv("NO TRANSPOSE", &i__2, &i__3, &c_b2, &a[*k + 1 + (i__ + 1) * a_dim1], lda,
        ...
        i__2 = *n - *k;
        aocl_blas_zscal(&i__2, &tau[i__], &y[*k + 1 + i__ * y_dim1], &c__1);
        /* Compute T(1:I,I) */
        i__2 = i__ - 1;
        i__3 = i__;
        z__1.r = -tau[i__3].r;
    ...
    /* Compute Y(1:K,1:NB) */
    aocl_lapack_zlacpy("ALL", k, nb, &a[(a_dim1 << 1) + 1], lda, &y[y_offset], ldy);
    aocl_blas_ztrmm("RIGHT", "Lower", "NO TRANSPOSE", "UNIT", k, nb, &c_b2, &a[*k + 1 + a_dim1],
                    lda, &y[y_offset], ldy);
    if(*n > *k + *nb)
    {
        i__1 = *n - *k - *nb;
        aocl_blas_zgemm("NO TRANSPOSE", "NO TRANSPOSE", k, nb, &i__1, &c_b2,
                        &a[(*nb + 2) * a_dim1 + 1], lda, &a[*k + 1 + *nb + a_dim1], lda, &c_b2,
                        &y[y_offset], ldy);
    }
    aocl_blas_ztrmm("RIGHT", "Upper", "NO TRANSPOSE", "NON-UNIT", k, nb, &c_b2, &t[t_offset], ldt,
                    &y[y_offset], ldy);
    AOCL_DTL_TRACE_LOG_EXIT
    return;
    /* End of ZLAHR2 */
