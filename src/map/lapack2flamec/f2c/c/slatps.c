/* ../netlib/slatps.f -- translated by f2c (version 20100827). You must link the resulting object
...
/* > \brief \b SLATPS solves a triangular system of equations with the matrix held in packed
 * storage. */
/* =========== DOCUMENTATION =========== */
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/slatps.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/slatps.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/slatps.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
    ...
    *scale = 1.f;
    if(lsame_(normin, "N", 1, 1))
    {
        ...
        if(upper)
        {
            ...
            {
                i__2 = j - 1;
                cnorm[j] = aocl_blas_sasum(&i__2, &ap[ip], &c__1);
                ip += j;
                /* L10: */
            }
        }
        else
        {
            ...
            {
                i__2 = *n - j;
                cnorm[j] = aocl_blas_sasum(&i__2, &ap[ip + 1], &c__1);
                ip = ip + *n - j + 1;
                /* L20: */
            }
            cnorm[*n] = 0.f;
    ...
    if(grow * tscal > smlnum)
    {
        ...
        aocl_blas_stpsv(uplo, trans, diag, n, &ap[1], &x[1], &c__1);
    }
    else
    {
        ...
        if(notran)
        {
            ...
            L95: /* Scale x if necessary to avoid overflow when adding a */
                ...
                {
                    /* Scale x by 1/2. */
                    aocl_blas_sscal(n, &c_b36, &x[1], &c__1);
                    *scale *= .5f;
                }
                if(upper)
                {
