/* ../netlib/cla_syrcond_x.f -- translated by f2c (version 20100827). You must link the resulting
...
/* > \brief \b CLA_SYRCOND_X computes the infinity norm condition number of op(A)*diag(x) for
...
real cla_syrcond_x_(char *uplo, aocl_int_t *n, scomplex *a, aocl_int_t *lda, scomplex *af,
                    aocl_int_t *ldaf, aocl_int_t *ipiv, scomplex *x, aocl_int_t *info, scomplex *work,
                    real *rwork)
{
#if FLA_ENABLE_ILP64
    return aocl_lapack_cla_syrcond_x(uplo, n, a, lda, af, ldaf, ipiv, x, info, work, rwork);
#else
    aocl_int64_t n_64 = *n;
    ...
    real ret_val = aocl_lapack_cla_syrcond_x(uplo, &n_64, a, &lda_64, af, &ldaf_64, ipiv, x,
                                             &info_64, work, rwork);
...
real aocl_lapack_cla_syrcond_x(char *uplo, aocl_int64_t *n, scomplex *a, aocl_int64_t *lda,
                               ...
                               aocl_int64_t *info, scomplex *work, real *rwork)
{
    ...
    char buffer[256];
#if FLA_ENABLE_ILP64
    snprintf(buffer, 256, "cla_syrcond_x inputs: uplo %c, n %lld, lda %lld, ldaf %lld", *uplo, *n,
             *lda, *ldaf);
#else
    snprintf(buffer, 256, "cla_syrcond_x inputs: uplo %c, n %d, lda %d, ldaf %d", *uplo, *n, *lda,
             *ldaf);
#endif
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("CLA_SYRCOND_X", &i__1, (ftnlen)13);
