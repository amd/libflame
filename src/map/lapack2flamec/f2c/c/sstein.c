/* ../netlib/sstein.f -- translated by f2c (version 20160102). You must link the resulting object
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/sstein.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/sstein.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/sstein.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
void sstein_(aocl_int_t *n, real *d__, real *e, aocl_int_t *m, real *w, aocl_int_t *iblock,
             aocl_int_t *isplit, real *z__, aocl_int_t *ldz, real *work, aocl_int_t *iwork,
             aocl_int_t *ifail, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_sstein(n, d__, e, m, w, iblock, isplit, z__, ldz, work, iwork, ifail, info);
#else
    aocl_int64_t n_64 = *n;
    aocl_int64_t m_64 = *m;
    ...
    aocl_lapack_sstein(&n_64, d__, e, &m_64, w, iblock, isplit, z__, &ldz_64, work, iwork, ifail,
                       &info_64);
...
void aocl_lapack_sstein(aocl_int64_t *n, real *d__, real *e, aocl_int64_t *m, real *w,
                        aocl_int_t *iblock, aocl_int_t *isplit, real *z__, aocl_int64_t *ldz,
                        real *work, aocl_int_t *iwork, aocl_int_t *ifail, aocl_int64_t *info)
{
    AOCL_DTL_TRACE_LOG_INIT
    AOCL_DTL_SNPRINTF("sstein inputs: n %" FLA_IS ", m %" FLA_IS ", iblock %" FLA_IS
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("SSTEIN", &i__1, (ftnlen)6);
    ...
    for(nblk = 1; nblk <= i__1; ++nblk)
    {
        ...
        for(i__ = b1 + 1; i__ <= i__2; ++i__)
        {
            /* Computing MAX */
            r__4 = onenrm;
            r__5 = (r__1 = d__[i__], f2c_abs(r__1)) + (r__2 = e[i__ - 1], f2c_abs(r__2))
                   + (r__3 = e[i__], f2c_abs(r__3)); // , expr subst
    ...
    L60:
        ...
        L70:
            ...
            if(gpind != j)
            {
                ...
                {
                    ctr = -aocl_blas_sdot(&blksiz, &work[indrv1 + 1], &c__1,
                                          &z__[b1 + i__ * z_dim1], &c__1);
                    aocl_blas_saxpy(&blksiz, &ctr, &z__[b1 + i__ * z_dim1], &c__1,
                                    &work[indrv1 + 1], &c__1);
                    /* L80: */
                }
            }
