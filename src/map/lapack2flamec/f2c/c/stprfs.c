/* ../netlib/stprfs.f -- translated by f2c (version 20100827). You must link the resulting object
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/stprfs.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/stprfs.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/stprfs.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
void stprfs_(char *uplo, char *trans, char *diag, aocl_int_t *n, aocl_int_t *nrhs, real *ap,
             real *b, aocl_int_t *ldb, real *x, aocl_int_t *ldx, real *ferr, real *berr, real *work,
             aocl_int_t *iwork, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_stprfs(uplo, trans, diag, n, nrhs, ap, b, ldb, x, ldx, ferr, berr, work, iwork,
                       info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_stprfs(uplo, trans, diag, &n_64, &nrhs_64, ap, b, &ldb_64, x, &ldx_64, ferr, berr,
                       work, iwork, &info_64);
...
void aocl_lapack_stprfs(char *uplo, char *trans, char *diag, aocl_int64_t *n, aocl_int64_t *nrhs,
                        real *ap, real *b, aocl_int64_t *ldb, real *x, aocl_int64_t *ldx,
                        real *ferr, real *berr, real *work, aocl_int_t *iwork, aocl_int64_t *info)
{
    AOCL_DTL_TRACE_LOG_INIT
    AOCL_DTL_SNPRINTF("stprfs inputs: uplo %c, trans %c, diag %c, n %" FLA_IS ", nrhs %" FLA_IS
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("STPRFS", &i__1, (ftnlen)6);
    ...
    for(j = 1; j <= i__1; ++j)
    {
        ...
        aocl_blas_scopy(n, &x[j * x_dim1 + 1], &c__1, &work[*n + 1], &c__1);
    ...
    L210:
        ...
        if(kase != 0)
        {
            if(kase == 1)
            {
                ...
                aocl_blas_stpsv(uplo, transt, diag, n, &ap[1], &work[*n + 1], &c__1);
                i__2 = *n;
                for(i__ = 1; i__ <= i__2; ++i__)
                {
            ...
            else
            {
                ...
                aocl_blas_stpsv(uplo, trans, diag, n, &ap[1], &work[*n + 1], &c__1);
            }
            goto L210;
        }
        /* Normalize error. */
