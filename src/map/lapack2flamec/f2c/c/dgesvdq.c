/* ../netlib/v3.9.0/dgesvdq.f -- translated by f2c (version 20160102). You must link the resulting
...
/* > \brief <b> DGESVDQ computes the singular value decomposition (SVD) with a QR-Preconditioned QR
 * SVD Method for GE matrices</b> */
/* =========== DOCUMENTATION =========== */
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/dgesvdq
 * .f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/dgesvdq
 * .f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/dgesvdq
 * .f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
void dgesvdq_(char *joba, char *jobp, char *jobr, char *jobu, char *jobv, aocl_int_t *m,
              aocl_int_t *n, doublereal *a, aocl_int_t *lda, doublereal *s, doublereal *u,
              aocl_int_t *ldu, doublereal *v, aocl_int_t *ldv, aocl_int_t *numrank,
              aocl_int_t *iwork, aocl_int_t *liwork, doublereal *work, aocl_int_t *lwork,
              doublereal *rwork, aocl_int_t *lrwork, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_dgesvdq(joba, jobp, jobr, jobu, jobv, m, n, a, lda, s, u, ldu, v, ldv, numrank,
    ...
    aocl_lapack_dgesvdq(joba, jobp, jobr, jobu, jobv, &m_64, &n_64, a, &lda_64, s, u, &ldu_64, v,
                        &ldv_64, &numrank_64, iwork, &liwork_64, work, &lwork_64, rwork, &lrwork_64,
                        &info_64);
...
void aocl_lapack_dgesvdq(char *joba, char *jobp, char *jobr, char *jobu, char *jobv,
                         aocl_int64_t *m, aocl_int64_t *n, doublereal *a, aocl_int64_t *lda,
                         doublereal *s, doublereal *u, aocl_int64_t *ldu, doublereal *v,
                         aocl_int64_t *ldv, aocl_int64_t *numrank, aocl_int_t *iwork,
                         aocl_int64_t *liwork, doublereal *work, aocl_int64_t *lwork,
                         doublereal *rwork, aocl_int64_t *lrwork, aocl_int64_t *info)
{
    AOCL_DTL_TRACE_LOG_INIT
    AOCL_DTL_SNPRINTF("dgesvdq inputs: joba %c, jobp %c, jobr %c, jobu %c, jobv %c, m %" FLA_IS
                      ", n %" FLA_IS ", lda %" FLA_IS ", ldu %" FLA_IS ", ldv %" FLA_IS
                      ", lwork %" FLA_IS ", lrwork %" FLA_IS "",
                      *joba, *jobp, *jobr, *jobu, *jobv, *m, *n, *lda, *ldu, *ldv, *lwork, *lrwork);
    /* System generated locals */
    aocl_int64_t a_dim1, a_offset, u_dim1, u_offset, v_dim1, v_offset, i__1, i__2;
    ...
    doublereal big;
    aocl_int64_t lwrk_dgeqp3__, lwrk_dgelqf__, lwrk_dgeqrf__, lwrk_dgesvd__, lwrk_dormlq__,
        lwrk_dormqr__, lwrk_dgesvd2__, ierr, lwrk_dormqr2__;
    doublereal rtmp;
    logical lsvc0, accla;
    aocl_int64_t lwqp3;
    ...
    *info = 0;
    ...
    if(*info == 0)
    {
        ...
        if(lquery)
        {
            aocl_lapack_dgeqp3(m, n, &a[a_offset], lda, &iwork[1], rdummy, rdummy, &c_n1, &ierr);
            lwrk_dgeqp3__ = (integer)rdummy[0];
            if(wntus || wntur)
            {
                aocl_lapack_dormqr("L", "N", m, n, n, &a[a_offset], lda, rdummy, &u[u_offset], ldu,
                                   rdummy, &c_n1, &ierr);
                lwrk_dormqr__ = (integer)rdummy[0];
            }
            else if(wntua)
            {
                aocl_lapack_dormqr("L", "N", m, m, n, &a[a_offset], lda, rdummy, &u[u_offset], ldu,
                                   rdummy, &c_n1, &ierr);
                lwrk_dormqr__ = (integer)rdummy[0];
            }
            else
        ...
        if(!(lsvec || rsvec))
        {
            ...
            if(lquery)
            {
                aocl_lapack_dgesvd("N", "N", n, n, &a[a_offset], lda, &s[1], &u[u_offset], ldu,
                                   &v[v_offset], ldv, rdummy, &c_n1, &ierr);
        ...
        else if(lsvec && !rsvec)
        {
            ...
            if(lquery)
            {
                if(rtrans)
                {
                    aocl_lapack_dgesvd("N", "O", n, n, &a[a_offset], lda, &s[1], &u[u_offset], ldu,
                                       &v[v_offset], ldv, rdummy, &c_n1, &ierr);
                }
                else
                {
                    aocl_lapack_dgesvd("O", "N", n, n, &a[a_offset], lda, &s[1], &u[u_offset], ldu,
                                       &v[v_offset], ldv, rdummy, &c_n1, &ierr);
                }
                lwrk_dgesvd__ = (integer)rdummy[0];
                if(conda)
        ...
        else
        {
            ...
            if(rtrans)
            {
                ...
                if(wntva)
                {
                    ...
                    lworq2 = fla_max(*n, 1);
                    /* Computing MAX */
                    i__1 = lwqp3, i__2 = *n / 2 + lwqrf, i__1 = fla_max(i__1, i__2),
                    i__2 = *n / 2 + lwsvd2, i__1 = fla_max(i__1, i__2);
                    i__2 = *n / 2 + lworq2;
                    i__1 = fla_max(i__1, i__2); // ; expr subst
            ...
            else
            {
                ...
                if(wntva)
                {
                    ...
                    lworlq = fla_max(*n, 1);
                    /* Computing MAX */
                    i__1 = lwqp3, i__2 = *n / 2 + lwlqf, i__1 = fla_max(i__1, i__2),
                    i__2 = *n / 2 + lwsvd2, i__1 = fla_max(i__1, i__2);
                    i__2 = *n / 2 + lworlq;
                    i__1 = fla_max(i__1, i__2); // ; expr subst
                    minwrk2 = fla_max(i__1, lworq);
            ...
            if(lquery)
            {
                if(rtrans)
                {
                    aocl_lapack_dgesvd("O", "A", n, n, &a[a_offset], lda, &s[1], &u[u_offset], ldu,
                                       &v[v_offset], ldv, rdummy, &c_n1, &ierr);
                    lwrk_dgesvd__ = (integer)rdummy[0];
                    /* Computing MAX */
                    i__1 = fla_max(lwrk_dgeqp3__, lwrk_dgesvd__);
                    ...
                    {
                        i__1 = *n / 2;
                        aocl_lapack_dgeqrf(n, &i__1, &u[u_offset], ldu, rdummy, rdummy, &c_n1,
                                           &ierr);
                        lwrk_dgeqrf__ = (integer)rdummy[0];
                        i__1 = *n / 2;
                        i__2 = *n / 2;
                        aocl_lapack_dgesvd("S", "O", &i__1, &i__2, &v[v_offset], ldv, &s[1],
                                           &u[u_offset], ldu, &v[v_offset], ldv, rdummy, &c_n1,
                                           &ierr);
                        lwrk_dgesvd2__ = (integer)rdummy[0];
                        i__1 = *n / 2;
                        aocl_lapack_dormqr("R", "C", n, n, &i__1, &u[u_offset], ldu, rdummy,
                                           &v[v_offset], ldv, rdummy, &c_n1, &ierr);
                        lwrk_dormqr2__ = (integer)rdummy[0];
                        /* Computing MAX */
                        i__1 = lwrk_dgeqp3__, i__2 = *n / 2 + lwrk_dgeqrf__,
                        i__1 = fla_max(i__1, i__2), i__2 = *n / 2 + lwrk_dgesvd2__;
                ...
                {
                    aocl_lapack_dgesvd("S", "O", n, n, &a[a_offset], lda, &s[1], &u[u_offset], ldu,
                                       &v[v_offset], ldv, rdummy, &c_n1, &ierr);
                    lwrk_dgesvd__ = (integer)rdummy[0];
                    /* Computing MAX */
                    i__1 = fla_max(lwrk_dgeqp3__, lwrk_dgesvd__);
                    ...
                    {
                        i__1 = *n / 2;
                        aocl_lapack_dgelqf(&i__1, n, &u[u_offset], ldu, rdummy, rdummy, &c_n1,
                                           &ierr);
                        ...
                        lwrk_dgesvd2__ = (integer)rdummy[0];
                        i__1 = *n / 2;
                        aocl_lapack_dormlq("R", "N", n, n, &i__1, &u[u_offset], ldu, rdummy,
                                           &v[v_offset], ldv, rdummy, &c_n1, &ierr);
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("DGESVDQ", &i__1, (ftnlen)7);
    ...
    if(rowprm)
    {
        ...
        for(p = 1; p <= i__1; ++p)
        {
            ...
            rwork[p] = aocl_lapack_dlange("M", &c__1, n, &a[p + a_dim1], lda, rdummy);
            /* .. check for NaN's and Inf's */
            if(rwork[p] != rwork[p] || rwork[p] * 0. != 0.)
            {
                *info = -8;
                i__2 = -(*info);
                aocl_blas_xerbla("DGESVDQ", &i__2, (ftnlen)7);
                AOCL_DTL_TRACE_LOG_EXIT
                return;
            }
            /* L1904: */
        ...
        if(rwork[1] == 0.)
        {
            ...
            *numrank = 0;
            aocl_lapack_dlaset("G", n, &c__1, &c_b72, &c_b72, &s[1], n);
            if(wntus)
            {
                aocl_lapack_dlaset("G", m, n, &c_b72, &c_b76, &u[u_offset], ldu);
            }
            if(wntua)
            {
                aocl_lapack_dlaset("G", m, m, &c_b72, &c_b76, &u[u_offset], ldu);
            }
            if(wntva)
            {
                aocl_lapack_dlaset("G", n, n, &c_b72, &c_b76, &v[v_offset], ldv);
            }
            if(wntuf)
            {
                aocl_lapack_dlaset("G", n, &c__1, &c_b72, &c_b72, &work[1], n);
                aocl_lapack_dlaset("G", m, n, &c_b72, &c_b76, &u[u_offset], ldu);
            }
            i__1 = *n;
            for(p = 1; p <= i__1; ++p)
        ...
        if(rwork[1] > big / sqrt((doublereal)(*m)))
        {
            ...
            aocl_lapack_dlascl("G", &c__0, &c__0, &d__1, &c_b76, m, n, &a[a_offset], lda, &ierr);
            ascaled = TRUE_;
        }
        i__1 = *m - 1;
        aocl_lapack_dlaswp(n, &a[a_offset], lda, &c__1, &i__1, &iwork[*n + 1], &c__1);
    }
    ...
    if(!rowprm)
    {
        rtmp = aocl_lapack_dlange("M", m, n, &a[a_offset], lda, rdummy);
    ...
    if(accla)
    {
        ...
        for(p = 2; p <= i__1; ++p)
        {
            if((d__2 = a[p + p * a_dim1], f2c_dabs(d__2))
               < rtmp * (d__1 = a[a_dim1 + 1], f2c_dabs(d__1)))
            {
                goto L3002;
            }
            ++nr;
    ...
    else if(acclm)
    {
        ...
        {
            if((d__2 = a[p + p * a_dim1], f2c_dabs(d__2))
                   < epsln * (d__1 = a[p - 1 + (p - 1) * a_dim1], f2c_dabs(d__1))
               || (d__3 = a[p + p * a_dim1], f2c_dabs(d__3)) < sfmin)
            {
                goto L3402;
            }
            ++nr;
    ...
    L3502:
        if(conda)
        {
            ...
            for(p = 1; p <= i__1; ++p)
            {
                rtmp = aocl_blas_dnrm2(&p, &v[p * v_dim1 + 1], &c__1);
                d__1 = 1. / rtmp;
            ...
            if(!(lsvec || rsvec))
            {
                aocl_lapack_dpocon("U", &nr, &v[v_offset], ldv, &c_b76, &rtmp, &work[1],
                                   &iwork[*n + iwoff], &ierr);
            }
            else
            {
                aocl_lapack_dpocon("U", &nr, &v[v_offset], ldv, &c_b76, &rtmp, &work[*n + 1],
                                   &iwork[*n + iwoff], &ierr);
            }
            sconda = 1. / sqrt(rtmp);
    ...
    if(!(rsvec || lsvec))
    {
        ...
        if(rtrans)
        {
            ...
            aocl_lapack_dgesvd("N", "N", n, &nr, &a[a_offset], lda, &s[1], &u[u_offset], ldu,
        ...
        else
        {
            ...
            if(nr > 1)
            {
                i__1 = nr - 1;
                i__2 = nr - 1;
                aocl_lapack_dlaset("L", &i__1, &i__2, &c_b72, &c_b72, &a[a_dim1 + 2], lda);
            }
            aocl_lapack_dgesvd("N", "N", &nr, n, &a[a_offset], lda, &s[1], &u[u_offset], ldu,
                               &v[v_offset], ldv, &work[1], lwork, info);
        }
    }
    else if(lsvec && !rsvec)
    {
        ...
        if(rtrans)
        {
            ...
            if(nr > 1)
            {
                i__1 = nr - 1;
                i__2 = nr - 1;
                aocl_lapack_dlaset("U", &i__1, &i__2, &c_b72, &c_b72, &u[(u_dim1 << 1) + 1], ldu);
            }
            /* .. the left singular vectors not computed, the NR right singular */
            /* vectors overwrite [U](1:NR,1:NR) as transposed. These */
            /* will be pre-multiplied by Q to build the left singular vectors of A. */
            i__1 = *lwork - *n;
            aocl_lapack_dgesvd("N", "O", n, &nr, &u[u_offset], ldu, &s[1], &u[u_offset], ldu,
        ...
        else
        {
            ...
            aocl_lapack_dlacpy("U", &nr, n, &a[a_offset], lda, &u[u_offset], ldu);
            if(nr > 1)
            {
                i__1 = nr - 1;
                i__2 = nr - 1;
                aocl_lapack_dlaset("L", &i__1, &i__2, &c_b72, &c_b72, &u[u_dim1 + 2], ldu);
            }
            /* .. the right singular vectors not computed, the NR left singular */
            /* vectors overwrite [U](1:NR,1:NR) */
            i__1 = *lwork - *n;
            aocl_lapack_dgesvd("O", "N", &nr, n, &u[u_offset], ldu, &s[1], &u[u_offset], ldu,
                               &v[v_offset], ldv, &work[*n + 1], &i__1, info);
        ...
        {
            i__1 = *m - nr;
            aocl_lapack_dlaset("A", &i__1, &nr, &c_b72, &c_b72, &u[nr + 1 + u_dim1], ldu);
            if(nr < n1)
            {
                i__1 = n1 - nr;
                aocl_lapack_dlaset("A", &nr, &i__1, &c_b72, &c_b72, &u[(nr + 1) * u_dim1 + 1], ldu);
                i__1 = *m - nr;
                i__2 = n1 - nr;
                aocl_lapack_dlaset("A", &i__1, &i__2, &c_b72, &c_b76,
        ...
        {
            i__1 = *lwork - *n;
            aocl_lapack_dormqr("L", "N", m, &n1, n, &a[a_offset], lda, &work[1], &u[u_offset], ldu,
                               &work[*n + 1], &i__1, &ierr);
        }
        if(rowprm && !wntuf)
        {
            i__1 = *m - 1;
            aocl_lapack_dlaswp(&n1, &u[u_offset], ldu, &c__1, &i__1, &iwork[*n + 1], &c_n1);
        }
    }
    else if(rsvec && !lsvec)
    {
        ...
        if(rtrans)
        {
            ...
            if(nr > 1)
            {
                i__1 = nr - 1;
                i__2 = nr - 1;
                aocl_lapack_dlaset("U", &i__1, &i__2, &c_b72, &c_b72, &v[(v_dim1 << 1) + 1], ldv);
            }
            ...
            {
                i__1 = *lwork - *n;
                aocl_lapack_dgesvd("O", "N", n, &nr, &v[v_offset], ldv, &s[1], &u[u_offset], ldu,
                                   &u[u_offset], ldu, &work[*n + 1], &i__1, info);
                i__1 = nr;
                for(p = 1; p <= i__1; ++p)
                {
                ...
                aocl_lapack_dlapmt(&c_false, &nr, n, &v[v_offset], ldv, &iwork[1]);
            }
            else
            {
                ...
                i__1 = *n - nr;
                aocl_lapack_dlaset("G", n, &i__1, &c_b72, &c_b72, &v[(nr + 1) * v_dim1 + 1], ldv);
                i__1 = *lwork - *n;
                aocl_lapack_dgesvd("O", "N", n, n, &v[v_offset], ldv, &s[1], &u[u_offset], ldu,
                                   &u[u_offset], ldu, &work[*n + 1], &i__1, info);
                i__1 = *n;
                for(p = 1; p <= i__1; ++p)
                {
                ...
                aocl_lapack_dlapmt(&c_false, n, n, &v[v_offset], ldv, &iwork[1]);
            }
        }
        else
        {
            ...
            aocl_lapack_dlacpy("U", &nr, n, &a[a_offset], lda, &v[v_offset], ldv);
            if(nr > 1)
            {
                i__1 = nr - 1;
                i__2 = nr - 1;
                aocl_lapack_dlaset("L", &i__1, &i__2, &c_b72, &c_b72, &v[v_dim1 + 2], ldv);
            }
            ...
            {
                i__1 = *lwork - *n;
                aocl_lapack_dgesvd("N", "O", &nr, n, &v[v_offset], ldv, &s[1], &u[u_offset], ldu,
                                   &v[v_offset], ldv, &work[*n + 1], &i__1, info);
                aocl_lapack_dlapmt(&c_false, &nr, n, &v[v_offset], ldv, &iwork[1]);
            ...
            else
            {
                ...
                /* how to implement this, see the " FULL SVD " branch. */
                i__1 = *n - nr;
                aocl_lapack_dlaset("G", &i__1, n, &c_b72, &c_b72, &v[nr + 1 + v_dim1], ldv);
                i__1 = *lwork - *n;
                aocl_lapack_dgesvd("N", "O", n, n, &v[v_offset], ldv, &s[1], &u[u_offset], ldu,
                                   &v[v_offset], ldv, &work[*n + 1], &i__1, info);
                aocl_lapack_dlapmt(&c_false, n, n, &v[v_offset], ldv, &iwork[1]);
            }
    ...
    else
    {
        ...
        if(rtrans)
        {
            ...
            if(wntvr || nr == *n)
            {
                ...
                if(nr > 1)
                {
                    i__1 = nr - 1;
                    i__2 = nr - 1;
                    aocl_lapack_dlaset("U", &i__1, &i__2, &c_b72, &c_b72, &v[(v_dim1 << 1) + 1],
                                       ldv);
                }
                /* .. the left singular vectors of R**T overwrite [V], the NR right */
                /* singular vectors of R**T stored in [U](1:NR,1:NR) as transposed */
                i__1 = *lwork - *n;
                aocl_lapack_dgesvd("O", "A", n, &nr, &v[v_offset], ldv, &s[1], &v[v_offset], ldv,
                                   &u[u_offset], ldu, &work[*n + 1], &i__1, info);
                /* .. assemble V */
                i__1 = nr;
                for(p = 1; p <= i__1; ++p)
                ...
                if(nr < *m && !wntuf)
                {
                    ...
                    {
                        i__1 = n1 - nr;
                        aocl_lapack_dlaset("A", &nr, &i__1, &c_b72, &c_b72,
                                           &u[(nr + 1) * u_dim1 + 1], ldu);
                        i__1 = *m - nr;
                        i__2 = n1 - nr;
                        aocl_lapack_dlaset("A", &i__1, &i__2, &c_b72, &c_b76,
            ...
            else
            {
                ...
                if(optratio * nr > *n)
                {
                    ...
                    i__1 = *n - nr;
                    aocl_lapack_dlaset("A", n, &i__1, &c_b72, &c_b72, &v[(nr + 1) * v_dim1 + 1],
                                       ldv);
                    i__1 = *lwork - *n;
                    aocl_lapack_dgesvd("O", "A", n, n, &v[v_offset], ldv, &s[1], &v[v_offset], ldv,
                                       &u[u_offset], ldu, &work[*n + 1], &i__1, info);
                    i__1 = *n;
                    for(p = 1; p <= i__1; ++p)
                    {
                    ...
                    {
                        i__1 = *m - *n;
                        aocl_lapack_dlaset("A", &i__1, n, &c_b72, &c_b72, &u[*n + 1 + u_dim1], ldu);
                        if(*n < n1)
                        {
                            i__1 = n1 - *n;
                            aocl_lapack_dlaset("A", n, &i__1, &c_b72, &c_b72,
                                               &u[(*n + 1) * u_dim1 + 1], ldu);
                            i__1 = *m - *n;
                            i__2 = n1 - *n;
                            aocl_lapack_dlaset("A", &i__1, &i__2, &c_b72, &c_b76,
                ...
                else
                {
                    ...
                    if(nr > 1)
                    {
                        i__1 = nr - 1;
                        i__2 = nr - 1;
                        aocl_lapack_dlaset("U", &i__1, &i__2, &c_b72, &c_b72,
                                           &u[(nr + 2) * u_dim1 + 1], ldu);
                    }
                    i__1 = *lwork - *n - nr;
                    aocl_lapack_dgeqrf(n, &nr, &u[(nr + 1) * u_dim1 + 1], ldu, &work[*n + 1],
                    ...
                    aocl_lapack_dlaset("U", &i__1, &i__2, &c_b72, &c_b72, &v[(v_dim1 << 1) + 1],
                                       ldv);
                    i__1 = *lwork - *n - nr;
                    aocl_lapack_dgesvd("S", "O", &nr, &nr, &v[v_offset], ldv, &s[1], &u[u_offset],
                                       ldu, &v[v_offset], ldv, &work[*n + nr + 1], &i__1, info);
                    i__1 = *n - nr;
                    aocl_lapack_dlaset("A", &i__1, &nr, &c_b72, &c_b72, &v[nr + 1 + v_dim1], ldv);
                    i__1 = *n - nr;
                    aocl_lapack_dlaset("A", &nr, &i__1, &c_b72, &c_b72, &v[(nr + 1) * v_dim1 + 1],
                                       ldv);
                    i__1 = *n - nr;
                    i__2 = *n - nr;
                    aocl_lapack_dlaset("A", &i__1, &i__2, &c_b72, &c_b76,
                    ...
                    i__1 = *lwork - *n - nr;
                    aocl_lapack_dormqr("R", "C", n, n, &nr, &u[(nr + 1) * u_dim1 + 1], ldu,
                                       &work[*n + 1], &v[v_offset], ldv, &work[*n + nr + 1], &i__1,
                                       &ierr);
                    aocl_lapack_dlapmt(&c_false, n, n, &v[v_offset], ldv, &iwork[1]);
                    /* .. assemble the left singular vector matrix U of dimensions */
                    /* (M x NR) or (M x N) or (M x M). */
                    ...
                    {
                        i__1 = *m - nr;
                        aocl_lapack_dlaset("A", &i__1, &nr, &c_b72, &c_b72, &u[nr + 1 + u_dim1],
                                           ldu);
                        if(nr < n1)
                        {
                            i__1 = n1 - nr;
        ...
        else
        {
            ...
            if(wntvr || nr == *n)
            {
                ...
                i__1 = *lwork - *n;
                aocl_lapack_dgesvd("S", "O", &nr, n, &v[v_offset], ldv, &s[1], &u[u_offset], ldu,
                                   &v[v_offset], ldv, &work[*n + 1], &i__1, info);
                aocl_lapack_dlapmt(&c_false, &nr, n, &v[v_offset], ldv, &iwork[1]);
                /* .. now [V](1:NR,1:N) contains V(1:N,1:NR)**T */
                /* .. assemble the left singular vector matrix U of dimensions */
            ...
            else
            {
                ...
                if(optratio * nr > *n)
                {
                    ...
                    /* singular vectors of R stored in [U](1:NR,1:NR) */
                    i__1 = *n - nr;
                    aocl_lapack_dlaset("A", &i__1, n, &c_b72, &c_b72, &v[nr + 1 + v_dim1], ldv);
                    i__1 = *lwork - *n;
                    aocl_lapack_dgesvd("S", "O", n, n, &v[v_offset], ldv, &s[1], &u[u_offset], ldu,
                                       &v[v_offset], ldv, &work[*n + 1], &i__1, info);
                    aocl_lapack_dlapmt(&c_false, n, n, &v[v_offset], ldv, &iwork[1]);
                ...
                {
                    aocl_lapack_dlacpy("U", &nr, n, &a[a_offset], lda, &u[nr + 1 + u_dim1], ldu);
                    if(nr > 1)
                    {
                        i__1 = nr - 1;
                        i__2 = nr - 1;
                        aocl_lapack_dlaset("L", &i__1, &i__2, &c_b72, &c_b72, &u[nr + 2 + u_dim1],
                                           ldu);
                    }
                    i__1 = *lwork - *n - nr;
                    aocl_lapack_dgelqf(&nr, n, &u[nr + 1 + u_dim1], ldu, &work[*n + 1],
                                       &work[*n + nr + 1], &i__1, &ierr);
                    aocl_lapack_dlacpy("L", &nr, &nr, &u[nr + 1 + u_dim1], ldu, &v[v_offset], ldv);
                    if(nr > 1)
                    {
                        i__1 = nr - 1;
                        i__2 = nr - 1;
                    ...
                    aocl_lapack_dlaset("A", &i__1, &i__2, &c_b72, &c_b76,
                                       &v[nr + 1 + (nr + 1) * v_dim1], ldv);
                    i__1 = *lwork - *n - nr;
                    aocl_lapack_dormlq("R", "N", n, n, &nr, &u[nr + 1 + u_dim1], ldu, &work[*n + 1],
                                       &v[v_offset], ldv, &work[*n + nr + 1], &i__1, &ierr);
                    aocl_lapack_dlapmt(&c_false, n, n, &v[v_offset], ldv, &iwork[1]);
                    /* .. assemble the left singular vector matrix U of dimensions */
                    /* (M x NR) or (M x N) or (M x M). */
                    if(nr < *m && !wntuf)
...
L4002: /* .. if numerical rank deficiency is detected, the truncated */
    ...
    {
        i__1 = *n - nr;
        aocl_lapack_dlaset("G", &i__1, &c__1, &c_b72, &c_b72, &s[nr + 1], n);
    }
    /* .. undo scaling;
    ...
    {
        d__1 = sqrt((doublereal)(*m));
        aocl_lapack_dlascl("G", &c__0, &c__0, &c_b76, &d__1, &nr, &c__1, &s[1], n, &ierr);
    }
    if(conda)
    {
        rwork[1] = sconda;
