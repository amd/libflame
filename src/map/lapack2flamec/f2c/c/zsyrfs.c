/* ../netlib/zsyrfs.f -- translated by f2c (version 20100827). You must link the resulting object
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/zsyrfs.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/zsyrfs.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/zsyrfs.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
void zsyrfs_(char *uplo, aocl_int_t *n, aocl_int_t *nrhs, dcomplex *a, aocl_int_t *lda,
...
{
#if FLA_ENABLE_ILP64
    aocl_lapack_zsyrfs(uplo, n, nrhs, a, lda, af, ldaf, ipiv, b, ldb, x, ldx, ferr, berr, work,
                       rwork, info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_zsyrfs(uplo, &n_64, &nrhs_64, a, &lda_64, af, &ldaf_64, ipiv, b, &ldb_64, x,
...
void aocl_lapack_zsyrfs(char *uplo, aocl_int64_t *n, aocl_int64_t *nrhs, dcomplex *a,
                        ...
                        aocl_int64_t *info)
{
    ...
    AOCL_DTL_SNPRINTF("zsyrfs inputs: uplo %c, n %" FLA_IS ", nrhs %" FLA_IS ", lda %" FLA_IS
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("ZSYRFS", &i__1, (ftnlen)6);
    ...
    L20: /* Loop until stopping criterion is satisfied. */
        ...
        aocl_lapack_zsymv(uplo, n, &z__1, &a[a_offset], lda, &x[j * x_dim1 + 1], &c__1, &c_b1,
                          &work[1], &c__1);
        ...
        if(upper)
        {
            ...
            for(k = 1; k <= i__2; ++k)
            {
                ...
                i__3 = k + k * a_dim1;
                rwork[k] = rwork[k]
                           + ((d__1 = a[i__3].real, f2c_dabs(d__1))
        ...
        else
        {
            ...
            for(k = 1; k <= i__2; ++k)
            {
                ...
                xk = (d__1 = x[i__3].real, f2c_dabs(d__1))
                     + (d__2 = d_imag(&x[k + j * x_dim1]), f2c_dabs(d__2));
                i__3 = k + k * a_dim1;
                rwork[k] += ((d__1 = a[i__3].real, f2c_dabs(d__1))
                             + (d__2 = d_imag(&a[k + k * a_dim1]), f2c_dabs(d__2)))
                            * xk;
                i__3 = *n;
        ...
        if(berr[j] > eps && berr[j] * 2. <= lstres && count <= 5)
        {
            ...
            aocl_lapack_zsytrs(uplo, n, &c__1, &af[af_offset], ldaf, &ipiv[1], &work[1], n, info);
    ...
    L100:
        ...
        if(kase != 0)
        {
            if(kase == 1)
            {
                ...
                aocl_lapack_zsytrs(uplo, n, &c__1, &af[af_offset], ldaf, &ipiv[1], &work[1], n,
                                   info);
                i__2 = *n;
                for(i__ = 1; i__ <= i__2; ++i__)
                {
