...
void cgelst_(char *trans, aocl_int_t *m, aocl_int_t *n, aocl_int_t *nrhs, scomplex *a,
             aocl_int_t *lda, scomplex *b, aocl_int_t *ldb, scomplex *work, aocl_int_t *lwork,
             aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_cgelst(trans, m, n, nrhs, a, lda, b, ldb, work, lwork, info);
#else
    aocl_int64_t m_64 = *m;
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_cgelst(trans, &m_64, &n_64, &nrhs_64, a, &lda_64, b, &ldb_64, work, &lwork_64,
                       &info_64);
...
void aocl_lapack_cgelst(char *trans, aocl_int64_t *m, aocl_int64_t *n, aocl_int64_t *nrhs,
                        ...
                        aocl_int64_t *lwork, aocl_int64_t *info)
{
    ...
    *info = 0;
    ...
    if(*info == 0 || *info == -10)
    {
        ...
        nb = aocl_lapack_ilaenv(&c__1, "CGELST", " ", m, n, &c_n1, &c_n1);
        mnnrhs = fla_max(mn, *nrhs);
        /* Computing MAX */
        i__1 = 1;
        i__2 = (mn + mnnrhs) * nb; // , expr subst
        lwopt = fla_max(i__1, i__2);
        r__1 = aocl_lapack_sroundup_lwork(&lwopt);
        work[1].real = r__1;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("CGELST ", &i__1, (ftnlen)6);
    ...
    /* Computing MAX */
    i__1 = 2;
    i__2 = aocl_lapack_ilaenv(&c__2, "CGELST", " ", m, n, &c_n1, &c_n1); // , expr subst
    ...
    if(*m >= *n)
    {
        ...
        aocl_lapack_cgeqrt(m, n, &nb, &a[a_offset], lda, &work[1], &nb, &work[mn * nb + 1], info);
        if(!tpsd)
        {
            ...
            aocl_lapack_cgemqrt("Left", "Conjugate transpose", m, nrhs, n, &nb, &a[a_offset], lda,
        ...
        else
        {
            ...
            /* using the compact WY representation of Q, */
            /* workspace at least NRHS, optimally NRHS*NB. */
            aocl_lapack_cgemqrt("Left", "No transpose", m, nrhs, n, &nb, &a[a_offset], lda,
                                &work[1], &nb, &b[b_offset], ldb, &work[mn * nb + 1], info);
            scllen = *m;
        }
    }
    else
    {
        ...
        aocl_lapack_cgelqt(m, n, &nb, &a[a_offset], lda, &work[1], &nb, &work[mn * nb + 1], info);
        if(!tpsd)
        {
            /* M < N, A is not transposed: */
            /* Underdetermined system of equations, */
            ...
            /* using the compact WY representation of Q, */
            /* workspace at least NRHS, optimally NRHS*NB. */
            aocl_lapack_cgemlqt("Left", "Conjugate transpose", n, nrhs, m, &nb, &a[a_offset], lda,
                                &work[1], &nb, &b[b_offset], ldb, &work[mn * nb + 1], info);
            scllen = *n;
        }
        else
        {
            ...
            /* using the compact WY representation of Q, */
            /* workspace at least NRHS, optimally NRHS*NB. */
            aocl_lapack_cgemlqt("Left", "No transpose", n, nrhs, m, &nb, &a[a_offset], lda,
                                &work[1], &nb, &b[b_offset], ldb, &work[mn * nb + 1], info);
