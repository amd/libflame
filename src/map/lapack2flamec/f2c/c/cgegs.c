/* ../netlib/cgegs.f -- translated by f2c (version 20100827). You must link the resulting object
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/cgegs.f
 * "> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/cgegs.f
 * "> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/cgegs.f
 * "> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
{
#if FLA_ENABLE_ILP64
    aocl_lapack_cgegs(jobvsl, jobvsr, n, a, lda, b, ldb, alpha, beta, vsl, ldvsl, vsr, ldvsr, work, lwork, rwork, info);
#else
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_cgegs(jobvsl, jobvsr, &n_64, a, &lda_64, b, &ldb_64, alpha, beta, vsl, &ldvsl_64, vsr, &ldvsr_64, work, &lwork_64, rwork, &info_64);

    *info = (aocl_int_t)info_64;
#endif
}

void aocl_lapack_cgegs(char *jobvsl, char *jobvsr, aocl_int64_t *n, scomplex *a, aocl_int64_t *lda, scomplex *b,
            aocl_int64_t *ldb, scomplex *alpha, scomplex *beta, scomplex *vsl, aocl_int64_t *ldvsl,
            scomplex *vsr, aocl_int64_t *ldvsr, scomplex *work, aocl_int64_t *lwork, real *rwork,
            aocl_int64_t *info)
{
...
#if FLA_ENABLE_ILP64
    snprintf(buffer, 256,
             "cgegs inputs: jobvsl %c, jobvsr %c, n %lld, lda %lld, ldb %lld, ldvsl %lld, ldvsr "
             "%lld, lwork %lld",
             *jobvsl, *jobvsr, *n, *lda, *ldb, *ldvsl, *ldvsr, *lwork);
#else
    snprintf(
        buffer, 256,
        "cgegs inputs: jobvsl %c, jobvsr %c, n %d, lda %d, ldb %d, ldvsl %d, ldvsr %d, lwork %d",
        *jobvsl, *jobvsr, *n, *lda, *ldb, *ldvsl, *ldvsr, *lwork);
#endif
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("CGEGS ", &i__1, (ftnlen)6);
    ...
    if(ilascl)
    {
        aocl_lapack_clascl("G", &c_n1, &c_n1, &anrm, &anrmto, n, n, &a[a_offset], lda, &iinfo);
        if(iinfo != 0)
        {
            *info = *n + 9;
            AOCL_DTL_TRACE_EXIT(AOCL_DTL_LEVEL_TRACE_5);
    ...
    if(ilbscl)
    {
        aocl_lapack_clascl("G", &c_n1, &c_n1, &bnrm, &bnrmto, n, n, &b[b_offset], ldb, &iinfo);
        if(iinfo != 0)
        {
            *info = *n + 9;
            AOCL_DTL_TRACE_EXIT(AOCL_DTL_LEVEL_TRACE_5);
    ...
    if(ilascl)
    {
        aocl_lapack_clascl("U", &c_n1, &c_n1, &anrmto, &anrm, n, n, &a[a_offset], lda, &iinfo);
        if(iinfo != 0)
        {
            *info = *n + 9;
            AOCL_DTL_TRACE_EXIT(AOCL_DTL_LEVEL_TRACE_5);
    ...
    if(ilbscl)
    {
        aocl_lapack_clascl("U", &c_n1, &c_n1, &bnrmto, &bnrm, n, n, &b[b_offset], ldb, &iinfo);
        if(iinfo != 0)
        {
            *info = *n + 9;
            AOCL_DTL_TRACE_EXIT(AOCL_DTL_LEVEL_TRACE_5);
