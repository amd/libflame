/* ../netlib/zgbbrd.f -- translated by f2c (version 20100827). You must link the resulting object
...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/zgbbrd.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/zgbbrd.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/zgbbrd.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
/* > \verbatim */
/* > */
/* > ZGBBRD reduces a scomplex general m-by-n band matrix A to real upper */
...
void zgbbrd_(char *vect, aocl_int_t *m, aocl_int_t *n, aocl_int_t *ncc, aocl_int_t *kl,
             aocl_int_t *ku, dcomplex *ab, aocl_int_t *ldab, doublereal *d__, doublereal *e,
             dcomplex *q, aocl_int_t *ldq, dcomplex *pt, aocl_int_t *ldpt,
                        ...
                        aocl_int64_t *info)
{
    ...
    *info = 0;
    ...
    if(wantq)
    {
        aocl_lapack_zlaset("Full", m, m, &c_b1, &c_b2, &q[q_offset], ldq);
    }
    if(wantpt)
    {
        aocl_lapack_zlaset("Full", n, n, &c_b1, &c_b2, &pt[pt_offset], ldpt);
    }
        ...
        for(i__ = 1; i__ <= i__1; ++i__)
        {
            ...
            for(kk = 1; kk <= i__2; ++kk)
            {
                ...
                for(j = j1; i__4 < 0 ? j >= i__3 : j <= i__3; j += i__4)
                {
                    ...
                    i__6 = j;
                    i__7 = (j + kun) * ab_dim1 + 1;
                    z__1.real = rwork[i__6] * ab[i__7].real;
                    z__1.imag = rwork[i__6] * ab[i__7].imag; // , expr subst
                    ab[i__5].real = z__1.real;
                    ab[i__5].imag = z__1.imag; // , expr subst
                    /* L40: */
                }
                /* generate plane rotations to annihilate nonzero elements */
                ...
                if(ml == ml0 && mu > mu0)
                {
                    if(mu <= *n - i__ + 1)
                    {
                        ...
                        aocl_lapack_zrot(&i__4, &ab[*ku - mu + 4 + (i__ + mu - 2) * ab_dim1], &c__1,
    ...
    if(*ku == 0 && *kl > 0)
    {
        ...
        for(i__ = 1; i__ <= i__1; ++i__)
        {
            ...
            if(i__ < *n)
            {
                ...
                z__1.real = rs.real * ab[i__4].real - rs.imag * ab[i__4].imag;
                z__1.imag = rs.real * ab[i__4].imag + rs.imag * ab[i__4].real; // , expr subst
                ab[i__2].real = z__1.real;
                ab[i__2].imag = z__1.imag; // , expr subst
                i__2 = (i__ + 1) * ab_dim1 + 1;
                i__4 = (i__ + 1) * ab_dim1 + 1;
                z__1.real = rc * ab[i__4].real;
                z__1.imag = rc * ab[i__4].imag; // , expr subst
                ab[i__2].real = z__1.real;
                ab[i__2].imag = z__1.imag; // , expr subst
            }
            if(wantq)
            ...
            if(wantc)
            {
                aocl_lapack_zrot(ncc, &c__[i__ + c_dim1], ldc, &c__[i__ + 1 + c_dim1], ldc, &rc,
                                 &rs);
            }
            /* L100: */
        }
        ...
        if(*ku > 0 && *m < *n)
        {
            ...
            for(i__ = *m; i__ >= 1; --i__)
            {
                ...
                if(i__ > 1)
                {
                    ...
                    z__1.imag = rc * ab[i__2].imag; // , expr subst
                ...
                {
                    d_cnjg(&z__1, &rs);
                    aocl_lapack_zrot(n, &pt[i__ + pt_dim1], ldpt, &pt[*m + 1 + pt_dim1], ldpt, &rc,
                                     &z__1);
                }
                /* L110: */
            }
    ...
    for(i__ = 1; i__ <= i__1; ++i__)
    {
        ...
        if(abst != 0.)
        {
            z__1.real = t.real / abst;
            z__1.imag = t.imag / abst; // , expr subst
            t.real = z__1.real;
        ...
        if(wantq)
        {
            aocl_blas_zscal(m, &t, &q[i__ * q_dim1 + 1], &c__1);
        }
        if(wantc)
        {
            d_cnjg(&z__1, &t);
            aocl_blas_zscal(ncc, &z__1, &c__[i__ + c_dim1], ldc);
        }
        if(i__ < minmn)
        {
            if(*ku == 0 && *kl == 0)
            ...
            else
            {
                if(*ku == 0)
                {
                    i__2 = i__ * ab_dim1 + 2;
                    d_cnjg(&z__2, &t);
                    z__1.real = ab[i__2].real * z__2.real - ab[i__2].imag * z__2.imag;
                    z__1.imag = ab[i__2].real * z__2.imag + ab[i__2].imag * z__2.real; // , expr subst
                    t.real = z__1.real;
                    t.imag = z__1.imag; // , expr subst
                }
                else
                ...
                if(wantpt)
                {
                    aocl_blas_zscal(n, &t, &pt[i__ + 1 + pt_dim1], ldpt);
                }
