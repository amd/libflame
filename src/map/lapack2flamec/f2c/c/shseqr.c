...
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/shseqr.
 * f"> */
/* > [TGZ]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/shseqr.
 * f"> */
/* > [ZIP]</a> */
/* > <a
 * href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/shseqr.
 * f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
...
/* > scomplex conjugate pairs of eigenvalues) are returned in */
...
/* ===================================================================== */
/* Subroutine */
void shseqr_(char *job, char *compz, integer *n, integer *ilo, integer *ihi, real *h__, integer *ldh, real *wr, real *wi, real *z__, integer *ldz, real *work, integer *lwork, integer *info)
{
    ...
    logical wantt, wantz;
    extern /* Subroutine */
    void slaqr0_(logical *, logical *, integer *, integer *, integer *, real *, integer *, real *, real *, integer *, integer *, real *, integer *, real *, integer *, integer *), xerbla_(const char *srname, const integer *info, ftnlen srname_len);
    ...
    *info = 0;
    ...
    else if(lquery)
    {
        ...
        aocl_lapack_slaqr0(&wantt, &wantz, n, ilo, ihi, &h__[h_offset], ldh, &wr[1], &wi[1], ilo,
    ...
    else
    {
        ...
        if(initz)
        {
            aocl_lapack_slaset("A", n, n, &c_b11, &c_b12, &z__[z_offset], ldz);
        }
    ...
    AOCL_DTL_TRACE_LOG_EXIT
        ...
        if(*n > nmin)
        {
            aocl_lapack_slaqr0(&wantt, &wantz, n, ilo, ihi, &h__[h_offset], ldh, &wr[1], &wi[1],
                               ilo, ihi, &z__[z_offset], ldz, &work[1], lwork, info);
        }
        else
        {
            /* ==== Small matrix ==== */
            aocl_lapack_slahqr(&wantt, &wantz, n, ilo, ihi, &h__[h_offset], ldh, &wr[1], &wi[1],
            ...
            if(*info > 0)
            {
                ...
                if(*n >= 49)
                {
                    ...
                    aocl_lapack_slaqr0(&wantt, &wantz, n, ilo, &kbot, &h__[h_offset], ldh, &wr[1],
                ...
                else
                {
                    ...
                    aocl_lapack_slacpy("A", n, n, &h__[h_offset], ldh, hl, &c__49);
                    hl[*n + 1 + *n * 49 - 50] = 0.f;
                    i__1 = 49 - *n;
                    aocl_lapack_slaset("A", &c__49, &i__1, &c_b11, &c_b11, &hl[(*n + 1) * 49 - 49],
                                       &c__49);
                    aocl_lapack_slaqr0(&wantt, &wantz, &c__49, ilo, &kbot, hl, &c__49, &wr[1],
                                       &wi[1], ilo, ihi, &z__[z_offset], ldz, workl, &c__49, info);
                    if(wantt || *info != 0)
                    {
                        aocl_lapack_slacpy("A", n, n, hl, &c__49, &h__[h_offset], ldh);
                    }
                }
            }
        }
        ...
        if((wantt || *info != 0) && *n > 2)
        {
            i__1 = *n - 2;
            i__3 = *n - 2;
            aocl_lapack_slaset("L", &i__1, &i__3, &c_b11, &c_b11, &h__[h_dim1 + 3], ldh);
        }
        /* ==== Ensure reported workspace size is backward-compatible with */
        /* . previous LAPACK versions. ==== */
        /* Computing MAX */
